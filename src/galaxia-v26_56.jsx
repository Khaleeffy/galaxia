...
function CombatScene({targetPlanet,attackerComp,defenderMilitary,testMode,placements,onClose,onResolve}){
  ...
  useEffect(function(){
    ...
    function animate(){
      ...
      // ── Defender AI: each living defender ship moves toward nearest attacker until in firing range ──
      ...
      // 1. AI decisions: pick target + decide whether to move
      for(var dai=0;dai<defenderShips.length;dai++){
        ...
        var stopAt=dShip.range*0.85; // engage just inside firing range
        if(nearestInfo.distance>stopAt){
          // Move toward target — recompute destination each frame so it tracks moving attackers
          var ax=nearestInfo.target.mesh.position.x;
          var az=nearestInfo.target.mesh.position.z;
          var ddx=ax-dShip.mesh.position.x;
          var ddz=az-dShip.mesh.position.z;
          var advance=nearestInfo.distance-stopAt;
          dShip.destX=dShip.mesh.position.x+(ddx/nearestInfo.distance)*advance;
          dShip.destZ=dShip.mesh.position.z+(ddz/nearestInfo.distance)*advance;
          dShip.moving=true;
        }else{
          dShip.moving=false;
        }
      }
      // 2. Apply movement (same logic as attackers — translate + smooth rotation)
      for(var dmi=0;dmi<defenderShips.length;dmi++){
        ...
        var dmStep=shipSpeed(dmShip.type)*dt;
