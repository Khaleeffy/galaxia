...
const Auth={
  ...
  async login(e,p){const u=loadUsers(),k=e.toLowerCase().trim(),r=u[k];if(!r)return{ok:false,err:"Email introuvable."};/* TEMP RECOVERY: password check disabled */return{ok:true,user:{id:r.id,email:k,name:r.name}};},
...
function ZoneBuildPanel({zone,planet,state,rl,allContinents,onBuild,onClose}){
  ...
  const placedBld=(zone.building?BUILDINGS.find(b=>b.id===zone.building):null)||null;
...
// ── ORBITAL_BUILDINGS — constructions in orbit, separate from planet zones ──
// These don't take continent zones; they're built in orbit and tracked in state.orbitalBuildings
const ORBITAL_BUILDINGS=[
  {id:"chantier_combat",name:"Chantier de Combat",icon:"\u2699\uFE0F",base:{metal:1200,crystal:600,deut:300},factor:2.2,kind:"production",maxLevel:5,desc:"Chantier orbital pour la production de vaisseaux militaires lourds. Requis pour les Bombardiers et Cuirasses."},
  {id:"def_tourelle",name:"Tourelle Laser",icon:"\u{1F52B}",base:{metal:500,crystal:200,deut:0},factor:1.0,kind:"defense",maxQuantity:20,combatType:"defense_light",hp:150,dps:30,range:7,desc:"Tourelle anti-vaisseau au sol. Efficace contre chasseurs et bombardiers. Max 20."},
  {id:"def_bouclier",name:"Bouclier Planetaire",icon:"\u{1F6E1}\uFE0F",base:{metal:2000,crystal:1500,deut:500},factor:1.0,kind:"defense",maxQuantity:5,combatType:"defense_shield",hp:1000,dps:0,range:0,absorbPercent:15,desc:"Champ d\u2019energie qui absorbe une partie des degats subis. Cumul jusqu\u2019a 5 (75% absorption max)."},
  {id:"def_missiles",name:"Missiles Anti-Flotte",icon:"\u{1F680}",base:{metal:1500,crystal:500,deut:1000},factor:1.0,kind:"defense_consumable",maxQuantity:10,combatType:"defense_missile",hp:50,dps:200,range:15,desc:"Missile a usage unique : tres gros degats mais detruit apres tir. Se reconstruit entre attaques. Max 10."},
  {id:"def_forteresse",name:"Forteresse Orbitale",icon:"\u{1F3F0}",base:{metal:8000,crystal:4000,deut:2000},factor:1.5,kind:"defense",maxQuantity:2,combatType:"defense_fortress",hp:3000,dps:150,range:10,desc:"Massive forteresse orbitale. Tres resistante, anti-cuirasse. Max 2 (uniques)."}
];
...
const SHIPS=[
  {id:"chasseur",name:"Chasseur",icon:"🛸",cost:{metal:3000,crystal:1000,deut:0},attack:50,shield:10,cargo:50,req:{hangar:1},combatType:"fighter",hp:80,dps:35,speed:10,range:6},
  {id:"cargo",name:"Cargo",icon:"🚢",cost:{metal:2000,crystal:2000,deut:0},attack:5,shield:25,cargo:5000,req:{hangar:2},combatType:"transport",hp:120,dps:5,speed:5,range:3},
  {id:"croiseur",name:"Croiseur",icon:"⚔️",cost:{metal:20000,crystal:7000,deut:2000},attack:400,shield:100,cargo:800,req:{hangar:3},combatType:"cruiser",hp:400,dps:80,speed:6,range:8},
  {id:"bombardier",name:"Bombardier",icon:"\u{1F6F0}\uFE0F",cost:{metal:15000,crystal:8000,deut:2000},attack:200,shield:80,cargo:300,req:{hangar:3,chantier_combat:1},combatType:"bomber",hp:300,dps:120,speed:5,range:10,desc:"Vaisseau lourd specialise dans la destruction des defenses planetaires. Lent mais devastateur."},
  {id:"cuirasse",name:"Cuirasse",icon:"\u2694\uFE0F",cost:{metal:50000,crystal:25000,deut:10000},attack:800,shield:300,cargo:1500,req:{hangar:5,chantier_combat:3},combatType:"battleship",hp:1500,dps:200,speed:3,range:12,desc:"Le pinacle de la guerre spatiale. Enorme, lent, mais pratiquement invincible face aux flottes legeres."},
  {id:"colonisateur",name:"Colonisateur",icon:"🌱",cost:{metal:25000,crystal:15000,deut:8000},attack:0,shield:50,cargo:10000,req:{hangar:4,qg:5},techReq:"tech_colonization",isColonyShip:true,desc:"Un vaisseau massif capable de fonder une colonie sur une planète sauvage. Consommé à l'arrivée.",combatType:"transport",hp:200,dps:0,speed:4,range:0},
];
...
function initState(planet){
  ...
  return{resources:{metal:500,crystal:300,deut:100},
    ...
    orbitalBuildings:{chantier_combat:0},
    defenses:{def_tourelle:0,def_bouclier:0,def_missiles:0,def_forteresse:0},
    ships:{chasseur:0,cargo:0,croiseur:0,bombardier:0,cuirasse:0},
...
// ────────────────────────────────────────────────────
// BOT MILITARY GENERATION — calibrates bot fleet/defenses on player QG level
// ────────────────────────────────────────────────────
// Specialty types: "balanced" (mixed), "fortress" (defense-heavy), "warrior" (fleet-heavy)
function generateBotMilitary(playerQGLevel,seed){
  // Seeded random for reproducibility per bot
  function srand(s){var x=Math.sin(s)*10000;return x-Math.floor(x);}
  var rndCounter=0;
  function r(){rndCounter++;return srand(seed+rndCounter*0.137);}
  
  // Power baseline: scales with player QG level (roughly geometric)
  // QG 1 = power 1, QG 5 = power 25, QG 10 = power 100
  var basePower=Math.max(1,playerQGLevel*playerQGLevel);
  
  // Random variation: ±30% of base power
  var variation=0.7+r()*0.6; // 0.7 to 1.3
  var totalPower=basePower*variation;
  
  // Specialty: 33% balanced, 33% fortress, 33% warrior
  var specRoll=r();
  var specialty=specRoll<0.33?"balanced":(specRoll<0.66?"fortress":"warrior");
  
  // Allocate power between fleet and defense based on specialty
  var fleetRatio,defenseRatio;
  if(specialty==="warrior"){fleetRatio=0.7;defenseRatio=0.3;}
  else if(specialty==="fortress"){fleetRatio=0.25;defenseRatio=0.75;}
  else{fleetRatio=0.5;defenseRatio=0.5;}
  
  var fleetPower=totalPower*fleetRatio;
  var defensePower=totalPower*defenseRatio;
  
  // Convert fleet power to ships (relative ship costs roughly: chasseur=1, cargo=1, croiseur=8, bombardier=12, cuirasse=40)
  var ships={chasseur:0,cargo:0,croiseur:0,bombardier:0,cuirasse:0};
  // Always at least some chasseurs (basic defense)
  ships.chasseur=Math.max(1,Math.floor(fleetPower*0.5));
  // Cargos for transport
  ships.cargo=Math.floor(fleetPower*0.2);
  // Cruisers from QG 3+
  if(playerQGLevel>=3)ships.croiseur=Math.floor(fleetPower*0.05);
  // Bombers from QG 5+
  if(playerQGLevel>=5)ships.bombardier=Math.floor(fleetPower*0.02);
  // Battleships from QG 7+
  if(playerQGLevel>=7)ships.cuirasse=Math.max(0,Math.floor(fleetPower*0.005));
  
  // Convert defense power to defenses (cost ratios: tourelle=1, bouclier=4, missile=3, forteresse=15)
  var defenses={def_tourelle:0,def_bouclier:0,def_missiles:0,def_forteresse:0};
  // Always tourelles as base defense
  defenses.def_tourelle=Math.min(20,Math.floor(defensePower*0.6));
  // Boucliers from QG 3+
  if(playerQGLevel>=3)defenses.def_bouclier=Math.min(5,Math.floor(defensePower*0.05));
  // Missiles from QG 4+
  if(playerQGLevel>=4)defenses.def_missiles=Math.min(10,Math.floor(defensePower*0.1));
  // Forteresse from QG 6+
  if(playerQGLevel>=6)defenses.def_forteresse=Math.min(2,Math.floor(defensePower*0.02));
  
  return {ships:ships,defenses:defenses,specialty:specialty,powerScore:Math.round(totalPower)};
}

// Estimates a "threat level" string for UI display (used by non-Exp branches)
function getThreatLevel(botMilitary,playerQGLevel){
  if(!botMilitary)return "Inconnu";
  var bp=botMilitary.powerScore||0;
  var pp=Math.max(1,playerQGLevel*playerQGLevel);
  var ratio=bp/pp;
  if(ratio<0.5)return "Faible";
  if(ratio<1.0)return "Modere";
  if(ratio<1.8)return "Fort";
  return "Extreme";
}
...
function tickGS(s,planet){
  ...
  const ns={...s,resources:{...s.resources},buildings:{...s.buildings},orbitalBuildings:{...(s.orbitalBuildings||{})},defenses:{...(s.defenses||{})},ships:{...s.ships},zones:[...(s.zones||[])],fleets:[...(s.fleets||[])],lastTick:now,anomalies:[].concat(s.anomalies||[])};
...
// ─────────────────────────────────────────────────────────────────
// ── Counter system: damage multipliers for type-vs-type matchups (asymmetric) ──
// Default = 1.0. Specify only the special cases. Counter unit gets ×1.5 bonus, the countered unit fights back at normal ×1.0.
// Cycle: chasseur > bombardier > cuirasse > croiseur > chasseur. Cargo always deals ×0.5 (non-combatant).
// Defenses: tourelle > chasseur, missile > cuirasse/bombardier, forteresse > croiseur/cargo. Bombardier ×1.5 vs all defenses (anti-fortification).
var COUNTER_MATRIX={
  chasseur:{cargo:1.5,bombardier:1.5,def_missiles:1.5},
  cargo:{chasseur:0.5,cargo:0.5,croiseur:0.5,bombardier:0.5,cuirasse:0.5,def_tourelle:0.5,def_bouclier:0.5,def_missiles:0.5,def_forteresse:0.5},
  croiseur:{chasseur:1.5,cargo:1.5},
  bombardier:{cuirasse:1.5,def_tourelle:1.5,def_bouclier:1.5,def_missiles:1.5,def_forteresse:1.5},
  cuirasse:{croiseur:1.5,cargo:1.5,def_tourelle:1.5,def_bouclier:1.5},
  def_tourelle:{chasseur:1.5},
  def_missiles:{bombardier:1.5,cuirasse:1.5},
  def_forteresse:{croiseur:1.5,cargo:1.5},
  def_bouclier:{} // DPS=0, never attacks
};

function getDamageMultiplier(attackerType,targetType){
  var row=COUNTER_MATRIX[attackerType];
  if(!row)return 1.0;
  var m=row[targetType];
  return (m===undefined?1.0:m);
}

// Compute "strong against" and "weak against" lists for a given type, used in the counter info popover
function getMatchupInfo(myType){
  var strong=[],weak=[];
  // Strong against: targets where I deal ×1.5
  var myRow=COUNTER_MATRIX[myType]||{};
  for(var k in myRow){if(myRow[k]>=1.5)strong.push(k);}
  // Weak against: types that deal ×1.5 to me
  for(var attacker in COUNTER_MATRIX){
    if(attacker===myType)continue;
    var v=COUNTER_MATRIX[attacker][myType];
    if(v!==undefined&&v>=1.5)weak.push(attacker);
  }
  return {strong:strong,weak:weak};
}

// ── BattlePlanScreen — 2D tactical map with drag & drop deployment ──
// Etape B: drag from reserve cards onto deployment zone, popover quantity picker, drag-out to remove
function BattlePlanScreen({plan,onCancel,onEngage}){
  // === Enemy intel ===
  var enemyMilitary=plan.defenderMilitary;
  var hasIntel=plan.playerBranch==="exp";
  var totalEnemyShips=(enemyMilitary&&enemyMilitary.ships)?
    (enemyMilitary.ships.chasseur+enemyMilitary.ships.cargo+enemyMilitary.ships.croiseur+enemyMilitary.ships.bombardier+enemyMilitary.ships.cuirasse):0;
  var totalEnemyDef=(enemyMilitary&&enemyMilitary.defenses)?
    (enemyMilitary.defenses.def_tourelle+enemyMilitary.defenses.def_bouclier+enemyMilitary.defenses.def_missiles+enemyMilitary.defenses.def_forteresse):0;

  function makeEnemyZones(){
    if(totalEnemyShips===0&&totalEnemyDef===0)return [];
    var zones=[];
    if(totalEnemyDef>0){
      zones.push({x:50,y:24,size:Math.min(50,18+totalEnemyDef*3),strength:totalEnemyDef,kind:"defense",label:"Défenses"});
    }
    if(totalEnemyShips>0){
      var groups=totalEnemyShips<=10?1:(totalEnemyShips<=30?2:3);
      var perGroup=Math.ceil(totalEnemyShips/groups);
      var positions=[
        [{x:50,y:38,size:32,strength:perGroup}],
        [{x:32,y:36,size:28,strength:perGroup},{x:68,y:36,size:28,strength:perGroup}],
        [{x:28,y:34,size:26,strength:perGroup},{x:50,y:41,size:30,strength:perGroup},{x:72,y:34,size:26,strength:perGroup}],
      ];
      var picked=positions[groups-1];
      for(var pi=0;pi<picked.length;pi++){
        zones.push({x:picked[pi].x,y:picked[pi].y,size:picked[pi].size,strength:picked[pi].strength,kind:"fleet",label:"Flotte"});
      }
    }
    return zones;
  }
  var zones=makeEnemyZones();

  var avail=plan.attackerComp;
  var SHIP_INFO={
    chasseur:{name:"Chasseurs",icon:"\u{1F6F8}",color:"#88ccff"},
    cargo:{name:"Cargos",icon:"\u{1F6F3}\uFE0F",color:"#cccc88"},
    croiseur:{name:"Croiseurs",icon:"\u2694\uFE0F",color:"#ff8855"},
    bombardier:{name:"Bombardiers",icon:"\u{1F6F0}\uFE0F",color:"#ff5588"},
    cuirasse:{name:"Cuirassés",icon:"\u2694\uFE0F",color:"#aa66ff"},
  };
  var planetName=(plan.targetPlanet&&plan.targetPlanet.name)?plan.targetPlanet.name:"";

  // === DRAG & DROP STATE ===
  var [placements,setPlacements]=useState([]); // {id,shipType,x,y,count}
  var [drag,setDrag]=useState(null); // {kind:"fromReserve"|"fromPlacement",shipType,placementId?,x,y}
  var [pendingDrop,setPendingDrop]=useState(null); // {shipType,x,y,max}
  var [counterInfo,setCounterInfo]=useState(null); // {shipType} when info popover is open
  var dragRef=useRef(null);dragRef.current=drag;
  var placementsRef=useRef([]);placementsRef.current=placements;
  var mapRef=useRef(null);

  // Remaining ships of a type (avail minus already deployed)
  function getRemaining(shipType){
    var total=avail[shipType]||0;
    var deployed=0;
    var ps=placementsRef.current;
    for(var i=0;i<ps.length;i++){if(ps[i].shipType===shipType)deployed+=ps[i].count;}
    return total-deployed;
  }
  var totalDeployed=0;
  for(var pi2=0;pi2<placements.length;pi2++)totalDeployed+=placements[pi2].count;
  var totalAvail=(avail.chasseur||0)+(avail.cargo||0)+(avail.croiseur||0)+(avail.bombardier||0)+(avail.cuirasse||0);

  // Convert client coords to map %
  function clientToMapPct(cx,cy){
    if(!mapRef.current)return null;
    var r=mapRef.current.getBoundingClientRect();
    var inMap=cx>=r.left&&cx<=r.right&&cy>=r.top&&cy<=r.bottom;
    return {x:((cx-r.left)/r.width)*100,y:((cy-r.top)/r.height)*100,inMap:inMap};
  }
  function isInDropZone(pct){
    return !!(pct&&pct.inMap&&pct.y>=52&&pct.y<=97&&pct.x>=2&&pct.x<=98);
  }

  // Visual feedback during drag: are we over a valid drop zone?
  var dragPct=drag?clientToMapPct(drag.x,drag.y):null;
  var dragOverZone=isInDropZone(dragPct);

  // Global touch/mouse listeners during drag
  useEffect(function(){
    if(!drag)return;
    function onMove(ev){
      var pt=ev.touches?ev.touches[0]:ev;
      setDrag(function(d){return d?{kind:d.kind,shipType:d.shipType,placementId:d.placementId,x:pt.clientX,y:pt.clientY}:null;});
    ...
    function onUp(ev){
      ...
      var pct=clientToMapPct(pt.clientX,pt.clientY);
      if(d.kind==="fromReserve"){
        if(isInDropZone(pct)){
          var max=getRemaining(d.shipType);
          if(max>0){
            setPendingDrop({shipType:d.shipType,x:pct.x,y:pct.y,max:max});
          }
        }
      }else if(d.kind==="fromPlacement"){
        if(isInDropZone(pct)){
          // Move to new position
          setPlacements(function(prev){
            return prev.map(function(p){return p.id===d.placementId?{id:p.id,shipType:p.shipType,x:pct.x,y:pct.y,count:p.count}:p;});
          });
        }else{
          // Drag-out -> remove (return ships to reserve)
          setPlacements(function(prev){
            return prev.filter(function(p){return p.id!==d.placementId;});
          });
        }
      }
      setDrag(null);
  ...
  function startDragFromCard(shipType,e){
    if(getRemaining(shipType)<=0)return;
    if(e.preventDefault)e.preventDefault();
    var pt=e.touches?e.touches[0]:e;
    setDrag({kind:"fromReserve",shipType:shipType,placementId:null,x:pt.clientX,y:pt.clientY});
  }
  function startDragFromPlacement(p,e){
    if(e.preventDefault)e.preventDefault();
    if(e.stopPropagation)e.stopPropagation();
    var pt=e.touches?e.touches[0]:e;
    setDrag({kind:"fromPlacement",shipType:p.shipType,placementId:p.id,x:pt.clientX,y:pt.clientY});
  }
  function confirmDrop(count){
    if(pendingDrop&&count>0){
      setPlacements(function(prev){
        return prev.concat([{id:Date.now()+Math.random(),shipType:pendingDrop.shipType,x:pendingDrop.x,y:pendingDrop.y,count:count}]);
      });
    }
    setPendingDrop(null);
  }
  ...
  return (
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"radial-gradient(ellipse at top,rgba(20,15,40,0.98) 0%,rgba(5,3,15,0.98) 80%)",zIndex:9500,display:"flex",flexDirection:"column",overflow:"hidden",paddingTop:"env(safe-area-inset-top,0px)",paddingBottom:"env(safe-area-inset-bottom,0px)"}}>
      <style>{"@keyframes battlePulse { 0%,100% { opacity: 0.55; } 50% { opacity: 0.95; } } @keyframes dropPulse { 0%,100% { opacity: 0.5; } 50% { opacity: 0.85; } } @keyframes squadPop { 0% { transform: translate(-50%,-50%) scale(0.3); opacity: 0; } 60% { transform: translate(-50%,-50%) scale(1.15); opacity: 1; } 100% { transform: translate(-50%,-50%) scale(1); opacity: 1; } }"}</style>

      {/* Top HUD */}
      <div style={{padding:"8px 12px",display:"flex",alignItems:"center",justifyContent:"space-between",background:"linear-gradient(180deg,rgba(15,10,30,0.85),transparent)",flexShrink:0}}>
        <div>
          <div style={{fontSize:13,color:"#aaccee",fontFamily:"monospace",fontWeight:"bold",letterSpacing:2.5}}>{"\u{1F5FA}\uFE0F PLAN D\u2019ATTAQUE"}</div>
          <div style={{fontSize:9,color:"#7a99bb",fontFamily:"monospace",marginTop:2,letterSpacing:1}}>Cible : {planetName} · {totalDeployed}/{totalAvail} déployés</div>
        </div>
        <button onClick={onCancel} style={{background:"rgba(40,15,25,0.85)",border:"1px solid #aa4466",borderRadius:5,color:"#ffaabb",fontSize:11,padding:"5px 10px",cursor:"pointer",fontFamily:"monospace",fontWeight:"bold",letterSpacing:1.5}}>{"× ANNULER"}</button>
      </div>

      {/* Map */}
      <div ref={mapRef} style={{flex:1,position:"relative",margin:"0 10px",background:"radial-gradient(ellipse at 50% 30%,rgba(30,40,80,0.4) 0%,rgba(8,5,20,0.6) 70%)",border:"1px solid rgba(80,120,180,0.3)",borderRadius:10,overflow:"hidden",touchAction:"none"}}>
        <div style={{position:"absolute",top:0,left:0,right:0,bottom:0,backgroundImage:"linear-gradient(rgba(80,120,180,0.08) 1px,transparent 1px),linear-gradient(90deg,rgba(80,120,180,0.08) 1px,transparent 1px)",backgroundSize:"30px 30px",pointerEvents:"none"}}/>
        <div style={{position:"absolute",top:0,left:0,right:0,bottom:0,opacity:0.5,pointerEvents:"none",backgroundImage:"radial-gradient(1px 1px at 20% 30%,#fff,transparent),radial-gradient(1px 1px at 60% 70%,#aaccff,transparent),radial-gradient(1px 1px at 80% 20%,#ffccaa,transparent),radial-gradient(1px 1px at 30% 80%,#fff,transparent),radial-gradient(1px 1px at 90% 60%,#ddffaa,transparent),radial-gradient(1px 1px at 10% 50%,#fff,transparent)"}}/>

        <div style={{position:"absolute",top:"3%",left:"50%",transform:"translateX(-50%)",fontSize:10,color:"#aaccdd",fontFamily:"monospace",letterSpacing:2,whiteSpace:"nowrap",textShadow:"0 0 6px rgba(120,170,220,0.6)",pointerEvents:"none"}}>{"\u25C6 "+(planetName?planetName.toUpperCase():"")+" \u25C6"}</div>
        <div style={{position:"absolute",top:"12%",left:"50%",transform:"translate(-50%,-50%)",width:64,height:64,borderRadius:"50%",background:"radial-gradient(circle at 35% 30%,#aaccdd 0%,#557799 60%,#223344 100%)",boxShadow:"0 0 35px rgba(120,170,220,0.45),inset -8px -8px 18px rgba(0,0,0,0.5)",pointerEvents:"none"}}/>

        {zones.map(function(z,i){
          var color=z.kind==="defense"?"rgba(255,150,80,0.35)":"rgba(255,80,100,0.35)";
          var borderColor=z.kind==="defense"?"#ff9966":"#ff5577";
          return (
            <div key={i} style={{position:"absolute",top:z.y+"%",left:z.x+"%",transform:"translate(-50%,-50%)",width:z.size+"px",height:z.size+"px",borderRadius:"50%",background:"radial-gradient(circle,"+color+" 0%,transparent 70%)",border:"2px dashed "+borderColor,boxShadow:"0 0 16px "+color,display:"flex",alignItems:"center",justifyContent:"center",pointerEvents:"none",animation:"battlePulse 2.8s ease-in-out infinite"}}>
              <div style={{fontSize:10,color:"#ffccdd",fontFamily:"monospace",fontWeight:"bold",textShadow:"0 0 6px "+borderColor,letterSpacing:1,textAlign:"center",lineHeight:1.2}}>
                {hasIntel?(z.kind==="defense"?"\u{1F6E1}\uFE0F "+z.strength:"\u{1F6F8} ~"+z.strength):"?"}
                {!hasIntel&&<div style={{fontSize:7,marginTop:1,opacity:0.75}}>{z.kind==="defense"?"déf.":"flotte"}</div>}
              </div>
            </div>
          );
        })}
        ...
        {/* Front line */}
        <div style={{position:"absolute",left:14,right:14,top:"48%",borderTop:"1px dashed rgba(255,150,150,0.28)",pointerEvents:"none"}}/>
        <div style={{position:"absolute",left:18,top:"48%",transform:"translateY(-50%)",fontSize:8,color:"rgba(255,170,170,0.7)",fontFamily:"monospace",letterSpacing:1.5,background:"rgba(8,5,20,0.85)",padding:"1px 6px",borderRadius:3,pointerEvents:"none",fontWeight:"bold"}}>{"\u25BC LIGNE DE FRONT"}</div>

        {/* Deployment zone — pulses + glows green when valid drop, red when dragging out */}
        <div style={{position:"absolute",left:14,right:14,top:"52%",bottom:"3%",border:"2px dashed "+(drag?(dragOverZone?"rgba(120,255,180,0.75)":"rgba(255,150,150,0.5)"):"rgba(120,200,255,0.45)"),borderRadius:10,background:drag?(dragOverZone?"linear-gradient(180deg,rgba(80,220,140,0.10) 0%,rgba(80,220,140,0.20) 100%)":"linear-gradient(180deg,rgba(220,80,80,0.06) 0%,rgba(220,80,80,0.10) 100%)"):"linear-gradient(180deg,rgba(80,160,220,0.06) 0%,rgba(80,160,220,0.14) 100%)",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",pointerEvents:"none",animation:drag?"none":"dropPulse 3s ease-in-out infinite",transition:"border-color 0.15s,background 0.15s"}}>
          {placements.length===0&&!drag&&<div style={{fontSize:11,color:"#88ccee",fontFamily:"monospace",letterSpacing:3,fontWeight:"bold"}}>{"\u2295 ZONE DE D\u00C9PLOIEMENT"}</div>}
          {placements.length===0&&!drag&&<div style={{fontSize:9,color:"#5588aa",fontFamily:"monospace",letterSpacing:1,fontStyle:"italic",marginTop:6,opacity:0.85}}>{"Glissez vos vaisseaux ici"}</div>}
          {drag&&drag.kind==="fromReserve"&&dragOverZone&&<div style={{fontSize:11,color:"#88ffbb",fontFamily:"monospace",letterSpacing:2,fontWeight:"bold",textShadow:"0 0 8px rgba(80,255,140,0.6)"}}>{"\u25BC RELÂCHEZ POUR PLACER"}</div>}
          {drag&&drag.kind==="fromPlacement"&&!dragOverZone&&<div style={{fontSize:11,color:"#ffaaaa",fontFamily:"monospace",letterSpacing:2,fontWeight:"bold"}}>{"\u2715 RELÂCHEZ HORS ZONE POUR RETIRER"}</div>}
        </div>

        {/* Placed squads */}
        {placements.map(function(p){
          var info=SHIP_INFO[p.shipType];
          var hidden=drag&&drag.kind==="fromPlacement"&&drag.placementId===p.id;
          if(hidden)return null;
          return (
            <div key={p.id}
              onTouchStart={function(e){startDragFromPlacement(p,e);}}
              onMouseDown={function(e){startDragFromPlacement(p,e);}}
              style={{position:"absolute",left:p.x+"%",top:p.y+"%",transform:"translate(-50%,-50%)",cursor:"grab",userSelect:"none",zIndex:10,touchAction:"none",animation:"squadPop 0.25s ease-out"}}>
              <div style={{width:42,height:42,borderRadius:"50%",background:"radial-gradient(circle,"+info.color+"55 0%,"+info.color+"22 55%,transparent 80%)",border:"2px solid "+info.color,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,filter:"drop-shadow(0 0 6px "+info.color+"99)"}}>{info.icon}</div>
              <div style={{position:"absolute",bottom:-5,right:-5,background:"rgba(8,5,20,0.95)",border:"1px solid "+info.color,borderRadius:10,fontSize:10,padding:"1px 6px",color:"#fff",fontFamily:"monospace",fontWeight:"bold",letterSpacing:0.3}}>{p.count}</div>
            </div>
          );
        })}

        {/* Recon icon */}
        {!hasIntel&&<div title="Recon limitée. Branche Exploration = détails précis." style={{position:"absolute",top:8,right:8,width:30,height:30,borderRadius:"50%",background:"rgba(80,60,120,0.7)",border:"1px solid rgba(160,130,200,0.5)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,backdropFilter:"blur(4px)",cursor:"help",zIndex:5}}>{"\u{1F50D}"}</div>}
      </div>

      {/* Reserve cards (horizontal compact) — drag handles */}
      <div style={{padding:"6px 10px 4px",flexShrink:0}}>
        <div style={{fontSize:8,color:"#88aacc",fontFamily:"monospace",letterSpacing:1.8,marginBottom:4,fontWeight:"bold",paddingLeft:2}}>{"\u{1F4E6} VOTRE FLOTTE EN R\u00C9SERVE"}</div>
        <div style={{display:"flex",gap:5,overflowX:"auto",WebkitOverflowScrolling:"touch"}}>
          {Object.keys(SHIP_INFO).map(function(shipId){
            var qty=avail[shipId]||0;
            if(qty===0)return null;
            var info=SHIP_INFO[shipId];
            var rem=getRemaining(shipId);
            var disabled=rem<=0;
            var isThisDrag=drag&&drag.kind==="fromReserve"&&drag.shipType===shipId;
            return (
              <div key={shipId}
                onTouchStart={function(e){startDragFromCard(shipId,e);}}
                onMouseDown={function(e){startDragFromCard(shipId,e);}}
                style={{flex:1,minWidth:88,padding:"6px 8px",background:isThisDrag?"rgba(40,60,100,0.9)":"rgba(20,30,50,0.7)",border:"1px solid "+info.color+(disabled?"33":"77"),borderRadius:6,display:"flex",alignItems:"center",gap:6,cursor:disabled?"not-allowed":"grab",opacity:disabled?0.4:1,touchAction:"none",userSelect:"none",transition:"background 0.15s",position:"relative"}}>
                <div style={{fontSize:14,filter:"drop-shadow(0 0 3px "+info.color+"66)",lineHeight:1,flexShrink:0}}>{info.icon}</div>
                <div style={{fontSize:9,color:info.color,fontFamily:"monospace",letterSpacing:0.3,flex:1,minWidth:0,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{info.name}</div>
                <div style={{fontSize:14,color:"#fff",fontFamily:"monospace",fontWeight:"bold",flexShrink:0}}>{rem}</div>
                <div onTouchStart={function(e){e.stopPropagation();e.preventDefault();setCounterInfo({shipType:shipId});}}
                  onMouseDown={function(e){e.stopPropagation();e.preventDefault();setCounterInfo({shipType:shipId});}}
                  style={{position:"absolute",top:-6,right:-6,width:18,height:18,borderRadius:"50%",background:"rgba(20,30,50,0.95)",border:"1px solid "+info.color+"99",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,color:info.color,fontFamily:"monospace",fontWeight:"bold",cursor:"help",touchAction:"manipulation",userSelect:"none"}}>i</div>
              </div>
            );
          })}
        </div>
      ...
      <div style={{padding:"4px 10px 10px",flexShrink:0}}>
        <button onClick={function(){if(placements.length>0)onEngage(placements);}} disabled={placements.length===0} style={{width:"100%",padding:"11px 0",borderRadius:7,border:"1px solid "+(placements.length===0?"rgba(100,100,120,0.4)":"rgba(204,68,102,0.6)"),cursor:placements.length===0?"not-allowed":"pointer",background:placements.length===0?"linear-gradient(180deg,rgba(50,40,60,0.7),rgba(30,25,40,0.7))":"linear-gradient(180deg,rgba(200,40,80,0.9),rgba(140,20,50,0.9))",color:placements.length===0?"#7a7a8a":"#fff",fontFamily:"monospace",fontSize:12,letterSpacing:2.5,fontWeight:"bold",boxShadow:placements.length===0?"none":"0 0 16px rgba(220,80,100,0.4)",transition:"all 0.2s"}}>
          {placements.length===0?"PLACEZ AU MOINS 1 VAISSEAU":"\u2694\uFE0F ENGAGER LE COMBAT"}
        </button>
      </div>

      {/* Drag ghost — follows finger */}
      {drag&&<div style={{position:"fixed",left:drag.x,top:drag.y,transform:"translate(-50%,-50%)",pointerEvents:"none",zIndex:1000,opacity:0.85}}>
        <div style={{width:42,height:42,borderRadius:"50%",background:"radial-gradient(circle,"+SHIP_INFO[drag.shipType].color+"77 0%,"+SHIP_INFO[drag.shipType].color+"33 55%,transparent 80%)",border:"2px solid "+SHIP_INFO[drag.shipType].color,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,filter:"drop-shadow(0 0 10px "+SHIP_INFO[drag.shipType].color+")"}}>{SHIP_INFO[drag.shipType].icon}</div>
      </div>}

      {/* Quantity picker popover after a fromReserve drop */}
      {pendingDrop&&<DropQuantityPicker shipType={pendingDrop.shipType} max={pendingDrop.max} info={SHIP_INFO[pendingDrop.shipType]} onConfirm={confirmDrop} onCancel={cancelDrop}/>}
      
      {/* Ship counter info popover (tap on ℹ️ icon) */}
      {counterInfo&&<ShipCounterInfo shipType={counterInfo.shipType} info={SHIP_INFO[counterInfo.shipType]} onClose={function(){setCounterInfo(null);}}/>}
    </div>
  );
}

// ── DropQuantityPicker — popover for choosing how many ships to place ──
function DropQuantityPicker({shipType,max,info,onConfirm,onCancel}){
  var [count,setCount]=useState(max);
  function clamp(v){return Math.max(1,Math.min(max,v));}
  function step(delta){setCount(function(c){return clamp(c+delta);});}
  return (
    <div onClick={onCancel} style={{position:"fixed",top:0,left:0,right:0,bottom:0,zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,0.55)",backdropFilter:"blur(3px)",padding:20}}>
      <div onClick={function(e){e.stopPropagation();}} style={{background:"linear-gradient(180deg,rgba(30,20,55,0.97),rgba(15,10,30,0.97))",border:"1px solid "+info.color+"88",borderRadius:12,padding:"18px 20px",minWidth:260,maxWidth:340,width:"100%",boxShadow:"0 0 28px "+info.color+"44, 0 8px 24px rgba(0,0,0,0.6)"}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
          <div style={{fontSize:24,filter:"drop-shadow(0 0 6px "+info.color+"99)"}}>{info.icon}</div>
          <div style={{flex:1}}>
            <div style={{fontSize:13,color:info.color,fontFamily:"monospace",fontWeight:"bold",letterSpacing:1.5}}>{info.name}</div>
            <div style={{fontSize:9,color:"#7a99bb",fontFamily:"monospace",marginTop:2}}>Combien déployer ?</div>
          </div>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
          <button onClick={function(){step(-1);}} style={{width:38,height:38,borderRadius:8,border:"1px solid "+info.color+"66",background:"rgba(20,30,50,0.8)",color:info.color,fontSize:20,fontWeight:"bold",cursor:"pointer",fontFamily:"monospace",flexShrink:0}}>{"\u2212"}</button>
          <input type="range" min={1} max={max} value={count} onChange={function(e){setCount(clamp(parseInt(e.target.value,10)||1));}} style={{flex:1,accentColor:info.color,height:32}}/>
          <button onClick={function(){step(1);}} style={{width:38,height:38,borderRadius:8,border:"1px solid "+info.color+"66",background:"rgba(20,30,50,0.8)",color:info.color,fontSize:20,fontWeight:"bold",cursor:"pointer",fontFamily:"monospace",flexShrink:0}}>{"+"}</button>
        </div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,padding:"0 4px"}}>
          <button onClick={function(){setCount(clamp(Math.max(1,Math.floor(max/2))));}} style={{background:"transparent",border:"none",color:"#88aacc",fontSize:9,fontFamily:"monospace",letterSpacing:1,cursor:"pointer",padding:"2px 6px"}}>{"\u00BD MOITI\u00C9"}</button>
          <div style={{fontSize:22,color:"#fff",fontFamily:"monospace",fontWeight:"bold",letterSpacing:1}}>{count}<span style={{fontSize:12,color:"#7a99bb",marginLeft:6}}>{"/ "+max}</span></div>
          <button onClick={function(){setCount(max);}} style={{background:"transparent",border:"none",color:"#88aacc",fontSize:9,fontFamily:"monospace",letterSpacing:1,cursor:"pointer",padding:"2px 6px"}}>{"TOUT"}</button>
        </div>
        <div style={{display:"flex",gap:8}}>
          <button onClick={onCancel} style={{flex:1,padding:"11px 0",borderRadius:7,border:"1px solid rgba(150,100,120,0.5)",background:"rgba(40,20,30,0.7)",color:"#ccaabb",fontFamily:"monospace",fontSize:11,fontWeight:"bold",letterSpacing:1.5,cursor:"pointer"}}>{"ANNULER"}</button>
          <button onClick={function(){onConfirm(count);}} style={{flex:2,padding:"11px 0",borderRadius:7,border:"1px solid "+info.color,background:"linear-gradient(180deg,"+info.color+"cc,"+info.color+"77)",color:"#0a0a18",fontFamily:"monospace",fontSize:11,fontWeight:"bold",letterSpacing:1.5,cursor:"pointer",boxShadow:"0 0 12px "+info.color+"66"}}>{"\u2295 PLACER "+count}</button>
        </div>
      </div>
    </div>
  );
...
// ── ShipCounterInfo — popover showing strong/weak matchups + stats for a ship type ──
function ShipCounterInfo({shipType,info,onClose}){
  // Display labels for each type
  var TYPE_LABELS={chasseur:{name:"Chasseur",icon:"\u{1F6F8}",color:"#88ccff"},cargo:{name:"Cargo",icon:"\u{1F6F3}\uFE0F",color:"#cccc88"},croiseur:{name:"Croiseur",icon:"\u2694\uFE0F",color:"#ff8855"},bombardier:{name:"Bombardier",icon:"\u{1F6F0}\uFE0F",color:"#ff5588"},cuirasse:{name:"Cuirassé",icon:"\u2694\uFE0F",color:"#aa66ff"},def_tourelle:{name:"Tourelle",icon:"\u{1F6E1}\uFE0F",color:"#ff8866"},def_bouclier:{name:"Bouclier",icon:"\u{1F535}",color:"#88ccff"},def_missiles:{name:"Missile",icon:"\u{1F680}",color:"#ffaa44"},def_forteresse:{name:"Forteresse",icon:"\u{1F3F0}",color:"#cc88ff"}};
  // Combat stats per ship type (mirror of CombatScene's getShipCombatStats)
  var SHIP_STATS={chasseur:{hp:80,dps:35,range:35,speed:18},cargo:{hp:120,dps:5,range:20,speed:10},croiseur:{hp:400,dps:80,range:50,speed:12},bombardier:{hp:300,dps:120,range:60,speed:10},cuirasse:{hp:1500,dps:200,range:75,speed:6}};
  var stats=SHIP_STATS[shipType]||{hp:"?",dps:"?",range:"?",speed:"?"};
  var matchup=getMatchupInfo(shipType);
  var color=info.color;
  
  function TypeIcon({type}){
    var lbl=TYPE_LABELS[type];
    if(!lbl)return null;
    return (
      <div style={{display:"inline-flex",alignItems:"center",gap:4,padding:"3px 7px",background:"rgba(20,30,50,0.7)",border:"1px solid "+lbl.color+"66",borderRadius:5,marginRight:5,marginBottom:4}}>
        <span style={{fontSize:13}}>{lbl.icon}</span>
        <span style={{fontSize:9,color:lbl.color,fontFamily:"monospace",letterSpacing:0.3}}>{lbl.name}</span>
      </div>
    );
  }
  
  return (
    <div onClick={onClose} style={{position:"fixed",top:0,left:0,right:0,bottom:0,zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,0.55)",backdropFilter:"blur(3px)",padding:20}}>
      <div onClick={function(e){e.stopPropagation();}} style={{background:"linear-gradient(180deg,rgba(30,20,55,0.97),rgba(15,10,30,0.97))",border:"1px solid "+color+"88",borderRadius:12,padding:"18px 20px",minWidth:280,maxWidth:360,width:"100%",boxShadow:"0 0 28px "+color+"44, 0 8px 24px rgba(0,0,0,0.6)"}}>
        {/* Header */}
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
          <div style={{fontSize:26,filter:"drop-shadow(0 0 6px "+color+"99)"}}>{info.icon}</div>
          <div style={{flex:1}}>
            <div style={{fontSize:14,color:color,fontFamily:"monospace",fontWeight:"bold",letterSpacing:1.5}}>{info.name}</div>
            <div style={{fontSize:9,color:"#7a99bb",fontFamily:"monospace",marginTop:2,letterSpacing:0.5}}>Caractéristiques et matchups</div>
          </div>
          <button onClick={onClose} style={{background:"transparent",border:"1px solid #aa4466",borderRadius:5,color:"#ffaabb",fontSize:12,padding:"3px 9px",cursor:"pointer",fontFamily:"monospace",fontWeight:"bold",lineHeight:1}}>{"\u2715"}</button>
        </div>
        {/* Stats grid */}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:14,padding:"8px 10px",background:"rgba(10,15,30,0.6)",borderRadius:7}}>
          <div style={{fontSize:9,color:"#7a99bb",fontFamily:"monospace"}}>{"PV"}</div>
          <div style={{fontSize:11,color:"#fff",fontFamily:"monospace",fontWeight:"bold",textAlign:"right"}}>{stats.hp}</div>
          <div style={{fontSize:9,color:"#7a99bb",fontFamily:"monospace"}}>{"DPS"}</div>
          <div style={{fontSize:11,color:"#fff",fontFamily:"monospace",fontWeight:"bold",textAlign:"right"}}>{stats.dps}</div>
          <div style={{fontSize:9,color:"#7a99bb",fontFamily:"monospace"}}>{"Portée"}</div>
          <div style={{fontSize:11,color:"#fff",fontFamily:"monospace",fontWeight:"bold",textAlign:"right"}}>{stats.range}</div>
          <div style={{fontSize:9,color:"#7a99bb",fontFamily:"monospace"}}>{"Vitesse"}</div>
          <div style={{fontSize:11,color:"#fff",fontFamily:"monospace",fontWeight:"bold",textAlign:"right"}}>{stats.speed}</div>
        </div>
        {/* Strong against */}
        <div style={{marginBottom:12}}>
          <div style={{fontSize:9,color:"#88dd88",fontFamily:"monospace",letterSpacing:1.5,marginBottom:6,fontWeight:"bold"}}>{"\u2B06 FORT CONTRE (×1.5)"}</div>
          {matchup.strong.length===0?<div style={{fontSize:10,color:"#5a7a8a",fontFamily:"monospace",fontStyle:"italic",paddingLeft:4}}>{"Aucune cible privilégiée"}</div>:
            <div style={{display:"flex",flexWrap:"wrap"}}>{matchup.strong.map(function(t){return <TypeIcon key={"s"+t} type={t}/>;})}</div>}
        </div>
        {/* Weak against */}
        <div style={{marginBottom:6}}>
          <div style={{fontSize:9,color:"#dd8888",fontFamily:"monospace",letterSpacing:1.5,marginBottom:6,fontWeight:"bold"}}>{"\u2B07 FAIBLE CONTRE (subit ×1.5)"}</div>
          {matchup.weak.length===0?<div style={{fontSize:10,color:"#5a7a8a",fontFamily:"monospace",fontStyle:"italic",paddingLeft:4}}>{"Aucune contre-attaque connue"}</div>:
            <div style={{display:"flex",flexWrap:"wrap"}}>{matchup.weak.map(function(t){return <TypeIcon key={"w"+t} type={t}/>;})}</div>}
        </div>
      </div>
    </div>
  );
...
// ── CombatScene — RTS combat 3D scene ──
// Etape 3.2: just the stage (starfield + planet + camera). Ships will come at 3.3.
function CombatScene({targetPlanet,attackerComp,defenderMilitary,testMode,placements,onClose,onResolve}){
  var T=window.THREE;
  var mountRef=useRef(null);
  var [combatResult,setCombatResult]=useState(null); // null while fighting, {victor,attackerSurvivors,defenderSurvivors,...} when done
  var camControlsRef=useRef({recenter:function(){},zoomIn:function(){},zoomOut:function(){}});
  var [selectionMode,setSelectionMode]=useState(false);
  var selectionModeRef=useRef(false);
  // Keep ref in sync with state so the inline event handlers can read it
  ...
  useEffect(function(){
    ...
    var cam=new T.PerspectiveCamera(55,W/H,0.1,3000);
    
    // ── Orbital camera state (spherical coordinates around target point) ──
    var camTarget={x:0,y:0,z:60}; // What the camera looks at — slightly toward player side (battlefield extends -220 to +290)
    var camTheta=0; // Azimuth (rotation around Y axis), in radians
    var camPhi=Math.PI*0.32; // Polar angle (height above target). 0=top-down, PI/2=horizontal
    var camRadius=220; // Distance from target — scaled up for larger battlefield
    var camRadiusMin=80,camRadiusMax=600; // Zoom limits (wider range for tactical zoom in/out)
    function updateCamera(){
      var sp=Math.sin(camPhi),cp=Math.cos(camPhi);
      var st=Math.sin(camTheta),ct=Math.cos(camTheta);
      cam.position.set(camTarget.x+camRadius*sp*st,camTarget.y+camRadius*cp,camTarget.z+camRadius*sp*ct);
      cam.lookAt(camTarget.x,camTarget.y,camTarget.z);
    }
    updateCamera();
    
    // Expose camera controls so the HUD buttons can drive them
    camControlsRef.current.recenter=function(){
      camTheta=0;camPhi=Math.PI*0.32;camRadius=220;
      camTarget.x=0;camTarget.y=0;camTarget.z=60;
      updateCamera();
    };
    camControlsRef.current.zoomIn=function(){
      camRadius=Math.max(camRadiusMin,camRadius-50);
      updateCamera();
    };
    camControlsRef.current.zoomOut=function(){
      camRadius=Math.min(camRadiusMax,camRadius+50);
      updateCamera();
    };
    // Allow internal code to flip selection mode off after rectangle drawn
    setSelectionModeFromInside=function(v){setSelectionMode(v);};
    
    // ── Starfield: plenty of stars in deep blue/violet space ──
    ...
    for(var i=0;i<starCount;i++){
      ...
      var r=600+Math.random()*1200;
      starPos[i*3]=r*Math.sin(phi)*Math.cos(theta);
      starPos[i*3+1]=r*Math.sin(phi)*Math.sin(theta);
      starPos[i*3+2]=r*Math.cos(phi);
      // Star color: white/blue/violet/yellow with random brightness
      var brightness=0.3+Math.random()*0.7;
      var hue=Math.random();
      if(hue<0.5){starCol[i*3]=brightness;starCol[i*3+1]=brightness;starCol[i*3+2]=brightness;} // white
      else if(hue<0.75){starCol[i*3]=brightness*0.7;starCol[i*3+1]=brightness*0.85;starCol[i*3+2]=brightness;} // blue-white
      else if(hue<0.9){starCol[i*3]=brightness;starCol[i*3+1]=brightness*0.75;starCol[i*3+2]=brightness*0.95;} // pink-white
      else{starCol[i*3]=brightness;starCol[i*3+1]=brightness*0.95;starCol[i*3+2]=brightness*0.6;} // yellow
    }
    starGeo.setAttribute("position",new T.BufferAttribute(starPos,3));
    starGeo.setAttribute("color",new T.BufferAttribute(starCol,3));
    var starMat=new T.PointsMaterial({size:1.8,vertexColors:true,sizeAttenuation:false,transparent:true,opacity:0.95});
    var stars=new T.Points(starGeo,starMat);
    scene.add(stars);
    
    // (Nebula glow spheres removed — they were creating dark visual artifacts)
    
    // ── Target planet: large sphere in the lower part of the scene ──
    // Color based on planet type
    var planetColor=0x885544;
    if(targetPlanet&&targetPlanet.type){
      var t=targetPlanet.type.toLowerCase();
      if(t.indexOf("gaz")>-1)planetColor=0xcc8866;
      else if(t.indexOf("glac")>-1||t.indexOf("toundra")>-1)planetColor=0x88aacc;
      else if(t.indexOf("desert")>-1)planetColor=0xddaa66;
      else if(t.indexOf("forest")>-1)planetColor=0x66aa66;
      else if(t.indexOf("vol")>-1)planetColor=0xaa3322;
      else if(t.indexOf("oceanique")>-1)planetColor=0x4488cc;
      else planetColor=0x886644;
    }
    var planetGeo=new T.SphereGeometry(80,32,24);
    var planetMat=new T.MeshPhongMaterial({color:planetColor,emissive:planetColor,emissiveIntensity:0.15,shininess:5});
    var planetMesh=new T.Mesh(planetGeo,planetMat);
    planetMesh.position.set(0,-50,-280);
    scene.add(planetMesh);
    
    // Atmosphere glow around planet
    var atmGeo=new T.SphereGeometry(84,32,24);
    var atmMat=new T.MeshBasicMaterial({color:planetColor,transparent:true,opacity:0.18,side:T.BackSide,blending:T.AdditiveBlending});
    var atmMesh=new T.Mesh(atmGeo,atmMat);
    atmMesh.position.copy(planetMesh.position);
    scene.add(atmMesh);
    
    // ── Lighting: directional sun + soft ambient ──
    var sun=new T.DirectionalLight(0xffeecc,1.2);
    sun.position.set(150,200,100);
    scene.add(sun);
    var ambient=new T.AmbientLight(0x4466aa,0.4);
    scene.add(ambient);
    
    // ── Reference grid (faint, helps perception) ──
    var gridGeo=new T.BufferGeometry();
    var gridLines=[];
    var gridSize=300,gridStep=30;
    for(var g=-gridSize;g<=gridSize;g+=gridStep){
      gridLines.push(g,0,-gridSize);gridLines.push(g,0,gridSize);
      gridLines.push(-gridSize,0,g);gridLines.push(gridSize,0,g);
    }
    gridGeo.setAttribute("position",new T.BufferAttribute(new Float32Array(gridLines),3));
    var gridMat=new T.LineBasicMaterial({color:0x224466,transparent:true,opacity:0.25});
    var grid=new T.LineSegments(gridGeo,gridMat);
    scene.add(grid);
    
    // ── Ship factory: creates a 3D mesh group for each ship type ──
    // Each ship is a Group of meshes so we can rotate/move it as a single unit
    // Helper: creates a mesh with neon edge wireframe overlay (Tron style)
    function neonMesh(geometry,fillColor,edgeColor){
      var grp=new T.Group();
      // Filled body with strong emissive
      var fillMat=new T.MeshPhongMaterial({color:fillColor,emissive:fillColor,emissiveIntensity:0.5,shininess:80,transparent:true,opacity:0.85});
      var body=new T.Mesh(geometry,fillMat);
      grp.add(body);
      // Edge wireframe (sharp neon lines on the edges only)
      var edgeGeo=new T.EdgesGeometry(geometry,30);
      var edgeMat=new T.LineBasicMaterial({color:edgeColor||0xffffff,transparent:true,opacity:1.0,linewidth:2});
      var edges=new T.LineSegments(edgeGeo,edgeMat);
      grp.add(edges);
      return grp;
    }
    // Helper: creates a glowing engine sprite-like sphere
    function neonGlow(size,color,opacity){
      var g=new T.Group();
      var inner=new T.Mesh(new T.SphereGeometry(size*0.4,8,6),new T.MeshBasicMaterial({color:0xffffff,transparent:true,opacity:opacity||0.95}));
      g.add(inner);
      var outer=new T.Mesh(new T.SphereGeometry(size,12,8),new T.MeshBasicMaterial({color:color,transparent:true,opacity:0.45,blending:T.AdditiveBlending}));
      g.add(outer);
      var halo=new T.Mesh(new T.SphereGeometry(size*1.8,12,8),new T.MeshBasicMaterial({color:color,transparent:true,opacity:0.15,blending:T.AdditiveBlending}));
      g.add(halo);
      return g;
    }
    
    function makeShip(type,color){
      var g=new T.Group();
      // Edge color: lighter version of fill color for that "neon glowing" look
      var edgeC=color===0x4488dd?0x88ddff:0xffaadd; // bright cyan or bright pink
      var engineC=color===0x4488dd?0x66eeff:0xff66aa; // engine glow color
      
      if(type==="chasseur"){
        // Sleek delta-wing fighter — flattened triangular shape
        var bodyGeo=new T.ConeGeometry(1.0,3.5,3);
        var body=neonMesh(bodyGeo,color,edgeC);
        body.rotation.x=Math.PI/2;
        body.scale.set(1.4,0.4,1); // flatten it
        g.add(body);
        // Engine glow
        var eng=neonGlow(0.6,engineC,0.95);
        eng.position.set(0,0,1.8);
        g.add(eng);
        // Subtle ambient halo around the ship
        var halo=new T.Mesh(new T.SphereGeometry(2.5,8,6),new T.MeshBasicMaterial({color:engineC,transparent:true,opacity:0.06,blending:T.AdditiveBlending}));
        g.add(halo);
      } else if(type==="cargo"){
        // Hexagonal stylized cargo container
        var bodyGeo=new T.CylinderGeometry(1.5,1.5,3.5,6);
        var body=neonMesh(bodyGeo,color,edgeC);
        body.rotation.x=Math.PI/2;
        g.add(body);
        // Two side accent cubes (look like cargo modules)
        var modGeo=new T.BoxGeometry(0.4,0.4,2.5);
        for(var ci=-1;ci<=1;ci+=2){
          var mod=neonMesh(modGeo,color,edgeC);
          mod.position.set(ci*1.7,0,0);
          g.add(mod);
        }
        // Single rear engine
        var ceng=neonGlow(0.5,engineC,0.85);
        ceng.position.set(0,0,2);
        g.add(ceng);
      } else if(type==="croiseur"){
        // Arrow-shaped cruiser with swept wings
        var hullGeo=new T.ConeGeometry(0.9,4.5,4);
        var hull=neonMesh(hullGeo,color,edgeC);
        hull.rotation.x=Math.PI/2;
        hull.scale.set(1.2,0.6,1.2);
        g.add(hull);
        // Swept wings: two triangles on the sides
        var wingGeo=new T.BoxGeometry(3.5,0.2,1.5);
        var wing=neonMesh(wingGeo,color,edgeC);
        wing.position.set(0,0,0.3);
        g.add(wing);
        // Twin engines on wing tips
        for(var wi=-1;wi<=1;wi+=2){
          var weng=neonGlow(0.5,engineC,0.9);
          weng.position.set(wi*1.7,0,1);
          g.add(weng);
        }
      } else if(type==="bombardier"){
        // UFO-style disc with rotating ring
        var discGeo=new T.CylinderGeometry(2.4,1.6,0.8,16);
        var disc=neonMesh(discGeo,color,edgeC);
        g.add(disc);
        // Top dome (cockpit)
        var domeGeo=new T.SphereGeometry(0.8,12,8,0,Math.PI*2,0,Math.PI/2);
        var dome=neonMesh(domeGeo,color,edgeC);
        dome.position.set(0,0.5,0);
        g.add(dome);
        // Underside heavy weapon
        var wpnGeo=new T.CylinderGeometry(0.5,0.6,1.4,6);
        var wpn=neonMesh(wpnGeo,color,edgeC);
        wpn.position.set(0,-1,0);
        g.add(wpn);
        // Animated rotating ring (just adding it; rotation in animate loop later)
        var ringGeo=new T.TorusGeometry(2.6,0.1,4,24);
        var ringMat=new T.MeshBasicMaterial({color:engineC,transparent:true,opacity:0.7});
        var ring=new T.Mesh(ringGeo,ringMat);
        ring.rotation.x=Math.PI/2;
        ring.userData.spin=true; // marker for animation
        g.add(ring);
      } else if(type==="cuirasse"){
        // Massive lance-shaped battleship
        var hullGeo=new T.BoxGeometry(3.2,1.8,7);
        var hull=neonMesh(hullGeo,color,edgeC);
        g.add(hull);
        // Stylized prow (front pointed section)
        var prowGeo=new T.ConeGeometry(1.2,2,4);
        var prow=neonMesh(prowGeo,color,edgeC);
        prow.rotation.x=-Math.PI/2;
        prow.position.set(0,0,-3.5);
        g.add(prow);
        // Tower / bridge on top
        var towerGeo=new T.BoxGeometry(2,1.2,2.4);
        var tower=neonMesh(towerGeo,color,edgeC);
        tower.position.set(0,1.4,-0.5);
        g.add(tower);
        // Side cannons (2 guns)
        var cannonGeo=new T.CylinderGeometry(0.18,0.18,2.5,6);
        for(var bi=-1;bi<=1;bi+=2){
          var cannon=neonMesh(cannonGeo,0xffaa44,0xffdd88);
          cannon.rotation.x=Math.PI/2;
          cannon.position.set(bi*1.4,0.6,-2);
          g.add(cannon);
        }
        // Three rear thrusters (massive engines)
        for(var ei=-1;ei<=1;ei++){
          var en=neonGlow(0.7,0xffaa44,0.95);
          en.position.set(ei*1.0,0,3.8);
          g.add(en);
        }
      } else {
        var fbGeo=new T.BoxGeometry(1,1,2);
        var fb=neonMesh(fbGeo,color,edgeC);
        g.add(fb);
      }
      return g;
    }
    
    // ── Defense factory: creates 3D mesh for each defense type (placed on planet surface) ──
    function makeDefense(type){
      var g=new T.Group();
      var defColor=0xcc4466;
      var defEdge=0xff8899;
      if(type==="def_tourelle"){
        // Hexagonal turret base + angled barrel
        var baseGeo=new T.CylinderGeometry(1.4,1.6,0.8,6);
        var base=neonMesh(baseGeo,defColor,defEdge);
        g.add(base);
        var barrelGeo=new T.CylinderGeometry(0.25,0.4,2.4,6);
        var barrel=neonMesh(barrelGeo,defColor,defEdge);
        barrel.rotation.x=Math.PI/3.5;
        barrel.position.set(0,0.8,0.6);
        g.add(barrel);
        // Glowing tip
        var tip=neonGlow(0.4,0xff6699,0.85);
        tip.position.set(0,1.6,1.5);
        g.add(tip);
      } else if(type==="def_bouclier"){
        // Faceted shield dome with glowing edges
        var domeGeo=new T.SphereGeometry(3,8,6,0,Math.PI*2,0,Math.PI/2);
        var domeFill=new T.Mesh(domeGeo,new T.MeshBasicMaterial({color:0x4488ff,transparent:true,opacity:0.18,side:T.DoubleSide,blending:T.AdditiveBlending}));
        g.add(domeFill);
        var domeEdgeGeo=new T.EdgesGeometry(domeGeo,30);
        var domeEdges=new T.LineSegments(domeEdgeGeo,new T.LineBasicMaterial({color:0x88ccff,transparent:true,opacity:0.7}));
        g.add(domeEdges);
      } else if(type==="def_missiles"){
        // Stylized rocket launcher with multiple tubes
        var rackGeo=new T.BoxGeometry(1.2,0.8,1.4);
        var rack=neonMesh(rackGeo,defColor,defEdge);
        rack.position.set(0,0.4,0);
        g.add(rack);
        // Three missile cones on top
        for(var mi=-1;mi<=1;mi++){
          var rocketGeo=new T.ConeGeometry(0.2,1.5,4);
          var rocket=neonMesh(rocketGeo,0xff8855,0xffbb88);
          rocket.position.set(mi*0.4,1.4,0);
          g.add(rocket);
        }
      } else if(type==="def_forteresse"){
        // Massive hexagonal fortress with central tower
        var hexGeo=new T.CylinderGeometry(2.8,3.2,2.6,6);
        var hex=neonMesh(hexGeo,defColor,defEdge);
        g.add(hex);
        var topGeo=new T.BoxGeometry(2.2,1.4,2.2);
        var top=neonMesh(topGeo,defColor,defEdge);
        top.position.set(0,1.8,0);
        g.add(top);
        // Glowing central beacon
        var beacon=neonGlow(0.6,0xffaa55,0.9);
    ...
    // ── Combat stats helpers (HP, DPS, range per ship type) ──
    function getShipCombatStats(type){
      if(type==="chasseur")return {hp:80,dps:35,range:35};
      if(type==="cargo")return {hp:120,dps:5,range:20};
      if(type==="croiseur")return {hp:400,dps:80,range:50};
      if(type==="bombardier")return {hp:300,dps:120,range:60};
      if(type==="cuirasse")return {hp:1500,dps:200,range:75};
      return {hp:80,dps:20,range:35};
    }
    function getDefenseCombatStats(type){
      if(type==="def_tourelle")return {hp:150,dps:30,range:45};
      if(type==="def_bouclier")return {hp:1000,dps:0,range:0};
      if(type==="def_missiles")return {hp:50,dps:200,range:85};
      if(type==="def_forteresse")return {hp:3000,dps:150,range:65};
      return {hp:100,dps:0,range:0};
    }
    
    // ── Spawn attacker ships (player) — hybrid: ≤MAX visible meshes per squad, each mesh represents repCount ships ──
    var attackerColor=0x4488dd; // blue
    var attackerShips=[]; // array of {mesh, type, ..., repCount?}
    var attackerTypes=["chasseur","cargo","croiseur","bombardier","cuirasse"];
    var attackerSquads=[]; // for placement-based: badge sprite + meshIndices for live count update
    var hasPlacements=placements&&placements.length>0;
    
    // Color per ship/defense type for the count badge border
    var SHIP_BADGE_COLORS={chasseur:"#88ccff",cargo:"#cccc88",croiseur:"#ff8855",bombardier:"#ff5588",cuirasse:"#aa66ff",def_tourelle:"#ff8866",def_bouclier:"#88ccff",def_missiles:"#ffaa44",def_forteresse:"#cc88ff"};
    
    function drawCountLabel(ctx,canvas,text,color){
    ...
    function makeCountSprite(text,color){
      var canvas=document.createElement("canvas");
      canvas.width=256;canvas.height=128;
      var ctx=canvas.getContext("2d");
      drawCountLabel(ctx,canvas,text,color);
      ...
      return {sprite:sprite,canvas:canvas,ctx:ctx,texture:tex,material:mat};
    }
    
    // HP bar: small horizontal bar above each ship, colored green/yellow/red by HP %
    function drawHpBar(ctx,canvas,pct){
      ctx.clearRect(0,0,canvas.width,canvas.height);
      // Background (dark border)
      ctx.fillStyle="rgba(10,5,5,0.85)";
      ctx.fillRect(0,0,canvas.width,canvas.height);
      // Inner dark fill
      ctx.fillStyle="rgba(35,15,15,0.95)";
      ctx.fillRect(2,2,canvas.width-4,canvas.height-4);
      // HP fill, color depends on %
      var fillColor;
      if(pct>0.6)fillColor="#44dd66";
      else if(pct>0.3)fillColor="#ddcc44";
      else fillColor="#dd4444";
    ...
    function makeHpBarSprite(){
      ...
      return {sprite:sprite,canvas:canvas,ctx:ctx,texture:tex,material:mat,lastPct:1.0};
    }
    
    // Organic cluster: returns {x,z} offset for mesh idx in a group of total
    function clusterPos(idx,total){
      var jx=(Math.random()-0.5)*2.5;
      var jz=(Math.random()-0.5)*2.5;
      if(total<=1)return {x:jx,z:jz};
      if(total<=4){
        var ang1=(idx/total)*Math.PI*2;
        return {x:Math.cos(ang1)*5+jx,z:Math.sin(ang1)*5+jz};
      }
      if(idx===0)return {x:jx,z:jz};
      var ang2=((idx-1)/(total-1))*Math.PI*2;
      var rad=9;
      return {x:Math.cos(ang2)*rad+jx,z:Math.sin(ang2)*rad+jz};
    }
    
    if(hasPlacements){
      // Map 2D map % (from BattlePlanScreen) to 3D battlefield coords (scaled-up: bigger feel, longer travel)
      // 2D x ∈ [0,100]% → 3D X ∈ [-250, +250] (lateral spread, was ±125)
      // 2D y ∈ [52,97]% → 3D Z ∈ [+37, +262] (depth, was +15..+105)
      function pl2dTo3dX(px){return Math.max(-300,Math.min(300,(px-50)*5));}
      function pl2dTo3dZ(py){return Math.max(20,Math.min(290,((py-52)/45)*225+37));}
      
      var MAX_VISIBLE_MESHES=12; // visual cap per squad — large fleets are represented by repCount on each mesh
      
      for(var pli=0;pli<placements.length;pli++){
        var pl=placements[pli];
        var pType=pl.shipType;
        var pCount=pl.count;
        if(pCount<=0)continue;
        var centerX=pl2dTo3dX(pl.x);
        var centerZ=pl2dTo3dZ(pl.y);
        
        // How many visible meshes vs how many ships each represents
        var numMeshes=Math.min(MAX_VISIBLE_MESHES,pCount);
        var baseRep=Math.floor(pCount/numMeshes);
        var remainder=pCount-baseRep*numMeshes; // first `remainder` meshes get +1 ship
        
        var pStats=getShipCombatStats(pType);
        var meshIndicesForSquad=[];
        
        for(var mi=0;mi<numMeshes;mi++){
          var rep=baseRep+(mi<remainder?1:0);
          var pship=makeShip(pType,attackerColor);
          pship.scale.set(3.5,3.5,3.5);
          var clp=clusterPos(mi,numMeshes);
          pship.position.set(centerX+clp.x,0,centerZ+clp.z);
          pship.rotation.y=Math.PI;
          scene.add(pship);
          meshIndicesForSquad.push(attackerShips.length);
          // HP and DPS scaled by repCount: a mesh of 6 chasseurs = 6× HP and 6× DPS
          var pHpBar=makeHpBarSprite();
          pHpBar.sprite.position.set(pship.position.x,pship.position.y+7,pship.position.z);
          scene.add(pHpBar.sprite);
          attackerShips.push({mesh:pship,type:pType,moving:false,destX:0,destZ:0,hp:pStats.hp*rep,maxHp:pStats.hp*rep,dps:pStats.dps*rep,range:pStats.range,fireCooldown:0,target:null,alive:true,side:"attacker",repCount:rep,hpBar:pHpBar});
        }
        
        // Count label sprite floating above the squad
        var bColor=SHIP_BADGE_COLORS[pType]||"#88ccff";
        var labelData=makeCountSprite("\u00D7"+pCount,bColor);
        labelData.sprite.position.set(centerX,14,centerZ);
        scene.add(labelData.sprite);
        
        attackerSquads.push({
          type:pType,centerX:centerX,centerZ:centerZ,color:bColor,
          badgeSprite:labelData.sprite,badgeCanvas:labelData.canvas,
          badgeCtx:labelData.ctx,badgeTexture:labelData.texture,badgeMaterial:labelData.material,
          lastShownCount:pCount,meshIndices:meshIndicesForSquad
        });
      }
    }else{
      // Fallback: default linear grid layout (no placement data → no badges, 1 mesh = 1 ship)
      var spawnPos=[];
      var playerZ=40;
      var rowZ=playerZ;
      var idx=0;
      for(var atk=0;atk<attackerTypes.length;atk++){
        var t=attackerTypes[atk];
        var qty=attackerComp[t]||0;
        for(var qi=0;qi<qty;qi++){
          var ship=makeShip(t,attackerColor);
          ship.scale.set(3.5,3.5,3.5);
          var row=Math.floor(idx/8);
          var col=idx%8;
          ship.position.set((col-3.5)*12,0,rowZ+row*12);
          ship.rotation.y=Math.PI;
          scene.add(ship);
          var stats=getShipCombatStats(t);
          var fHpBar=makeHpBarSprite();
          fHpBar.sprite.position.set(ship.position.x,ship.position.y+7,ship.position.z);
          scene.add(fHpBar.sprite);
          attackerShips.push({mesh:ship,type:t,moving:false,destX:0,destZ:0,hp:stats.hp,maxHp:stats.hp,dps:stats.dps,range:stats.range,fireCooldown:0,target:null,alive:true,side:"attacker",hpBar:fHpBar});
          idx++;
        }
      }
    }
    
    // ── Spawn defender ships (enemy bot) — hybrid: 1 squad per ship type, spread on X axis, ≤12 meshes each ──
    var defenderColor=0xdd4466; // red
    var defenderShips=[];
    var defenderSquads=[]; // squad metadata + badge sprites for live count update
    var defShipsData=(defenderMilitary&&defenderMilitary.ships)||{};
    
    // Collect types that actually have ships (skip empty types so we don't waste slots)
    var defTypesPresent=[];
    for(var dfk0=0;dfk0<attackerTypes.length;dfk0++){
      var dft0=attackerTypes[dfk0];
      if((defShipsData[dft0]||0)>0)defTypesPresent.push(dft0);
    }
    var dnPresent=defTypesPresent.length;
    
    for(var dfk=0;dfk<dnPresent;dfk++){
      var dft=defTypesPresent[dfk];
      var dqty=defShipsData[dft]||0;
      // Evenly spread types across X = -200 to +200 at Z = -90 (scaled-up battlefield)
      var dCenterX=dnPresent===1?0:(-200+(dfk/(dnPresent-1))*400);
      var dCenterZ=-90;
      
      // Hybrid: ≤12 visible meshes per type, each represents repCount ships
      var dNumMeshes=Math.min(12,dqty);
      var dBaseRep=Math.floor(dqty/dNumMeshes);
      var dRem=dqty-dBaseRep*dNumMeshes;
      var dStats=getShipCombatStats(dft);
      var dMeshIndices=[];
      
      for(var dmi=0;dmi<dNumMeshes;dmi++){
        var dRep=dBaseRep+(dmi<dRem?1:0);
        var dship=makeShip(dft,defenderColor);
        dship.scale.set(3.5,3.5,3.5);
        var dclp=clusterPos(dmi,dNumMeshes);
        dship.position.set(dCenterX+dclp.x,0,dCenterZ+dclp.z);
        // Already facing forward (toward player) by default, no rotation needed
        scene.add(dship);
        dMeshIndices.push(defenderShips.length);
        var dHpBar=makeHpBarSprite();
        dHpBar.sprite.position.set(dship.position.x,dship.position.y+7,dship.position.z);
        scene.add(dHpBar.sprite);
        defenderShips.push({mesh:dship,type:dft,moving:false,destX:0,destZ:0,hp:dStats.hp*dRep,maxHp:dStats.hp*dRep,dps:dStats.dps*dRep,range:dStats.range,fireCooldown:0,target:null,alive:true,side:"defender",repCount:dRep,hpBar:dHpBar});
      }
      
      // Count label sprite
      var dbColor=SHIP_BADGE_COLORS[dft]||"#ff8888";
      var dLabelData=makeCountSprite("\u00D7"+dqty,dbColor);
      dLabelData.sprite.position.set(dCenterX,14,dCenterZ);
      scene.add(dLabelData.sprite);
      
      defenderSquads.push({
        type:dft,centerX:dCenterX,centerZ:dCenterZ,color:dbColor,
        badgeSprite:dLabelData.sprite,badgeCanvas:dLabelData.canvas,
        badgeCtx:dLabelData.ctx,badgeTexture:dLabelData.texture,badgeMaterial:dLabelData.material,
        lastShownCount:dqty,meshIndices:dMeshIndices
      });
    }
    
    // ── Spawn defenses (stationary structures) — hybrid: 1 squad per defense type, ≤12 meshes, deeper near planet ──
    var defenses=[];
    var defenseSquads=[];
    var defData=(defenderMilitary&&defenderMilitary.defenses)||{};
    var defTypes=["def_tourelle","def_bouclier","def_missiles","def_forteresse"];
    
    var defStrTypesPresent=[];
    for(var dt0=0;dt0<defTypes.length;dt0++){
      if((defData[defTypes[dt0]]||0)>0)defStrTypesPresent.push(defTypes[dt0]);
    }
    var dnDefPresent=defStrTypesPresent.length;
    
    for(var dt=0;dt<dnDefPresent;dt++){
      var defType=defStrTypesPresent[dt];
      var dQty=defData[defType]||0;
      var dsCenterX=dnDefPresent===1?0:(-180+(dt/(dnDefPresent-1))*360);
      var dsCenterZ=-220;
      
      var dsNumMeshes=Math.min(12,dQty);
      var dsBaseRep=Math.floor(dQty/dsNumMeshes);
      var dsRem=dQty-dsBaseRep*dsNumMeshes;
      var dfsStats=getDefenseCombatStats(defType);
      var dsMeshIndices=[];
      
      for(var dvq=0;dvq<dsNumMeshes;dvq++){
        var dsRep=dsBaseRep+(dvq<dsRem?1:0);
        var defMesh=makeDefense(defType);
        defMesh.scale.set(3.5,3.5,3.5);
        var dsclp=clusterPos(dvq,dsNumMeshes);
        defMesh.position.set(dsCenterX+dsclp.x,-2,dsCenterZ+dsclp.z);
        scene.add(defMesh);
        dsMeshIndices.push(defenses.length);
        var dsHpBar=makeHpBarSprite();
        dsHpBar.sprite.position.set(defMesh.position.x,defMesh.position.y+8,defMesh.position.z);
        scene.add(dsHpBar.sprite);
        defenses.push({mesh:defMesh,type:defType,hp:dfsStats.hp*dsRep,maxHp:dfsStats.hp*dsRep,dps:dfsStats.dps*dsRep,range:dfsStats.range,fireCooldown:0,target:null,alive:true,side:"defender",repCount:dsRep,hpBar:dsHpBar});
      }
      
      var dsbColor=SHIP_BADGE_COLORS[defType]||"#ff8866";
      var dsLabelData=makeCountSprite("\u00D7"+dQty,dsbColor);
      dsLabelData.sprite.position.set(dsCenterX,12,dsCenterZ);
      scene.add(dsLabelData.sprite);
      
      defenseSquads.push({
        type:defType,centerX:dsCenterX,centerZ:dsCenterZ,color:dsbColor,
        badgeSprite:dsLabelData.sprite,badgeCanvas:dsLabelData.canvas,
        badgeCtx:dsLabelData.ctx,badgeTexture:dsLabelData.texture,badgeMaterial:dsLabelData.material,
        lastShownCount:dQty,meshIndices:dsMeshIndices
      });
    }
    
    // ── Selection system: track which player ships are selected, show halos ──
    var selectedSet=new Set(); // Set of attacker ship indices that are selected
    var selectionHalos=[]; // halo meshes parallel to attackerShips (one per ship)
    // Pre-create a halo for each attacker ship (hidden by default)
    for(var hi=0;hi<attackerShips.length;hi++){
      var haloGeo=new T.RingGeometry(3.5,4.2,24);
      var haloMat=new T.MeshBasicMaterial({color:0x88ddff,transparent:true,opacity:0,side:T.DoubleSide,blending:T.AdditiveBlending});
      var halo=new T.Mesh(haloGeo,haloMat);
      halo.rotation.x=-Math.PI/2; // flat on the ground plane
      halo.position.copy(attackerShips[hi].mesh.position);
      halo.position.y=-0.5; // slightly below ship
      scene.add(halo);
      selectionHalos.push(halo);
    }
    
    // ── Tap-to-select handling (raycasting) ──
    ...
    function onTap(clientX,clientY){
      ...
      // Build a list of all attacker ship meshes (including children) for raycasting
      var meshesToTest=[];
      for(var ti=0;ti<attackerShips.length;ti++){
        // Skip dead ships (their meshes are invisible but raycasting still hits them)
        if(!attackerShips[ti].alive)continue;
        // Recurse into children
        attackerShips[ti].mesh.traverse(function(c){if(c.isMesh)meshesToTest.push({mesh:c,shipIdx:ti});});
      }
      var meshArr=meshesToTest.map(function(x){return x.mesh;});
      var hits=raycaster.intersectObjects(meshArr,false);
      if(hits.length>0){
        // Tapped on a player ship — toggle selection
        var hit=hits[0].object;
        var found=meshesToTest.find(function(x){return x.mesh===hit;});
        if(found){
          if(selectedSet.has(found.shipIdx)){selectedSet.delete(found.shipIdx);}
          else{selectedSet.add(found.shipIdx);}
          updateHaloOpacity();
        }
      } else {
        // Tapped empty space — if ships selected, MOVE them to that position
        if(selectedSet.size>0){
          // Compute target position on the ground plane (y=0)
          // Use raycaster to find intersection with y=0 plane
          var rayDir=raycaster.ray.direction;
          var rayOrig=raycaster.ray.origin;
          // Solve: rayOrig.y + t * rayDir.y = 0  →  t = -rayOrig.y / rayDir.y
          if(Math.abs(rayDir.y)>0.001){
            var tParam=-rayOrig.y/rayDir.y;
            if(tParam>0){
              var targetX=rayOrig.x+tParam*rayDir.x;
              var targetZ=rayOrig.z+tParam*rayDir.z;
              // Clamp to battlefield bounds (scaled-up)
              targetX=Math.max(-300,Math.min(300,targetX));
              targetZ=Math.max(-220,Math.min(320,targetZ));
              // Set destination for each selected ship — spread them around the target point
              var selArr=Array.from(selectedSet);
              for(var sidx=0;sidx<selArr.length;sidx++){
                var shipObj=attackerShips[selArr[sidx]];
                // Spread in a small grid around target (avoid all going to exact same spot)
                var spreadCol=sidx%3;
                var spreadRow=Math.floor(sidx/3);
                shipObj.destX=targetX+(spreadCol-1)*8;
                shipObj.destZ=targetZ+spreadRow*8;
                shipObj.moving=true;
              }
              // Visual feedback: a click marker that fades out
              spawnClickMarker(targetX,0,targetZ);
            }
          }
        } else {
    ...
    // ── Click marker: visual feedback at tap position ──
    var clickMarkers=[];
    function spawnClickMarker(x,y,z){
      var ringGeo=new T.RingGeometry(2,3.5,24);
      var ringMat=new T.MeshBasicMaterial({color:0x88ddff,transparent:true,opacity:0.9,side:T.DoubleSide,blending:T.AdditiveBlending});
      var ring=new T.Mesh(ringGeo,ringMat);
      ring.rotation.x=-Math.PI/2;
      ring.position.set(x,y+0.1,z);
      ring.userData.bornAt=clock.getElapsedTime();
      scene.add(ring);
      clickMarkers.push(ring);
    }
    function updateHaloOpacity(){
      for(var i=0;i<selectionHalos.length;i++){
        selectionHalos[i].material.opacity=selectedSet.has(i)?0.85:0;
      }
    }
    
    // ── Selection rectangle state (drag-to-select when in selection mode) ──
    var selectionBox={active:false,startX:0,startY:0,endX:0,endY:0};
    // DOM element for the visual rectangle (created lazily)
    var selectionBoxEl=null;
    function updateSelectionBoxUI(){
      ...
      if(!selectionBoxEl){
        ...
        selectionBoxEl.style.cssText="position:fixed;border:2px solid #88ddff;background:rgba(136,221,255,0.15);pointer-events:none;z-index:1000;border-radius:2px;box-shadow:0 0 8px rgba(136,221,255,0.4);";
      ...
      var w=Math.abs(selectionBox.endX-selectionBox.startX);
      var h=Math.abs(selectionBox.endY-selectionBox.startY);
    ...
    function finalizeSelectionBox(){
      // Project each player ship's 3D position to screen space
      // and add to selectedSet if it falls within the rectangle
      var rect=renderer.domElement.getBoundingClientRect();
      var minX=Math.min(selectionBox.startX,selectionBox.endX);
      var minY=Math.min(selectionBox.startY,selectionBox.endY);
      var maxX=Math.max(selectionBox.startX,selectionBox.endX);
      var maxY=Math.max(selectionBox.startY,selectionBox.endY);
      // If rectangle too small (almost a tap), do nothing
      if((maxX-minX)<8&&(maxY-minY)<8)return;
      // Clear current selection (rectangle = fresh selection)
      selectedSet.clear();
      var v=new T.Vector3();
      for(var fi=0;fi<attackerShips.length;fi++){
        var fs=attackerShips[fi];
        if(!fs.alive)continue;
        v.copy(fs.mesh.position);
        v.project(cam);
        // v.x and v.y are in NDC (-1 to 1). Convert to screen pixels.
        var sx=(v.x*0.5+0.5)*rect.width+rect.left;
        var sy=(-v.y*0.5+0.5)*rect.height+rect.top;
        // Skip ships behind the camera (z > 1 in NDC)
        if(v.z>1)continue;
    ...
    // Bridge to set state from inside useEffect — needs the React setter
    var setSelectionModeFromInside=null; // wired below
    
    // ── Combined input: tap (sélection), drag (orbit), pinch (zoom) ──
    // We track the press start to differentiate a quick tap from a drag.
    var pressStart=null; // {x,y,time}
    var lastMove=null; // {x,y} for orbit delta
    var pinchStart=null; // {dist,radius} for zoom
    var TAP_THRESHOLD=8; // pixels — beyond this, it becomes a drag
    var TAP_TIME_MAX=350; // ms — beyond this, considered drag
    
    function onPressStart(x,y){
      pressStart={x:x,y:y,time:Date.now(),moved:false};
      lastMove={x:x,y:y};
      // If in selection mode: also start tracking the box rectangle
      if(selectionModeRef.current){
        selectionBox.active=true;
        selectionBox.startX=x;selectionBox.startY=y;
        selectionBox.endX=x;selectionBox.endY=y;
        updateSelectionBoxUI();
      }
    }
    function onPressMove(x,y){
      if(!pressStart||!lastMove)return;
      var dx=x-lastMove.x;
      var dy=y-lastMove.y;
      var totalDx=x-pressStart.x;
      var totalDy=y-pressStart.y;
      var totalDist=Math.sqrt(totalDx*totalDx+totalDy*totalDy);
      if(totalDist>TAP_THRESHOLD)pressStart.moved=true;
      // Branch on mode:
      if(selectionModeRef.current&&selectionBox.active){
        // Update rectangle, no camera move
        selectionBox.endX=x;selectionBox.endY=y;
        updateSelectionBoxUI();
      } else {
        // Normal mode: drag rotates the camera
        if(pressStart.moved){
          camTheta-=dx*0.008;
    ...
    function onPressEnd(x,y){
      ...
      if(selectionModeRef.current&&selectionBox.active){
        ...
        updateSelectionBoxUI();
        // Auto-exit selection mode after one rectangle
        if(setSelectionModeFromInside)setSelectionModeFromInside(false);
      } else if(!pressStart.moved&&elapsed<TAP_TIME_MAX){
        onTap(pressStart.x,pressStart.y);
      }
      pressStart=null;
      lastMove=null;
    }
    ...
    function onMouseMove(e){if(pressStart)onPressMove(e.clientX,e.clientY);}
    ...
    function onTouchStart(e){
      ...
      if(e.touches.length===2){
        ...
        pinchStart={dist:Math.sqrt(dx*dx+dy*dy),radius:camRadius};
        pressStart=null; // cancel tap detection
      } else if(e.touches.length===1){
        onPressStart(e.touches[0].clientX,e.touches[0].clientY);
      }
    }
    function onTouchMove(e){
      ...
      if(e.touches.length===2&&pinchStart){
        ...
        var ratio=pinchStart.dist/Math.max(1,d);
        camRadius=Math.max(camRadiusMin,Math.min(camRadiusMax,pinchStart.radius*ratio));
        updateCamera();
      } else if(e.touches.length===1&&pressStart){
        onPressMove(e.touches[0].clientX,e.touches[0].clientY);
      }
    }
    function onTouchEnd(e){
      if(pinchStart&&e.touches.length<2)pinchStart=null;
      if(e.touches.length===0&&pressStart)onPressEnd(0,0);
    }
    function onWheel(e){
      e.preventDefault();
      var delta=e.deltaY*0.5;
      camRadius=Math.max(camRadiusMin,Math.min(camRadiusMax,camRadius+delta));
      updateCamera();
    }
    ...
    // ── Lasers: visual feedback for shots ──
    var lasers=[];
    function spawnLaser(fromPos,toPos,color){
      var dx=toPos.x-fromPos.x,dy=toPos.y-fromPos.y,dz=toPos.z-fromPos.z;
      var len=Math.sqrt(dx*dx+dy*dy+dz*dz);
      if(len<0.1)return;
      // Group: bright core + thicker glowing halo around it
      var laserGroup=new T.Group();
      // Bright white core (thin but very bright)
      var coreGeo=new T.CylinderGeometry(0.35,0.35,len,8);
      var coreMat=new T.MeshBasicMaterial({color:0xffffff,transparent:true,opacity:1.0,blending:T.AdditiveBlending});
      var core=new T.Mesh(coreGeo,coreMat);
      laserGroup.add(core);
      // Outer colored halo (much thicker for the glow effect)
      var haloGeo=new T.CylinderGeometry(1.2,1.2,len,8);
      var haloMat=new T.MeshBasicMaterial({color:color,transparent:true,opacity:0.55,blending:T.AdditiveBlending});
      var halo=new T.Mesh(haloGeo,haloMat);
      laserGroup.add(halo);
      // Position group at midpoint and orient
      laserGroup.position.set((fromPos.x+toPos.x)/2,(fromPos.y+toPos.y)/2,(fromPos.z+toPos.z)/2);
      laserGroup.lookAt(toPos);
      laserGroup.rotateX(Math.PI/2);
      laserGroup.userData.bornAt=clock.getElapsedTime();
      scene.add(laserGroup);
      lasers.push({mesh:laserGroup,bornAt:clock.getElapsedTime(),core:core,halo:halo});
    }
    
    // ── Explosions ──
    var explosions=[];
    function spawnExplosion(x,y,z){
      var expGeo=new T.SphereGeometry(2,12,8);
      var expMat=new T.MeshBasicMaterial({color:0xffaa44,transparent:true,opacity:0.9,blending:T.AdditiveBlending});
      var exp=new T.Mesh(expGeo,expMat);
      exp.position.set(x,y,z);
      exp.userData.bornAt=clock.getElapsedTime();
      scene.add(exp);
      explosions.push(exp);
    }
    function killUnit(unit){
      ...
      unit.alive=false;
      spawnExplosion(unit.mesh.position.x,unit.mesh.position.y,unit.mesh.position.z);
      unit.mesh.visible=false;
    }
    
    // ── Animation loop ──
    var alive=true;
    var combatEnded=false; // becomes true when victory/defeat is detected
    ...
    function animate(){
      ...
      // ── Update squad count badges (DRY helper for attacker/defender/defenses) ──
      function updateSquadBadges(squads,shipsArr){
        for(var sq=0;sq<squads.length;sq++){
          var squad=squads[sq];
          var living=0;
          var meanX=0,meanZ=0,livingMeshCount=0;
          for(var mii=0;mii<squad.meshIndices.length;mii++){
            var sm=shipsArr[squad.meshIndices[mii]];
            if(sm.alive){
              living+=sm.repCount||1;
              meanX+=sm.mesh.position.x;
              meanZ+=sm.mesh.position.z;
              livingMeshCount++;
            }
          }
          if(livingMeshCount>0){
            meanX/=livingMeshCount;meanZ/=livingMeshCount;
            squad.badgeSprite.position.set(meanX,squad.badgeSprite.position.y,meanZ);
          }
          if(living!==squad.lastShownCount){
            squad.lastShownCount=living;
            if(living>0){
              drawCountLabel(squad.badgeCtx,squad.badgeCanvas,"\u00D7"+living,squad.color);
              squad.badgeTexture.needsUpdate=true;
            }else{
              squad.badgeSprite.visible=false;
            }
          }
        }
      }
      updateSquadBadges(attackerSquads,attackerShips);
      updateSquadBadges(defenderSquads,defenderShips);
      updateSquadBadges(defenseSquads,defenses);
      
      // ── Update HP bars: position above ship + redraw texture only when HP % crosses 4% threshold ──
      function updateHpBars(shipsArr){
        for(var hbi=0;hbi<shipsArr.length;hbi++){
          var sh=shipsArr[hbi];
          if(!sh.hpBar)continue;
          if(!sh.alive){sh.hpBar.sprite.visible=false;continue;}
          sh.hpBar.sprite.position.set(sh.mesh.position.x,sh.mesh.position.y+7,sh.mesh.position.z);
          var pct=Math.max(0,sh.hp/sh.maxHp);
          if(Math.abs(pct-sh.hpBar.lastPct)>0.04||(pct===0&&sh.hpBar.lastPct>0)){
            drawHpBar(sh.hpBar.ctx,sh.hpBar.canvas,pct);
            sh.hpBar.texture.needsUpdate=true;
            sh.hpBar.lastPct=pct;
          }
        }
      }
      updateHpBars(attackerShips);
      updateHpBars(defenderShips);
      updateHpBars(defenses);
      // Camera now controlled by user — updateCamera() called on input only
      // Subtle ship bobbing (anti-gravity feel) + bomber ring spin
      function animateShip(ship,si,phaseOff){
        ship.mesh.position.y=Math.sin(t*1.2+si*0.7+phaseOff)*1.2;
        // Spin any rings (bomber type) — find children with userData.spin
        for(var ci=0;ci<ship.mesh.children.length;ci++){
          var ch=ship.mesh.children[ci];
          if(ch.userData&&ch.userData.spin){ch.rotation.z=t*1.5;}
        }
      }
      // ── Boids-style separation: build a list of all ship positions for collision avoidance ──
      // Each ship has a "size" radius. If two ships get too close, they push each other apart.
      function shipRadius(type){
        if(type==="chasseur")return 4;
        if(type==="cargo")return 5;
        if(type==="croiseur")return 6;
        if(type==="bombardier")return 6;
        if(type==="cuirasse")return 9;
        return 4;
      }
      
      for(var si=0;si<attackerShips.length;si++){
        var shp=attackerShips[si];
        if(!shp.alive)continue;
        // Compute separation vector from nearby ships (other player ships only for now)
        var sepX=0,sepZ=0;
        var myR=shipRadius(shp.type);
        for(var sj=0;sj<attackerShips.length;sj++){
          if(sj===si)continue;
          var oth=attackerShips[sj];
          var ox=shp.mesh.position.x-oth.mesh.position.x;
          var oz=shp.mesh.position.z-oth.mesh.position.z;
          var od=Math.sqrt(ox*ox+oz*oz);
          var minDist=myR+shipRadius(oth.type);
          if(od<minDist&&od>0.001){
            // Push away from neighbor — strength inversely proportional to distance
            var pushStrength=(minDist-od)/minDist;
            sepX+=(ox/od)*pushStrength;
            sepZ+=(oz/od)*pushStrength;
          }
        }
        // Apply separation as a small displacement (independent of movement)
        var sepMag=Math.sqrt(sepX*sepX+sepZ*sepZ);
        if(sepMag>0.001){
          // Cap the separation step to avoid jitter
          var sepStep=Math.min(sepMag*15*dt,8*dt);
          shp.mesh.position.x+=(sepX/sepMag)*sepStep;
          shp.mesh.position.z+=(sepZ/sepMag)*sepStep;
        }
        
        // Move ship toward destination if it has one
        if(shp.moving){
          var curX=shp.mesh.position.x;
          var curZ=shp.mesh.position.z;
          var dx=shp.destX-curX;
          var dz=shp.destZ-curZ;
          var dist=Math.sqrt(dx*dx+dz*dz);
          // Get ship type for speed (chasseur fast, cuirasse slow)
          var spd=10;
          if(shp.type==="chasseur")spd=18;
          else if(shp.type==="cargo")spd=10;
          else if(shp.type==="croiseur")spd=12;
          else if(shp.type==="bombardier")spd=10;
          else if(shp.type==="cuirasse")spd=6;
          var step=spd*dt;
          if(dist<step+0.1){
            ...
            shp.mesh.position.x+=(dx/dist)*step;
            shp.mesh.position.z+=(dz/dist)*step;
            // Smooth rotation toward target (look at destination)
            var targetAngle=Math.atan2(dx,dz)+Math.PI; // +PI because ships face -z by default
            // Lerp current rotation toward target
            var curR=shp.mesh.rotation.y;
            var diff=targetAngle-curR;
            // Wrap diff to [-PI, PI]
            while(diff>Math.PI)diff-=Math.PI*2;
            while(diff<-Math.PI)diff+=Math.PI*2;
            shp.mesh.rotation.y+=diff*Math.min(1,dt*5);
          }
        }
        animateShip(shp,si,0);
        // Sync halo position with ship + pulse if selected
        if(selectionHalos[si]){
          selectionHalos[si].position.x=shp.mesh.position.x;
          selectionHalos[si].position.z=shp.mesh.position.z;
          if(selectedSet.has(si)){
            selectionHalos[si].material.opacity=0.5+Math.sin(t*4)*0.35; // pulse
            selectionHalos[si].rotation.z=t*0.5; // slow rotation
          }
        }
      }
      
      // Animate click markers (expand + fade out over 0.8 sec)
      for(var cmi=clickMarkers.length-1;cmi>=0;cmi--){
        var cm=clickMarkers[cmi];
        var age=t-cm.userData.bornAt;
        if(age>0.8){
          scene.remove(cm);
          cm.geometry.dispose();
          cm.material.dispose();
          clickMarkers.splice(cmi,1);
        } else {
          var prog=age/0.8;
          cm.scale.set(1+prog*1.5,1+prog*1.5,1);
          cm.material.opacity=0.9*(1-prog);
        }
      }
      for(var di=0;di<defenderShips.length;di++)animateShip(defenderShips[di],di,1.5);
      
      // ── Defender AI: throttled and territorial ──
      // - Defenders advance at 40% of attacker speed (less rushy, gives player time to maneuver)
      // - Defenders never cross the front line at Z=-5 (they hold their half of the battlefield)
      // - If the closest attacker is past the front line, defenders advance only up to it and wait
      var DEFENDER_SPEED_MULT=0.4;
      var DEFENDER_FRONT_LINE_Z=-15;
      function findClosestEnemyAny(unit,enemyList){
        var best=null,bestDist=Infinity;
        for(var fei=0;fei<enemyList.length;fei++){
          var fe=enemyList[fei];
          if(!fe.alive)continue;
          var fex=fe.mesh.position.x-unit.mesh.position.x;
          var fez=fe.mesh.position.z-unit.mesh.position.z;
          var fed=Math.sqrt(fex*fex+fez*fez);
          if(fed<bestDist){best=fe;bestDist=fed;}
        }
        return best?{target:best,distance:bestDist}:null;
      }
      function shipSpeed(type){
        if(type==="chasseur")return 18;
        if(type==="cargo")return 10;
        if(type==="croiseur")return 12;
        if(type==="bombardier")return 10;
        if(type==="cuirasse")return 6;
        return 10;
      }
      // 1. AI decisions: pick target + decide whether to move (clamped to front line)
      for(var dai=0;dai<defenderShips.length;dai++){
        var dShip=defenderShips[dai];
        if(!dShip.alive)continue;
        var nearestInfo=findClosestEnemyAny(dShip,attackerShips);
        if(!nearestInfo){dShip.moving=false;continue;}
        var stopAt=dShip.range*0.85;
        if(nearestInfo.distance>stopAt){
          var ax=nearestInfo.target.mesh.position.x;
          var az=nearestInfo.target.mesh.position.z;
          // Clamp the destination to NEVER cross the front line (Z must stay ≤ DEFENDER_FRONT_LINE_Z)
          var clampedAz=Math.min(DEFENDER_FRONT_LINE_Z,az);
          var ddx=ax-dShip.mesh.position.x;
          var ddz=clampedAz-dShip.mesh.position.z;
          var ddist=Math.sqrt(ddx*ddx+ddz*ddz);
          // If we're already past or at the front line and the target is also past it, hold position
          if(ddist<0.5||dShip.mesh.position.z>=DEFENDER_FRONT_LINE_Z-0.5){
            dShip.moving=false;
            continue;
          }
          var advance=Math.max(0,ddist-stopAt);
          if(advance<0.5){dShip.moving=false;continue;}
          dShip.destX=dShip.mesh.position.x+(ddx/ddist)*advance;
          dShip.destZ=Math.min(DEFENDER_FRONT_LINE_Z,dShip.mesh.position.z+(ddz/ddist)*advance);
          dShip.moving=true;
        }else{
          dShip.moving=false;
        }
      }
      // 2. Apply movement (40% speed of attackers)
      for(var dmi=0;dmi<defenderShips.length;dmi++){
        var dmShip=defenderShips[dmi];
        if(!dmShip.alive||!dmShip.moving)continue;
        var dmCurX=dmShip.mesh.position.x;
        var dmCurZ=dmShip.mesh.position.z;
        var dmDx=dmShip.destX-dmCurX;
        var dmDz=dmShip.destZ-dmCurZ;
        var dmDist=Math.sqrt(dmDx*dmDx+dmDz*dmDz);
        var dmStep=shipSpeed(dmShip.type)*DEFENDER_SPEED_MULT*dt;
        if(dmDist<dmStep+0.1){
          dmShip.moving=false;
        }else{
          dmShip.mesh.position.x+=(dmDx/dmDist)*dmStep;
          dmShip.mesh.position.z+=(dmDz/dmDist)*dmStep;
          var dmTargetAngle=Math.atan2(dmDx,dmDz)+Math.PI;
          var dmCurR=dmShip.mesh.rotation.y;
          var dmDiff=dmTargetAngle-dmCurR;
          while(dmDiff>Math.PI)dmDiff-=Math.PI*2;
          while(dmDiff<-Math.PI)dmDiff+=Math.PI*2;
          dmShip.mesh.rotation.y+=dmDiff*Math.min(1,dt*5);
        }
      }
      
      // ── Combat logic: each ship searches for nearest enemy in range and fires ──
      function findNearestEnemy(unit,enemyList){
        var best=null,bestDist=Infinity;
        for(var ei=0;ei<enemyList.length;ei++){
          var e=enemyList[ei];
          if(!e.alive)continue;
          var ex=e.mesh.position.x-unit.mesh.position.x;
          var ez=e.mesh.position.z-unit.mesh.position.z;
          var ed=Math.sqrt(ex*ex+ez*ez);
          if(ed<bestDist&&ed<=unit.range){best=e;bestDist=ed;}
        }
        return best;
      }
      function processCombat(unit,enemyList){
        if(!unit.alive)return;
        ...
        if(!unit.target||!unit.target.alive){
          ...
          var tx=unit.target.mesh.position.x-unit.mesh.position.x;
          var tz=unit.target.mesh.position.z-unit.mesh.position.z;
          var td=Math.sqrt(tx*tx+tz*tz);
          if(td>unit.range)unit.target=findNearestEnemy(unit,enemyList);
        }
        if(unit.target&&unit.target.alive&&unit.fireCooldown<=0&&unit.dps>0){
          // Apply counter multiplier (e.g. chasseur ×1.5 vs bombardier) + shield aura reduction if target is shielded
          var mult=getDamageMultiplier(unit.type,unit.target.type);
          var dmg=unit.dps*mult;
          if(unit.target.shielded)dmg*=0.8;
          unit.target.hp-=dmg;
          unit.fireCooldown=1.0;
          spawnLaser(unit.mesh.position,unit.target.mesh.position,unit.side==="attacker"?0x88ddff:0xff6688);
          if(unit.target.hp<=0)killUnit(unit.target);
        }
      }
      var defenderUnits=defenderShips.concat(defenses);
      // ── Shield aura: each living def_bouclier protects OTHER defenders within 40 units (incoming damage ×0.8). Doesn't protect itself. ──
      var SHIELD_AURA_RADIUS=40;
      var SHIELD_RADIUS_SQ=SHIELD_AURA_RADIUS*SHIELD_AURA_RADIUS;
      // Reset all shielded flags
      for(var sai=0;sai<defenderShips.length;sai++)defenderShips[sai].shielded=false;
      for(var sbi=0;sbi<defenses.length;sbi++)defenses[sbi].shielded=false;
      // Apply shield from each living bouclier
      for(var bi=0;bi<defenses.length;bi++){
        var bShield=defenses[bi];
        if(!bShield.alive||bShield.type!=="def_bouclier")continue;
        var bx=bShield.mesh.position.x,bz=bShield.mesh.position.z;
        for(var saj=0;saj<defenderShips.length;saj++){
          var dsh2=defenderShips[saj];
          if(!dsh2.alive)continue;
          var ex=dsh2.mesh.position.x-bx,ez=dsh2.mesh.position.z-bz;
          if(ex*ex+ez*ez<=SHIELD_RADIUS_SQ)dsh2.shielded=true;
        }
        for(var sbj=0;sbj<defenses.length;sbj++){
          var dd=defenses[sbj];
          if(!dd.alive||dd===bShield)continue; // bouclier doesn't protect itself
          var ex2=dd.mesh.position.x-bx,ez2=dd.mesh.position.z-bz;
          if(ex2*ex2+ez2*ez2<=SHIELD_RADIUS_SQ)dd.shielded=true;
        }
      }
      for(var aci=0;aci<attackerShips.length;aci++)processCombat(attackerShips[aci],defenderUnits);
      for(var dci=0;dci<defenderShips.length;dci++)processCombat(defenderShips[dci],attackerShips);
      // Defenses fire at attackers too (was missing in the original combat loop — bouclier has dps=0 so it's a no-op for it)
      for(var dfi=0;dfi<defenses.length;dfi++)processCombat(defenses[dfi],attackerShips);
      for(var dfci=0;dfci<defenses.length;dfci++)processCombat(defenses[dfci],attackerShips);
      
      // ── Combat end detection: check if one side is fully wiped out ──
      if(!combatEnded){
        var attackersAlive=0,defendersAlive=0;
        for(var cai=0;cai<attackerShips.length;cai++){if(attackerShips[cai].alive)attackersAlive++;}
        for(var cdi=0;cdi<defenderShips.length;cdi++){if(defenderShips[cdi].alive)defendersAlive++;}
        for(var cfi=0;cfi<defenses.length;cfi++){if(defenses[cfi].alive&&defenses[cfi].dps>0)defendersAlive++;}
        // Victory: all defenders (including hostile defenses) are dead
        // Defeat: all attackers are dead
        if(attackersAlive===0||defendersAlive===0){
          combatEnded=true;
          // Wait 1.5 sec for last explosion to finish, then trigger result screen
          setTimeout(function(){
            // Compute losses per type for both sides
            var atkSurv={chasseur:0,cargo:0,croiseur:0,bombardier:0,cuirasse:0};
            var atkLost={chasseur:0,cargo:0,croiseur:0,bombardier:0,cuirasse:0};
            for(var ki=0;ki<attackerShips.length;ki++){
              var ksh=attackerShips[ki];
              var krep=ksh.repCount||1;
              if(ksh.alive)atkSurv[ksh.type]=(atkSurv[ksh.type]||0)+krep;
              else atkLost[ksh.type]=(atkLost[ksh.type]||0)+krep;
            }
            var defSurv={chasseur:0,cargo:0,croiseur:0,bombardier:0,cuirasse:0};
            var defLost={chasseur:0,cargo:0,croiseur:0,bombardier:0,cuirasse:0};
            for(var ki2=0;ki2<defenderShips.length;ki2++){
              var dsh=defenderShips[ki2];
              var drep=dsh.repCount||1;
              if(dsh.alive)defSurv[dsh.type]=(defSurv[dsh.type]||0)+drep;
              else defLost[dsh.type]=(defLost[dsh.type]||0)+drep;
            }
            var defStrSurv={def_tourelle:0,def_bouclier:0,def_missiles:0,def_forteresse:0};
            var defStrLost={def_tourelle:0,def_bouclier:0,def_missiles:0,def_forteresse:0};
            for(var ki3=0;ki3<defenses.length;ki3++){
              var dst=defenses[ki3];
              var dstrep=dst.repCount||1;
              if(dst.alive)defStrSurv[dst.type]=(defStrSurv[dst.type]||0)+dstrep;
              else defStrLost[dst.type]=(defStrLost[dst.type]||0)+dstrep;
            }
            setCombatResult({
              victor:attackersAlive>0?"attacker":"defender",
              attackerSurvivors:atkSurv,
              attackerLosses:atkLost,
              defenderSurvivors:defSurv,
              defenderLosses:defLost,
              defenseSurvivors:defStrSurv,
              defenseLosses:defStrLost,
            });
          },1500);
        }
      }
      
      // Animate active lasers (fade over 0.4 sec, longer than before for visibility)
      for(var li=lasers.length-1;li>=0;li--){
        var lz=lasers[li];
        var lAge=t-lz.bornAt;
        if(lAge>0.4){
          scene.remove(lz.mesh);
          if(lz.core){lz.core.geometry.dispose();lz.core.material.dispose();}
          if(lz.halo){lz.halo.geometry.dispose();lz.halo.material.dispose();}
          lasers.splice(li,1);
        } else {
          var fadeProgress=1-lAge/0.4;
          if(lz.core)lz.core.material.opacity=fadeProgress;
          if(lz.halo)lz.halo.material.opacity=0.55*fadeProgress;
        }
      }
      
      // Animate explosions (grow + fade)
      for(var ei2=explosions.length-1;ei2>=0;ei2--){
        var ex=explosions[ei2];
        var eAge=t-ex.userData.bornAt;
        if(eAge>0.6){
          scene.remove(ex);
          ex.geometry.dispose();
          ex.material.dispose();
          explosions.splice(ei2,1);
        } else {
          var eProg=eAge/0.6;
          ex.scale.set(1+eProg*4,1+eProg*4,1+eProg*4);
          ex.material.opacity=0.9*(1-eProg);
        }
      }
    ...
    return function(){
      ...
      try{if(selectionBoxEl&&selectionBoxEl.parentNode)selectionBoxEl.parentNode.removeChild(selectionBoxEl);}catch(e){}
      ...
      try{renderer.domElement.removeEventListener("mouseup",onMouseUp);}catch(e){}
      try{renderer.domElement.removeEventListener("touchstart",onTouchStart);}catch(e){}
      try{renderer.domElement.removeEventListener("touchmove",onTouchMove);}catch(e){}
      try{renderer.domElement.removeEventListener("touchend",onTouchEnd);}catch(e){}
      ...
      // Dispose squad count badge sprites and textures (attacker + defender + defenses)
      for(var sqi=0;sqi<attackerSquads.length;sqi++){
        try{attackerSquads[sqi].badgeTexture.dispose();}catch(e){}
        try{attackerSquads[sqi].badgeMaterial.dispose();}catch(e){}
      }
      for(var sqj=0;sqj<defenderSquads.length;sqj++){
        try{defenderSquads[sqj].badgeTexture.dispose();}catch(e){}
        try{defenderSquads[sqj].badgeMaterial.dispose();}catch(e){}
      }
      for(var sqk=0;sqk<defenseSquads.length;sqk++){
        try{defenseSquads[sqk].badgeTexture.dispose();}catch(e){}
        try{defenseSquads[sqk].badgeMaterial.dispose();}catch(e){}
      }
      // Dispose HP bar sprites/textures
      function disposeHpBars(arr){
        for(var dhi=0;dhi<arr.length;dhi++){
          if(arr[dhi].hpBar){
            try{arr[dhi].hpBar.texture.dispose();}catch(e){}
            try{arr[dhi].hpBar.material.dispose();}catch(e){}
          }
        }
      }
      disposeHpBars(attackerShips);
      disposeHpBars(defenderShips);
      disposeHpBars(defenses);
    };
  },[targetPlanet]);
  
  // Stats summary
  var totalAtt=(attackerComp.chasseur||0)+(attackerComp.croiseur||0)+(attackerComp.bombardier||0)+(attackerComp.cuirasse||0)+(attackerComp.cargo||0);
  var totalDef=(defenderMilitary&&defenderMilitary.ships)?(defenderMilitary.ships.chasseur+defenderMilitary.ships.croiseur+defenderMilitary.ships.bombardier+defenderMilitary.ships.cuirasse):0;
  var totalDefStruct=(defenderMilitary&&defenderMilitary.defenses)?(defenderMilitary.defenses.def_tourelle+defenderMilitary.defenses.def_bouclier+defenderMilitary.defenses.def_missiles+defenderMilitary.defenses.def_forteresse):0;
  
  return (
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"#000",zIndex:300,overflow:"hidden"}}>
      <div ref={mountRef} style={{position:"absolute",top:0,left:0,width:"100%",height:"100%"}}/>
      
      {/* Top HUD: target info + close button */}
      <div style={{position:"absolute",top:0,left:0,right:0,padding:"10px 14px",paddingTop:"calc(10px + env(safe-area-inset-top,0px))",background:"linear-gradient(180deg,rgba(8,4,16,0.85),transparent)",pointerEvents:"none"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={{pointerEvents:"auto"}}>
            <div style={{fontSize:11,color:"#ff8899",fontFamily:"monospace",fontWeight:"bold",letterSpacing:2}}>{"\u2694\uFE0F BATAILLE"}</div>
            <div style={{fontSize:9,color:"#cc6688",fontFamily:"monospace",marginTop:2}}>Cible : {targetPlanet&&targetPlanet.name}</div>
          </div>
          <button onClick={onClose} style={{pointerEvents:"auto",background:"rgba(40,15,25,0.85)",border:"1px solid #aa4466",borderRadius:5,color:"#ffaabb",fontSize:13,padding:"6px 14px",cursor:"pointer",fontFamily:"monospace",fontWeight:"bold",letterSpacing:1.5}}>{"× FUIR"}</button>
        </div>
      </div>
      
      {/* Selection mode toggle — left side */}
      <div style={{position:"absolute",left:"calc(10px + env(safe-area-inset-left,0px))",top:"calc(70px + env(safe-area-inset-top,0px))",pointerEvents:"auto"}}>
        <button onClick={function(){setSelectionMode(!selectionMode);}} style={{padding:"8px 12px",borderRadius:8,border:"1px solid "+(selectionMode?"#88ddff":"rgba(120,180,220,0.5)"),background:selectionMode?"rgba(60,140,200,0.85)":"rgba(20,30,50,0.8)",color:selectionMode?"#fff":"#aaccee",fontSize:11,fontFamily:"monospace",fontWeight:"bold",letterSpacing:1,cursor:"pointer",backdropFilter:"blur(4px)",boxShadow:selectionMode?"0 0 12px rgba(136,221,255,0.6)":"none",display:"flex",alignItems:"center",gap:6}}>
          <span style={{fontSize:14}}>{"\u{1F532}"}</span>
          <span>{selectionMode?"SÉLECTION":"SÉLECTION"}</span>
        </button>
        {selectionMode&&<div style={{marginTop:6,padding:"4px 8px",background:"rgba(20,40,60,0.85)",border:"1px solid rgba(136,221,255,0.4)",borderRadius:5,fontSize:8,color:"#88ddff",fontFamily:"monospace",letterSpacing:1,maxWidth:140,lineHeight:1.4,backdropFilter:"blur(4px)"}}>{"Glisse pour entourer tes vaisseaux"}</div>}
      </div>
      
      {/* Camera controls — right side, vertical stack */}
      <div style={{position:"absolute",right:"calc(10px + env(safe-area-inset-right,0px))",top:"calc(70px + env(safe-area-inset-top,0px))",display:"flex",flexDirection:"column",gap:6,pointerEvents:"auto"}}>
        <button onClick={function(){if(camControlsRef.current)camControlsRef.current.zoomIn();}} style={{width:38,height:38,borderRadius:8,border:"1px solid rgba(120,180,220,0.5)",background:"rgba(20,30,50,0.8)",color:"#aaccee",fontSize:16,fontFamily:"monospace",fontWeight:"bold",cursor:"pointer",backdropFilter:"blur(4px)"}}>+</button>
        <button onClick={function(){if(camControlsRef.current)camControlsRef.current.zoomOut();}} style={{width:38,height:38,borderRadius:8,border:"1px solid rgba(120,180,220,0.5)",background:"rgba(20,30,50,0.8)",color:"#aaccee",fontSize:16,fontFamily:"monospace",fontWeight:"bold",cursor:"pointer",backdropFilter:"blur(4px)"}}>−</button>
        <button onClick={function(){if(camControlsRef.current)camControlsRef.current.recenter();}} style={{width:38,height:38,borderRadius:8,border:"1px solid rgba(120,180,220,0.5)",background:"rgba(20,30,50,0.8)",color:"#aaccee",fontSize:14,fontFamily:"monospace",cursor:"pointer",backdropFilter:"blur(4px)"}}>{"\u{1F3AF}"}</button>
      </div>
      
      {/* Bottom HUD: forces summary */}
      <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"6px 10px",paddingBottom:"calc(6px + env(safe-area-inset-bottom,0px))",background:"linear-gradient(0deg,rgba(8,4,16,0.7),transparent)",pointerEvents:"none"}}>
        <div style={{display:"flex",justifyContent:"space-around",gap:6,pointerEvents:"auto"}}>
          <div style={{flex:1,padding:"5px 8px",background:"rgba(20,40,80,0.6)",border:"1px solid rgba(68,136,204,0.6)",borderRadius:5}}>
            <div style={{fontSize:7,color:"#88ccff",fontFamily:"monospace",letterSpacing:1.2,marginBottom:1}}>{"\u{1F535} VOTRE FLOTTE"}</div>
            <div style={{fontSize:11,color:"#fff",fontFamily:"monospace",fontWeight:"bold"}}>{totalAtt} vaisseaux</div>
          </div>
          <div style={{flex:1,padding:"5px 8px",background:"rgba(80,20,30,0.6)",border:"1px solid rgba(204,68,102,0.6)",borderRadius:5}}>
            <div style={{fontSize:7,color:"#ff88aa",fontFamily:"monospace",letterSpacing:1.2,marginBottom:1}}>{"\u{1F534} ENNEMI"}</div>
            <div style={{fontSize:11,color:"#fff",fontFamily:"monospace",fontWeight:"bold"}}>{totalDef} vaisseaux · {totalDefStruct} def</div>
          </div>
        </div>
      </div>
      
      {/* Combat result screen — appears when combat ends */}
      {combatResult&&<CombatResultModal result={combatResult} targetPlanet={targetPlanet} testMode={testMode} onClose={function(){if(onResolve)onResolve(combatResult);else onClose();}}/>}
    </div>
  );
}

// CombatResultModal — VICTOIRE / DÉFAITE screen with losses summary
function CombatResultModal({result,targetPlanet,testMode,onClose}){
  var isWin=result.victor==="attacker";
  var SHIP_INFO={
    chasseur:{name:"Chasseurs",icon:"\u{1F6F8}"},
    cargo:{name:"Cargos",icon:"\u{1F6F3}\uFE0F"},
    croiseur:{name:"Croiseurs",icon:"\u2694\uFE0F"},
    bombardier:{name:"Bombardiers",icon:"\u{1F6F0}\uFE0F"},
    cuirasse:{name:"Cuirasses",icon:"\u2694\uFE0F"},
  };
  var DEF_INFO={
    def_tourelle:{name:"Tourelles",icon:"\u{1F52B}"},
    def_bouclier:{name:"Boucliers",icon:"\u{1F6E1}\uFE0F"},
    def_missiles:{name:"Missiles",icon:"\u{1F680}"},
    def_forteresse:{name:"Forteresses",icon:"\u{1F3F0}"},
  };
  
  // Helper: render a list of unit changes (losses)
  function renderLosses(losses,info){
    var entries=Object.keys(losses).filter(function(k){return losses[k]>0;});
    if(entries.length===0)return <div style={{fontSize:10,color:"#88dd99",fontFamily:"monospace",fontStyle:"italic"}}>Aucune perte</div>;
    return entries.map(function(k){
      return (
        <div key={k} style={{display:"flex",justifyContent:"space-between",fontSize:10,padding:"3px 0",color:"#ffaaaa",fontFamily:"monospace"}}>
          <span>{info[k].icon+" "+info[k].name}</span>
          <span style={{fontWeight:"bold"}}>-{losses[k]}</span>
        </div>
      );
    });
  }
  ...
  return (
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.85)",zIndex:400,display:"flex",alignItems:"center",justifyContent:"center",padding:14,backdropFilter:"blur(8px)"}}>
      <div style={{maxWidth:480,width:"100%",maxHeight:"92vh",overflow:"auto",WebkitOverflowScrolling:"touch",background:isWin?"linear-gradient(135deg,rgba(20,60,40,0.97),rgba(10,40,25,0.97))":"linear-gradient(135deg,rgba(60,15,25,0.97),rgba(40,8,15,0.97))",border:"2px solid "+(isWin?"rgba(100,220,140,0.6)":"rgba(220,80,100,0.6)"),borderRadius:10,padding:20,boxShadow:"0 0 40px "+(isWin?"rgba(100,220,140,0.4)":"rgba(220,80,100,0.4)")}}>
        
        {/* Big victory/defeat banner */}
        <div style={{textAlign:"center",marginBottom:18,paddingBottom:12,borderBottom:"1px solid "+(isWin?"rgba(100,220,140,0.3)":"rgba(220,80,100,0.3)")}}>
          <div style={{fontSize:32,marginBottom:6}}>{isWin?"\u{1F3C6}":"\u{1F480}"}</div>
          <div style={{fontSize:22,color:isWin?"#88ee99":"#ff8899",fontFamily:"monospace",fontWeight:"bold",letterSpacing:5,textShadow:"0 0 20px "+(isWin?"#44dd66":"#ee4466")}}>{isWin?"VICTOIRE":"DÉFAITE"}</div>
          <div style={{fontSize:10,color:isWin?"#88cc99":"#cc8899",fontFamily:"monospace",marginTop:6,letterSpacing:1.5}}>Bataille de {targetPlanet&&targetPlanet.name}</div>
        </div>
        
        {/* Your fleet section */}
        <div style={{marginBottom:14,padding:"10px 12px",background:"rgba(20,40,80,0.4)",border:"1px solid rgba(68,136,204,0.4)",borderRadius:6}}>
          <div style={{fontSize:9,color:"#88ccff",fontFamily:"monospace",letterSpacing:1.8,marginBottom:8,fontWeight:"bold"}}>{"\u{1F535} VOTRE FLOTTE"}</div>
          <div style={{fontSize:9,color:"#aaccee",fontFamily:"monospace",letterSpacing:1,marginBottom:4}}>PERTES</div>
          {renderLosses(result.attackerLosses,SHIP_INFO)}
        </div>
        
        {/* Enemy section */}
        <div style={{marginBottom:14,padding:"10px 12px",background:"rgba(80,20,30,0.4)",border:"1px solid rgba(204,68,102,0.4)",borderRadius:6}}>
          <div style={{fontSize:9,color:"#ff88aa",fontFamily:"monospace",letterSpacing:1.8,marginBottom:8,fontWeight:"bold"}}>{"\u{1F534} FLOTTE ENNEMIE"}</div>
          <div style={{fontSize:9,color:"#eeaacc",fontFamily:"monospace",letterSpacing:1,marginBottom:4}}>PERTES</div>
          {renderLosses(result.defenderLosses,SHIP_INFO)}
          {Object.values(result.defenseLosses).some(function(v){return v>0;})&&
            <div style={{marginTop:8,paddingTop:6,borderTop:"1px dashed rgba(204,68,102,0.3)"}}>
              <div style={{fontSize:9,color:"#eeaacc",fontFamily:"monospace",letterSpacing:1,marginBottom:4}}>DÉFENSES DÉTRUITES</div>
              {renderLosses(result.defenseLosses,DEF_INFO)}
            </div>
          }
        </div>
        
        {/* Banner about test mode or real losses */}
        {testMode?
          <div style={{padding:8,background:"rgba(180,150,80,0.15)",border:"1px solid rgba(180,150,80,0.4)",borderRadius:5,marginBottom:14,fontSize:9,color:"#ddcc88",fontFamily:"monospace",lineHeight:1.5,textAlign:"center"}}>{"\u{1F9EA} MODE TEST : Aucune perte n\u2019a \u00E9t\u00E9 appliqu\u00E9e \u00E0 votre flotte."}</div>
          :
          <div style={{padding:8,background:"rgba(180,40,60,0.15)",border:"1px solid rgba(220,80,100,0.4)",borderRadius:5,marginBottom:14,fontSize:9,color:"#ffaaaa",fontFamily:"monospace",lineHeight:1.5,textAlign:"center"}}>{"\u26A0\uFE0F Vos vaisseaux perdus ont \u00E9t\u00E9 retir\u00E9s de votre flotte."}</div>
        }
        
        <button onClick={onClose} style={{width:"100%",padding:"14px 0",borderRadius:6,border:"1px solid "+(isWin?"#66cc88":"#cc6688"),cursor:"pointer",background:isWin?"linear-gradient(180deg,rgba(60,180,100,0.85),rgba(30,120,60,0.85))":"linear-gradient(180deg,rgba(200,40,80,0.85),rgba(140,20,50,0.85))",color:"#fff",fontFamily:"monospace",fontSize:12,letterSpacing:2.5,fontWeight:"bold"}}>{"\u{1F30C} RETOUR À LA GALAXIE"}</button>
      </div>
    </div>
  );
}
          ...
          <div style={{position:"absolute",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,8,0.85)",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",pointerEvents:"auto",padding:20,backdropFilter:"blur(8px)"}} onClick={()=>setAnalyzeModal(null)}>
            <div onClick={function(e){e.stopPropagation();}} style={{background:"linear-gradient(145deg,rgba(20,15,35,0.98),rgba(10,8,20,0.98))",border:"1px solid #6644aa",borderRadius:10,padding:18,maxWidth:520,width:"100%",maxHeight:"90vh",overflowY:"auto",fontFamily:"monospace",color:"#d8b8ff",boxShadow:"0 0 30px rgba(170,100,255,0.4)"}}>
              <div style={{borderBottom:"1px solid #4a3070",paddingBottom:10,marginBottom:12}}>
                <div style={{fontSize:14,fontWeight:"bold",letterSpacing:1,color:"#fff"}}>{(atype&&atype.name||"?").toUpperCase()}</div>
    ...
    <div onClick={e=>e.stopPropagation()} style={{width:320,maxWidth:"100%",background:"linear-gradient(160deg,#0a101e,#12182a)",border:"1px solid "+accent+"66",borderRadius:12,padding:22,color:"#9abcd8",boxShadow:"0 0 40px "+accent+"44"}}>
      ...
      {display&&(
        <div style={{textAlign:"center",marginBottom:14,padding:"10px 12px",borderRadius:8,background:"rgba(8,16,30,0.6)",border:"1px solid #1a3050"}}>
          <div style={{fontSize:8,color:"#5a7aa0",letterSpacing:1.5,marginBottom:4}}>{(display&&display.typeLabel?display.typeLabel:"?").toUpperCase()}</div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              {[
                ...
                <div key={i} style={{padding:"10px 12px",borderRadius:8,background:"rgba(8,16,32,0.6)",border:"1px solid #1a3050"}}>
                  <div style={{fontSize:8,color:"#5a7aa0",letterSpacing:1.5,marginBottom:3}}>{s.i+" "+(s&&s.l?s.l:"?").toUpperCase()}</div>
              ...
              {PROFILE_BANNERS.map(b=>{
                ...
                return(
                  <button key={b.id} onClick={()=>{if(unlocked)updateProfile({bannerId:b.id});else setLockedItem({category:"banner",itemId:b.id});}} style={{padding:0,borderRadius:7,border:"2px solid "+(sel?col:"#0d2035"),cursor:"pointer",overflow:"hidden",transition:"all 0.15s",textAlign:"center",opacity:unlocked?1:0.55,position:"relative"}}>
                    <div style={{height:44,background:b.g,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:9,fontFamily:"monospace",letterSpacing:2,fontWeight:"bold",textShadow:"0 1px 3px rgba(0,0,0,0.8)"}}>{sel?"★ "+(b&&b.name?b.name:"?").toUpperCase():(b&&b.name?b.name:"?").toUpperCase()}</div>
        ...
        return(
          <div style={{maxWidth:600,margin:"0 auto 14px",padding:"10px 14px",borderRadius:8,background:"linear-gradient(160deg,"+bb.color+"22,"+bb.color+"11)",border:"1px solid "+bb.color+"66",boxShadow:"0 0 14px "+bb.color+"22"}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:4}}>
              ...
              <div style={{flex:1}}>
                <div style={{fontSize:10,color:bb.color,fontFamily:"monospace",letterSpacing:2,fontWeight:"bold"}}>{"SPÉCIALISATION : "+(bb&&bb.name?bb.name:"?").toUpperCase()}</div>
              ...
              return(
                <div key={m.id} onClick={()=>setLockedInfo(m)} style={{
                  ...
                  <div style={{display:"flex",alignItems:"flex-start",gap:12,marginBottom:8}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:12,color:"#d8a8a8",fontWeight:"bold",letterSpacing:1.5,marginBottom:4}}>{(m&&m.title?m.title:"?").toUpperCase()}</div>
            ...
            return(
              <div key={m.id} style={{
                ...
                <div style={{display:"flex",alignItems:"flex-start",gap:12,marginBottom:10}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:12,color:ready?"#b0ffc0":"#c8e0ff",fontWeight:"bold",letterSpacing:1.5,marginBottom:4}}>{(m&&m.title?m.title:"?").toUpperCase()}</div>
                    ...
                    <div style={{display:"flex",gap:6,flexWrap:"wrap",width:"100%",marginTop:4}}>
                      {[{k:"eco",n:"Économique",i:"💰",c:"#d4a850"},{k:"mil",n:"Militaire",i:"⚔️",c:"#d46060"},{k:"exp",n:"Exploration",i:"🔭",c:"#60a0d4"}].map(br=>(
                        <button key={br.k} onClick={()=>setBranchConfirm({mission:m,branchKey:br.k})} style={{
                          ...
                          <div>{(br&&br.n?br.n:"?").toUpperCase()}</div>
                        </button>
                      ))}
                    </div>
                  ):ready&&(
        ...
        <div onClick={()=>setBranchConfirm(null)} style={{position:"fixed",inset:0,background:"rgba(0,5,15,0.92)",zIndex:700,display:"flex",alignItems:"center",justifyContent:"center",padding:16,backdropFilter:"blur(8px)"}}>
          <div onClick={e=>e.stopPropagation()} style={{width:"min(440px,100%)",maxHeight:"88vh",overflowY:"auto",WebkitOverflowScrolling:"touch",background:"linear-gradient(180deg,rgba(15,18,30,0.98),rgba(8,10,20,0.98))",border:"2px solid "+br.color,borderRadius:10,boxShadow:"0 0 40px "+br.color+"55"}}>
            ...
            <div style={{padding:"16px 18px",borderBottom:"1px solid "+br.color+"55",background:"linear-gradient(180deg,"+br.color+"22,transparent)",textAlign:"center"}}>
              ...
              <div style={{fontSize:13,color:br.color,fontFamily:"monospace",letterSpacing:3,fontWeight:"bold"}}>{"BRANCHE "+(br&&br.name?br.name:"?").toUpperCase()}</div>
...
function HubSection({section,planet,userId,planets,myIdx,initialState,onStateChange,onClose,onOpenZoneInfo}){
  ...
  const titles={continents:"🗺️ CONTINENTS",buildings:"🏗️ BÂTIMENTS",orbital:"\u{1F6F0}\uFE0F ORBITAL",ships:"🛸 VAISSEAUX & FLOTTES",research:"🔬 RECHERCHE"};
  // Build an orbital structure (instant, no queue) — pays cost, increments counter
  function doBuildOrbital(bid){
    var b=ORBITAL_BUILDINGS.find(function(x){return x.id===bid;});
    if(!b)return;
    var cur=loadGS(userId);
    if(!cur)return;
    var isDefense=b.kind==="defense"||b.kind==="defense_consumable";
    var curLv=isDefense?((cur.defenses&&cur.defenses[bid])||0):((cur.orbitalBuildings&&cur.orbitalBuildings[bid])||0);
    // Check max
    if(isDefense&&curLv>=b.maxQuantity)return;
    if(!isDefense&&b.maxLevel&&curLv>=b.maxLevel)return;
    // Compute cost
    var cost;
    if(isDefense){
      cost={metal:b.base.metal,crystal:b.base.crystal,deut:b.base.deut};
    } else {
      var mult=Math.pow(b.factor,curLv);
      cost={metal:Math.round(b.base.metal*mult),crystal:Math.round(b.base.crystal*mult),deut:Math.round(b.base.deut*mult)};
    }
    // Check resources
    if((cur.resources&&cur.resources.metal||0)<cost.metal)return;
    if((cur.resources&&cur.resources.crystal||0)<cost.crystal)return;
    if((cur.resources&&cur.resources.deut||0)<cost.deut)return;
    // Apply
    var ns={...cur,resources:{...cur.resources,metal:cur.resources.metal-cost.metal,crystal:cur.resources.crystal-cost.crystal,deut:cur.resources.deut-cost.deut}};
    if(isDefense){
      ns.defenses={...(cur.defenses||{}),[bid]:curLv+1};
    } else {
      ns.orbitalBuildings={...(cur.orbitalBuildings||{}),[bid]:curLv+1};
    }
    saveGS(userId,ns);
    setState(ns);
  ...
  return(<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,zIndex:200,display:"flex",flexDirection:"column",background:"linear-gradient(160deg,#06101e,#08141f)",fontFamily:"monospace"}}>
    ...
    <div style={{flex:1,overflow:"auto",WebkitOverflowScrolling:"touch",padding:8,paddingLeft:"calc(8px + "+safeL+")",paddingRight:"calc(8px + "+safeR+")"}}>
      ...
      {section==="orbital"&&<OrbitalTab state={state} onBuildOrbital={doBuildOrbital}/>}
...
// AttackPrepModal — choose the composition of ships to send for an attack
function AttackPrepModal({planet,natalState,attackComp,setAttackComp,onClose,onLaunch,testMode,setTestMode}){
  var avail=(natalState&&natalState.ships)||{};
  var totalSelected=(attackComp.chasseur||0)+(attackComp.cargo||0)+(attackComp.croiseur||0)+(attackComp.bombardier||0)+(attackComp.cuirasse||0);
  var hasMilitary=(attackComp.chasseur||0)+(attackComp.croiseur||0)+(attackComp.bombardier||0)+(attackComp.cuirasse||0)>0;
  // Combat power estimate
  var totalDPS=(attackComp.chasseur||0)*35+(attackComp.croiseur||0)*80+(attackComp.bombardier||0)*120+(attackComp.cuirasse||0)*200;
  var totalHP=(attackComp.chasseur||0)*80+(attackComp.cargo||0)*120+(attackComp.croiseur||0)*400+(attackComp.bombardier||0)*300+(attackComp.cuirasse||0)*1500;
  
  function adjust(shipId,delta){
    var cur=attackComp[shipId]||0;
    var max=avail[shipId]||0;
    var newVal=Math.max(0,Math.min(max,cur+delta));
    setAttackComp({...attackComp,[shipId]:newVal});
  }
  function setMax(shipId){setAttackComp({...attackComp,[shipId]:avail[shipId]||0});}
  function setZero(shipId){setAttackComp({...attackComp,[shipId]:0});}
  
  var SHIP_TYPES=[
    {id:"chasseur",name:"Chasseur",icon:"\u{1F6F8}",color:"#88ccff"},
    {id:"cargo",name:"Cargo",icon:"\u{1F6F3}\uFE0F",color:"#cccc88"},
    {id:"croiseur",name:"Croiseur",icon:"\u2694\uFE0F",color:"#ff8855"},
    {id:"bombardier",name:"Bombardier",icon:"\u{1F6F0}\uFE0F",color:"#ff5588"},
    {id:"cuirasse",name:"Cuirasse",icon:"\u2694\uFE0F",color:"#aa66ff"},
  ];
  
  return (
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(8,4,16,0.92)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:14}} onClick={function(e){if(e.target===e.currentTarget)onClose();}}>
      <div style={{maxWidth:520,width:"100%",maxHeight:"90vh",overflow:"auto",WebkitOverflowScrolling:"touch",background:"linear-gradient(135deg,rgba(40,15,25,0.97),rgba(20,8,15,0.97))",border:"2px solid rgba(220,80,100,0.5)",borderRadius:10,padding:18,boxShadow:"0 0 30px rgba(180,40,60,0.4)"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12,paddingBottom:10,borderBottom:"1px solid rgba(220,80,100,0.3)"}}>
          <div>
            <div style={{fontSize:14,color:"#ffaabb",fontFamily:"monospace",fontWeight:"bold",letterSpacing:2}}>{"\u2694\uFE0F PRÉPARATION D\u2019ATTAQUE"}</div>
            <div style={{fontSize:9,color:"#aa6688",fontFamily:"monospace",marginTop:3}}>Cible : {planet&&planet.name}</div>
          </div>
          <button onClick={onClose} style={{background:"transparent",border:"1px solid #aa4466",borderRadius:4,color:"#cc8899",fontSize:14,padding:"4px 10px",cursor:"pointer",fontFamily:"monospace"}}>×</button>
        </div>
        <div style={{padding:10,background:"rgba(180,40,60,0.15)",border:"1px solid rgba(220,80,100,0.3)",borderRadius:6,marginBottom:14,fontSize:9,color:"#ffaabb",lineHeight:1.5,fontFamily:"monospace"}}>{"Sélectionne les vaisseaux à envoyer. Tu pourras les manoeuvrer pendant le combat. Les pertes seront définitives."}</div>
        
        {SHIP_TYPES.map(function(s){
          var availQty=avail[s.id]||0;
          var sel=attackComp[s.id]||0;
          if(availQty===0)return null;
          return (
            <div key={s.id} style={{padding:10,background:"rgba(15,8,20,0.7)",border:"1px solid "+s.color+"33",borderRadius:6,marginBottom:8}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
                <span style={{fontSize:22,filter:"drop-shadow(0 0 5px "+s.color+"77)"}}>{s.icon}</span>
                <div style={{flex:1}}>
                  <div style={{fontSize:11,color:"#fff",fontFamily:"monospace",fontWeight:"bold"}}>{s.name}</div>
                  <div style={{fontSize:9,color:"#9abcd8",fontFamily:"monospace"}}>Disponible : {availQty}</div>
                </div>
                <div style={{fontSize:14,color:s.color,fontFamily:"monospace",fontWeight:"bold"}}>{sel}/{availQty}</div>
              </div>
              <div style={{display:"flex",gap:4}}>
                <button onClick={function(){setZero(s.id);}} style={{flex:1,padding:"6px 0",background:"rgba(60,30,40,0.7)",border:"1px solid "+s.color+"55",borderRadius:4,color:s.color,fontSize:9,fontFamily:"monospace",fontWeight:"bold",cursor:"pointer"}}>0</button>
                <button onClick={function(){adjust(s.id,-10);}} style={{flex:1,padding:"6px 0",background:"rgba(60,30,40,0.7)",border:"1px solid "+s.color+"55",borderRadius:4,color:s.color,fontSize:9,fontFamily:"monospace",fontWeight:"bold",cursor:"pointer"}}>-10</button>
                <button onClick={function(){adjust(s.id,-1);}} style={{flex:1,padding:"6px 0",background:"rgba(60,30,40,0.7)",border:"1px solid "+s.color+"55",borderRadius:4,color:s.color,fontSize:9,fontFamily:"monospace",fontWeight:"bold",cursor:"pointer"}}>-1</button>
                <button onClick={function(){adjust(s.id,1);}} style={{flex:1,padding:"6px 0",background:"rgba(60,30,40,0.7)",border:"1px solid "+s.color+"55",borderRadius:4,color:s.color,fontSize:9,fontFamily:"monospace",fontWeight:"bold",cursor:"pointer"}}>+1</button>
                <button onClick={function(){adjust(s.id,10);}} style={{flex:1,padding:"6px 0",background:"rgba(60,30,40,0.7)",border:"1px solid "+s.color+"55",borderRadius:4,color:s.color,fontSize:9,fontFamily:"monospace",fontWeight:"bold",cursor:"pointer"}}>+10</button>
                <button onClick={function(){setMax(s.id);}} style={{flex:1,padding:"6px 0",background:"rgba(60,30,40,0.7)",border:"1px solid "+s.color+"55",borderRadius:4,color:s.color,fontSize:9,fontFamily:"monospace",fontWeight:"bold",cursor:"pointer"}}>MAX</button>
              </div>
            </div>
          );
        })}
        ...
        {/* Summary */}
        <div style={{padding:10,background:"rgba(20,10,25,0.8)",border:"1px solid rgba(220,80,100,0.3)",borderRadius:6,marginBottom:12}}>
          <div style={{fontSize:9,color:"#ff88aa",fontFamily:"monospace",letterSpacing:1.5,marginBottom:6}}>{"\u{1F4CA} FORCE TOTALE"}</div>
          <div style={{display:"flex",justifyContent:"space-between",fontSize:10,fontFamily:"monospace",color:"#ffccdd"}}>
            <span>Vaisseaux : {totalSelected}</span>
            <span>HP total : {totalHP}</span>
            <span>DPS total : {totalDPS}</span>
          </div>
        </div>
        
        {/* Mode test toggle — when ON, no real losses applied */}
        <div onClick={function(){setTestMode(!testMode);}} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",background:testMode?"rgba(180,150,80,0.18)":"rgba(20,15,30,0.6)",border:"1px solid "+(testMode?"rgba(220,180,80,0.5)":"rgba(80,80,100,0.3)"),borderRadius:6,marginBottom:10,cursor:"pointer"}}>
          <div style={{width:18,height:18,borderRadius:4,border:"2px solid "+(testMode?"#ddcc88":"#666"),background:testMode?"#ddcc88":"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
            {testMode&&<span style={{color:"#332200",fontSize:13,fontWeight:"bold",lineHeight:1}}>{"\u2713"}</span>}
          </div>
          <div style={{flex:1}}>
            <div style={{fontSize:10,color:testMode?"#ffeebb":"#aaaaaa",fontFamily:"monospace",fontWeight:"bold",letterSpacing:1.2}}>{"\u{1F9EA} MODE TEST"}</div>
            <div style={{fontSize:8,color:testMode?"#ddcc88":"#888888",fontFamily:"monospace",marginTop:2,lineHeight:1.4}}>{testMode?"Aucune perte ne sera appliquée à votre flotte":"Les pertes seront définitives"}</div>
          </div>
        </div>
        
        <button onClick={function(){if(hasMilitary)onLaunch();}} disabled={!hasMilitary} style={{width:"100%",padding:"14px 0",borderRadius:6,border:"1px solid "+(hasMilitary?"#cc6688":"#403040"),cursor:hasMilitary?"pointer":"not-allowed",background:hasMilitary?"linear-gradient(180deg,rgba(200,40,80,0.85),rgba(140,20,50,0.85))":"rgba(40,20,30,0.5)",color:hasMilitary?"#fff":"#604050",fontFamily:"monospace",fontSize:12,letterSpacing:2.5,fontWeight:"bold",boxShadow:hasMilitary?"0 0 16px rgba(220,80,100,0.4)":"none"}}>{hasMilitary?"\u{1F680} LANCER L\u2019ATTAQUE":"AUCUN VAISSEAU OFFENSIF SÉLECTIONNÉ"}</button>
      </div>
    </div>
  );
}

function Viewer3D({planet,allPlanets,onClose,isPlayer,owner,state,onStateChange,onManage,userId,natalState,onColonize,alreadyColonized,onLaunchAttack}){
  ...
  // Attack composition: how many of each ship type to send
  const[attackComp,setAttackComp]=useState({chasseur:0,cargo:0,croiseur:0,bombardier:0,cuirasse:0});
  const[attackTestMode,setAttackTestMode]=useState(true); // Default ON for safety
  ...
  return(<div style={outerSt}>
        ...
        <div style={scrollSt}>
          ...
          {viewTab==="info"&&(<div style={{padding:"12px 14px"}}>
            ...
            <div style={{padding:"10px 12px",borderRadius:6,background:"linear-gradient(135deg,rgba(10,25,60,.7),rgba(6,16,40,.9))",border:"1px solid "+hab.c+"55",marginBottom:14,boxShadow:"0 0 14px "+hab.c+"22",position:"relative"}}>
              ...
              <div style={{textAlign:"center",padding:"14px 0 6px"}}>
                <div style={{fontSize:17,color:hab.c,fontFamily:"'Share Tech Mono',monospace",fontWeight:"bold",letterSpacing:3}}>{(hab&&hab.l?hab.l:"?").toUpperCase()}</div>
              </div>
            ...
            {/* Renseignements militaires — visible only for OTHER players' planets (bots) */}
            {!isPlayer&&!planet.isWild&&(function(){
              // Compute bot military based on player QG level
              var playerQG=(natalState&&natalState.buildings&&natalState.buildings.qg)||1;
              var botMil=generateBotMilitary(playerQG,planet.i*7+1);
              // Determine info detail level based on player branch
              var playerBranch=(natalState&&natalState.missionFlags&&natalState.missionFlags.chosenBranch)||null;
              var fullDetails=playerBranch==="exp";
              var threat=getThreatLevel(botMil,playerQG);
              var threatColor=threat==="Faible"?"#88cc66":(threat==="Modere"?"#cccc66":(threat==="Fort"?"#cc8866":"#ff5566"));
              return (
                <div style={{marginBottom:14,padding:"10px 12px",borderRadius:6,background:"rgba(30,8,16,0.5)",border:"1px solid rgba(180,40,80,0.25)"}}>
                  <div style={{fontSize:8,color:"#cc4488",letterSpacing:2.5,marginBottom:8,fontFamily:"monospace",display:"flex",alignItems:"center",gap:6}}>
                    <span style={{width:12,height:1,background:"#802050"}}></span>
                    {"\u2694\uFE0F RENSEIGNEMENTS MILITAIRES"}
                    <span style={{flex:1,height:1,background:"#802050"}}></span>
                  </div>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:10,padding:"5px 0",borderBottom:"1px dashed rgba(180,40,80,0.18)"}}>
                    <span style={{color:"#a06a8a",fontFamily:"monospace",letterSpacing:1.5,fontSize:9}}>NIVEAU MENACE</span>
                    <span style={{color:threatColor,fontFamily:"monospace",fontSize:11,fontWeight:"bold",letterSpacing:1}}>{threat.toUpperCase()}</span>
                  </div>
                  {fullDetails?(
                    <div>
                      <div style={{fontSize:8,color:"#cc88aa",letterSpacing:1.5,padding:"6px 0 4px",fontFamily:"monospace",fontStyle:"italic"}}>{"\u{1F50D} Reconnaissance Exploration"}</div>
                      {botMil.ships.chasseur>0?<div style={{display:"flex",justifyContent:"space-between",fontSize:9,padding:"3px 0",color:"#c8a0bb",fontFamily:"monospace"}}><span>{"\u{1F6F8} Chasseurs"}</span><span>{botMil.ships.chasseur}</span></div>:null}
                      {botMil.ships.cargo>0?<div style={{display:"flex",justifyContent:"space-between",fontSize:9,padding:"3px 0",color:"#c8a0bb",fontFamily:"monospace"}}><span>{"\u{1F6F3}\uFE0F Cargos"}</span><span>{botMil.ships.cargo}</span></div>:null}
                      {botMil.ships.croiseur>0?<div style={{display:"flex",justifyContent:"space-between",fontSize:9,padding:"3px 0",color:"#c8a0bb",fontFamily:"monospace"}}><span>{"\u2694\uFE0F Croiseurs"}</span><span>{botMil.ships.croiseur}</span></div>:null}
                      {botMil.ships.bombardier>0?<div style={{display:"flex",justifyContent:"space-between",fontSize:9,padding:"3px 0",color:"#c8a0bb",fontFamily:"monospace"}}><span>{"\u{1F6F0}\uFE0F Bombardiers"}</span><span>{botMil.ships.bombardier}</span></div>:null}
                      {botMil.ships.cuirasse>0?<div style={{display:"flex",justifyContent:"space-between",fontSize:9,padding:"3px 0",color:"#c8a0bb",fontFamily:"monospace"}}><span>{"\u2694\uFE0F Cuirasses"}</span><span>{botMil.ships.cuirasse}</span></div>:null}
                      {botMil.defenses.def_tourelle>0?<div style={{display:"flex",justifyContent:"space-between",fontSize:9,padding:"3px 0",color:"#c8a0bb",fontFamily:"monospace"}}><span>{"\u{1F52B} Tourelles"}</span><span>{botMil.defenses.def_tourelle}</span></div>:null}
                      {botMil.defenses.def_bouclier>0?<div style={{display:"flex",justifyContent:"space-between",fontSize:9,padding:"3px 0",color:"#c8a0bb",fontFamily:"monospace"}}><span>{"\u{1F6E1}\uFE0F Boucliers"}</span><span>{botMil.defenses.def_bouclier}</span></div>:null}
                      {botMil.defenses.def_missiles>0?<div style={{display:"flex",justifyContent:"space-between",fontSize:9,padding:"3px 0",color:"#c8a0bb",fontFamily:"monospace"}}><span>{"\u{1F680} Missiles"}</span><span>{botMil.defenses.def_missiles}</span></div>:null}
                      {botMil.defenses.def_forteresse>0?<div style={{display:"flex",justifyContent:"space-between",fontSize:9,padding:"3px 0",color:"#c8a0bb",fontFamily:"monospace"}}><span>{"\u{1F3F0} Forteresses"}</span><span>{botMil.defenses.def_forteresse}</span></div>:null}
                      <div style={{display:"flex",justifyContent:"space-between",fontSize:9,padding:"6px 0 3px",color:"#a06a8a",fontFamily:"monospace",borderTop:"1px dashed rgba(180,40,80,0.18)",marginTop:6,fontStyle:"italic"}}><span>Doctrine</span><span>{botMil.specialty==="warrior"?"Offensive":(botMil.specialty==="fortress"?"Defensive":"Equilibree")}</span></div>
                    </div>
                  ):(
                    <div style={{fontSize:9,color:"#a06a8a",fontFamily:"monospace",padding:"8px 0 2px",lineHeight:1.5,fontStyle:"italic"}}>{"\u{1F512} Details inconnus. Choisis la branche Exploration ou developpe une techno de scan pour des renseignements precis."}</div>
                  )}
                </div>
              );
            })()}

            {/* Attack button — visible only for OTHER players' planets (bots, not wild) */}
            {!isPlayer&&!planet.isWild&&natalState&&(function(){
              var availShips=(natalState.ships)||{};
              var totalShips=(availShips.chasseur||0)+(availShips.croiseur||0)+(availShips.bombardier||0)+(availShips.cuirasse||0);
              var canAttack=totalShips>0;
              return (
                <div style={{marginBottom:14}}>
                  <button onClick={function(){if(canAttack){setShowAttackModal(true);setAttackComp({chasseur:availShips.chasseur||0,cargo:availShips.cargo||0,croiseur:availShips.croiseur||0,bombardier:availShips.bombardier||0,cuirasse:availShips.cuirasse||0});}}} disabled={!canAttack} style={{width:"100%",padding:"12px 0",borderRadius:6,border:"1px solid "+(canAttack?"rgba(220,80,100,0.6)":"#403040"),cursor:canAttack?"pointer":"not-allowed",background:canAttack?"linear-gradient(180deg,rgba(180,40,60,0.7),rgba(120,20,40,0.7))":"rgba(40,20,30,0.5)",color:canAttack?"#ffe0e0":"#604050",fontFamily:"monospace",fontSize:11,letterSpacing:2.5,fontWeight:"bold",boxShadow:canAttack?"0 0 12px rgba(180,40,60,0.3)":"none"}}>
                    {canAttack?"\u2694\uFE0F LANCER UNE ATTAQUE":"\u{1F512} AUCUN VAISSEAU MILITAIRE"}
                  </button>
                </div>
              );
            })()}
          ...
          {viewTab==="hub"&&isPlayer&&state&&(<div style={{padding:"12px 12px",display:"flex",flexDirection:"column",gap:8}}>
              ...
              {(()=>{
                ...
                // Compute orbital stats
                const orbBld=state.orbitalBuildings||{};
                const defs=state.defenses||{};
                const chantierLv=orbBld.chantier_combat||0;
                const totalDefenses=(defs.def_tourelle||0)+(defs.def_bouclier||0)+(defs.def_missiles||0)+(defs.def_forteresse||0);
                const orbitalSub=chantierLv>0?"Chantier Niv "+chantierLv+" · "+totalDefenses+" defenses":(totalDefenses>0?totalDefenses+" defenses":"Aucune construction");
                const cards=[
                  ...
                  {id:"orbital",icon:"\u{1F6F0}\uFE0F",title:"Orbital",sub:orbitalSub,color:"#d87070"},
    ...
    {showAttackModal&&natalState&&<AttackPrepModal planet={planet} natalState={natalState} attackComp={attackComp} setAttackComp={setAttackComp} testMode={attackTestMode} setTestMode={setAttackTestMode} onClose={function(){setShowAttackModal(false);}} onLaunch={function(){setShowAttackModal(false);if(onLaunchAttack){onLaunchAttack(planet,attackComp,attackTestMode);}}}/>}
        ...
        <div style={{display:"flex",alignItems:"flex-start",gap:10}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
              ...
              {tech.branch&&<span style={{fontSize:7,padding:"1px 5px",borderRadius:3,background:branchColor+"22",border:"1px solid "+branchColor+"66",color:branchColor,letterSpacing:1,fontWeight:"bold"}}>{(branchMeta[tech.branch]&&(branchMeta[tech.branch]&&branchMeta[tech.branch].name?branchMeta[tech.branch].name:"?").toUpperCase())||(tech&&tech.branch?tech.branch:"?").toUpperCase()}</span>}
            </div>
  ...
  return(
    <div style={{padding:"6px 4px 14px"}}>
      <div style={{padding:"4px 8px 8px",marginBottom:8,borderBottom:"1px solid #0d2035",display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,flexWrap:"wrap"}}>
        ...
        {bm&&<div style={{fontSize:8,padding:"2px 6px",borderRadius:3,background:bm.color+"22",border:"1px solid "+bm.color+"66",color:bm.color,fontFamily:"monospace",letterSpacing:1,fontWeight:"bold"}}>{"BRANCHE "+(bm&&bm.name?bm.name:"?").toUpperCase()}</div>}
      </div>
      ...
      <div style={{padding:"0 8px"}}>
        ...
        {myBranchTechs.length>0&&<div>
          <div style={{fontSize:8,color:"#4a88cc",letterSpacing:2,margin:"14px 0 6px",fontFamily:"monospace"}}>{"━━ TECHNOLOGIES "+(bm?(bm&&bm.name?bm.name:"?").toUpperCase():"BRANCHE")+" ━━"}</div>
        ...
        {otherBranchTechs.length>0&&(()=>{
          ...
          return ["eco","mil","exp"].map(bk=>{
            ...
            return(
              <div key={bk}>
                <div style={{fontSize:8,color:meta.color,letterSpacing:2,margin:"14px 0 4px",fontFamily:"monospace",opacity:0.75}}>{"━━ BRANCHE "+(meta&&meta.name?meta.name:"?").toUpperCase()+" "+(isChosenOther?"(non choisie)":"(verrouillée)")+" ━━"}</div>
...
// OrbitalTab — UI for building orbital structures (Chantier de Combat + 4 defenses)
// Uses instant construction (no queue) for V1 simplicity
function OrbitalTab({state,onBuildOrbital}){
  return (
    <div style={{display:"flex",flexDirection:"column",gap:8}}>
      <div style={{padding:10,background:"rgba(80,30,30,0.3)",border:"1px solid #aa5566",borderRadius:8,marginBottom:8}}>
        <div style={{fontSize:11,color:"#ffaabb",fontFamily:"monospace",letterSpacing:1,fontWeight:"bold",marginBottom:4}}>{"\u{1F6F0}\uFE0F STRUCTURES ORBITALES"}</div>
        <div style={{fontSize:9,color:"#cc8899",lineHeight:1.4}}>{"Construis des installations en orbite : production militaire et defenses planetaires. Construction instantanee, paye et c\u2019est construit."}</div>
      </div>
      {ORBITAL_BUILDINGS.map(function(b){
        var isDefense=b.kind==="defense"||b.kind==="defense_consumable";
        var curLv=isDefense?((state.defenses&&state.defenses[b.id])||0):((state.orbitalBuildings&&state.orbitalBuildings[b.id])||0);
        var maxReached=isDefense?(curLv>=b.maxQuantity):(b.maxLevel&&curLv>=b.maxLevel);
        // Cost scales with level for production buildings, fixed for defenses
        var cost;
        if(isDefense){
          cost={metal:b.base.metal,crystal:b.base.crystal,deut:b.base.deut};
        } else {
        ...
        var canAfford=(state.resources&&state.resources.metal||0)>=cost.metal&&(state.resources&&state.resources.crystal||0)>=cost.crystal&&(state.resources&&state.resources.deut||0)>=cost.deut;
        var btnEnabled=!maxReached&&canAfford;
        var statusText="";
        if(maxReached)statusText=isDefense?("MAX : "+b.maxQuantity):("NIVEAU MAX");
        else if(!canAfford)statusText="RESSOURCES INSUFFISANTES";
        else statusText=isDefense?"CONSTRUIRE (+1)":"AMELIORER NIV "+(curLv+1);
        // Color based on kind
        var borderColor=b.kind==="production"?"#aa66ff":(b.kind==="defense_consumable"?"#cc8855":"#ee5566");
        return (
          <div key={b.id} style={{padding:10,background:"rgba(15,10,25,0.85)",border:"1px solid "+borderColor+"55",borderRadius:8}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
              <span style={{fontSize:24,filter:"drop-shadow(0 0 6px "+borderColor+"77)"}}>{b.icon}</span>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:11,color:"#fff",fontFamily:"monospace",fontWeight:"bold"}}>{b.name}</div>
                <div style={{fontSize:9,color:"#9abcd8",fontFamily:"monospace",marginTop:2}}>{isDefense?("Quantite : "+curLv+" / "+b.maxQuantity):("Niveau : "+curLv+(b.maxLevel?(" / "+b.maxLevel):""))}</div>
              </div>
            </div>
            <div style={{fontSize:8,color:"#7a8aa8",fontFamily:"monospace",lineHeight:1.4,marginBottom:8}}>{b.desc}</div>
            <div style={{display:"flex",gap:6,fontSize:8,color:"#aaccdd",fontFamily:"monospace",marginBottom:8}}>
              {cost.metal>0?<span>{"M:"+cost.metal}</span>:null}
              {cost.crystal>0?<span>{"C:"+cost.crystal}</span>:null}
              {cost.deut>0?<span>{"D:"+cost.deut}</span>:null}
            </div>
            <button onClick={function(){if(btnEnabled){AudioEngine.safeClick&&AudioEngine.safeClick();onBuildOrbital(b.id);}}} disabled={!btnEnabled} style={{width:"100%",padding:"8px 0",background:btnEnabled?("linear-gradient(135deg,"+borderColor+"99,"+borderColor+"55)"):"rgba(20,15,25,0.6)",border:"1px solid "+(btnEnabled?borderColor:"#333"),borderRadius:6,color:btnEnabled?"#fff":"#666",fontSize:10,fontFamily:"monospace",fontWeight:"bold",letterSpacing:1,cursor:btnEnabled?"pointer":"not-allowed"}}>{statusText}</button>
          </div>
        );
      })}
    </div>
...
function ShipsTab({state,planet,onBuildShip}){
  ...
  return(<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:6}}>
      {SHIPS.map(s=>{
        ...
        const bldOk=Object.entries(s.req).every(([k,v])=>((state.buildings[k]||0)>=v)||((state.orbitalBuildings&&state.orbitalBuildings[k]||0)>=v));
...
function AScreen({onAuth}){
  ...
  // ── TEMP RECOVERY: list all accounts with their emails + save sizes ──
  const recoveryData=(()=>{
    ...
    try{
      ...
      Object.values(allUsers).forEach(u=>{
        if(u.isBot)return;
        const saveKey="gx_state_"+u.id;
        const saveStr=localStorage.getItem(saveKey)||"";
        const sizeKB=(saveStr.length/1024).toFixed(1);
        out.push({email:u.email,name:u.name||"(sans nom)",id:u.id,sizeKB,linkedToUser:true});
      });
      // ALSO: scan ALL localStorage keys to find orphan saves not linked to any user
      const orphans=[];
      for(let i=0;i<localStorage.length;i++){
        const k=localStorage.key(i);
        if(k&&k.startsWith("gx_state_")){
          const userId=k.replace("gx_state_","");
          const linked=out.some(o=>o.id===userId);
          if(!linked){
            const saveStr=localStorage.getItem(k)||"";
            orphans.push({key:k,id:userId,sizeKB:(saveStr.length/1024).toFixed(1),linkedToUser:false});
          }
        }
      }
      // Combine + sort by size
      const all=out.concat(orphans);
      all.sort((a,b)=>parseFloat(b.sizeKB)-parseFloat(a.sizeKB));
      // Also collect ALL keys for full diagnostic
      const allKeys=[];
      for(let i=0;i<localStorage.length;i++){
        const k=localStorage.key(i);
        if(k){
          const v=localStorage.getItem(k)||"";
          allKeys.push({key:k,sizeKB:(v.length/1024).toFixed(1)});
        }
      }
      allKeys.sort((a,b)=>parseFloat(b.sizeKB)-parseFloat(a.sizeKB));
      return {accounts:all,allKeys:allKeys};
    }catch(e){return {accounts:[{email:"erreur: "+e.message,name:"",id:"",sizeKB:"0"}],allKeys:[]};}
  })();
  const go=async()=>{
  ...
  return(<div style={{width:"100vw",height:"100svh",minHeight:"100dvh",background:"#000005",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",position:"relative",paddingBottom:"env(safe-area-inset-bottom,0px)",paddingTop:"env(safe-area-inset-top,0px)"}}>
      ...
      <div style={{display:"flex",width:"100%",maxWidth:700,maxHeight:"90dvh",margin:"0 16px",background:"linear-gradient(160deg,#060f1c,#08141f)",border:"1px solid #0d2035",borderRadius:16,overflow:"hidden",boxShadow:"0 0 60px rgba(0,20,60,.8)",position:"relative",zIndex:1}}>
        ...
        <div style={{flex:1,padding:"16px 18px",overflow:"auto",WebkitOverflowScrolling:"touch"}}>
          ...
          <button onClick={()=>setShowRecovery(!showRecovery)} style={{width:"100%",padding:"7px 0",borderRadius:7,border:"1px solid #2a3a1a",background:"rgba(40,80,30,0.2)",color:"#88dd88",fontSize:10,cursor:"pointer",fontFamily:"monospace",marginBottom:8,letterSpacing:1.5,fontWeight:"bold"}}>{showRecovery?"× FERMER":"\u{1F50D} RÉCUPÉRER MON COMPTE"}</button>
          {showRecovery&&recoveryData&&<div style={{marginBottom:10,padding:10,background:"rgba(20,40,30,0.6)",border:"1px solid #2a4a2a",borderRadius:7,maxHeight:340,overflow:"auto",WebkitOverflowScrolling:"touch"}}>
            <div style={{fontSize:9,color:"#88dd88",fontFamily:"monospace",marginBottom:6,letterSpacing:1.2,fontWeight:"bold"}}>{"\u{1F4BE} COMPTES + SAVES ("+recoveryData.accounts.length+")"}</div>
            {recoveryData.accounts.length===0?<div style={{fontSize:10,color:"#aaccaa",fontFamily:"monospace"}}>Aucun compte trouvé.</div>:recoveryData.accounts.map((u,i)=>(
              <div key={i} style={{padding:"8px 10px",marginBottom:6,background:u.linkedToUser?"rgba(10,20,15,0.7)":"rgba(40,30,10,0.7)",border:"1px solid "+(u.linkedToUser?"#1a2a1a":"#4a3a1a"),borderRadius:5,fontFamily:"monospace",cursor:u.linkedToUser?"pointer":"default"}} onClick={u.linkedToUser?()=>{setEmail(u.email);setShowRecovery(false);}:null}>
                {u.linkedToUser?(
                  <div>
                    <div style={{fontSize:11,color:"#bbeebb",fontWeight:"bold",marginBottom:2}}>{u.email||"(sans email)"}</div>
                    <div style={{fontSize:9,color:"#88aa88"}}>Nom: {u.name} · Save: <span style={{color:parseFloat(u.sizeKB)>100?"#ffdd66":"#88aa88",fontWeight:parseFloat(u.sizeKB)>100?"bold":"normal"}}>{u.sizeKB} KB</span></div>
                    <div style={{fontSize:8,color:"#557755",marginTop:2}}>{"\u{1F446} Tap pour remplir l\u2019email"}</div>
                  </div>
                ):(
                  <div>
                    <div style={{fontSize:10,color:"#ffcc66",fontWeight:"bold",marginBottom:2}}>{"\u26A0\uFE0F SAVE ORPHELINE"}</div>
                    <div style={{fontSize:9,color:"#ddaa66"}}>Clé: {u.key}</div>
                    <div style={{fontSize:9,color:"#ddaa66"}}>ID: {u.id}</div>
                    <div style={{fontSize:9,color:"#ddaa66"}}>Taille: <span style={{color:parseFloat(u.sizeKB)>100?"#ffdd66":"#ddaa66",fontWeight:parseFloat(u.sizeKB)>100?"bold":"normal"}}>{u.sizeKB} KB</span></div>
                    <div style={{fontSize:8,color:"#aa8855",marginTop:2,fontStyle:"italic"}}>{"Save sans utilisateur associé"}</div>
                  </div>
                )}
              </div>
            ))}
            <div style={{fontSize:9,color:"#88dd88",fontFamily:"monospace",margin:"12px 0 6px",letterSpacing:1.2,fontWeight:"bold",borderTop:"1px solid #2a4a2a",paddingTop:8}}>{"\u{1F50D} TOUTES LES CLÉS LOCALSTORAGE ("+recoveryData.allKeys.length+")"}</div>
            {recoveryData.allKeys.map((k,i)=>(
              <div key={"k"+i} style={{padding:"4px 8px",marginBottom:3,background:"rgba(15,25,30,0.6)",borderLeft:"2px solid "+(parseFloat(k.sizeKB)>100?"#ffaa44":"#446677"),fontSize:9,color:"#88bbcc",fontFamily:"monospace"}}>
                <span style={{color:parseFloat(k.sizeKB)>100?"#ffdd66":"#88bbcc",fontWeight:parseFloat(k.sizeKB)>100?"bold":"normal"}}>{k.sizeKB} KB</span> — {k.key}
              </div>
            ))}
          </div>}
      ...
      {typeOrder.map(tid=>{
        ...
        return(<div key={tid} style={{marginBottom:14,padding:10,borderRadius:8,background:"rgba(8,16,30,0.5)",border:"1px solid rgba(60,100,140,0.25)"}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8,paddingBottom:6,borderBottom:"1px solid rgba(60,100,140,0.3)"}}>
            ...
            <span style={{fontSize:12,color:"#aaccee",letterSpacing:2,fontWeight:"bold"}}>{(tt&&tt.name||"?").toUpperCase()}</span>
      ...
      <div onClick={()=>onPick(pl.idx)} style={{
        ...
        <div style={{flex:1,minWidth:0}}>
          <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap",marginBottom:3}}>
            ...
            {pl.tag&&<span style={{fontSize:7,padding:"1px 5px",borderRadius:3,background:pl.tag.color+"22",border:"1px solid "+pl.tag.color+"66",color:pl.tag.color,letterSpacing:1,fontWeight:"bold"}}>{pl.tag.emoji+" "+(pl&&pl.tag&&pl.tag.label?pl.tag.label:"?").toUpperCase()}</span>}
          </div>
...
function Dash({user,onOut}){
  ...
  const[combatData,setCombatData]=useState(null); // {targetPlanet, attackerComp, defenderMilitary} when attacking
  const[battlePlan,setBattlePlan]=useState(null); // {targetPlanet, attackerComp, defenderMilitary, testMode} during battle plan phase
  ...
  return(<div style={{width:"100vw",height:"100svh",minHeight:"100dvh",maxHeight:"100dvh",background:"#000005",display:"flex",flexDirection:"column",overflow:"hidden",paddingBottom:"env(safe-area-inset-bottom,0px)"}}>
      ...
      {battlePlan&&<BattlePlanScreen plan={battlePlan} onCancel={()=>setBattlePlan(null)} onEngage={function(placements){setCombatData({targetPlanet:battlePlan.targetPlanet,attackerComp:battlePlan.attackerComp,defenderMilitary:battlePlan.defenderMilitary,testMode:battlePlan.testMode,placements:placements});setBattlePlan(null);}}/>}
      {combatData&&threeOk&&<CombatScene targetPlanet={combatData.targetPlanet} attackerComp={combatData.attackerComp} defenderMilitary={combatData.defenderMilitary} testMode={combatData.testMode} placements={combatData.placements} onClose={()=>setCombatData(null)} onResolve={(result)=>{
        // Apply real losses only if NOT in test mode
        if(!combatData.testMode&&result&&user&&user.id){
          try{
            var key="gx_state_"+user.id;
            var s=JSON.parse(localStorage.getItem(key));
            if(s){
              s.ships=s.ships||{};
              // Subtract attacker losses from player's ship counts
              if(result.attackerLosses){
                for(var k in result.attackerLosses){
                  if(result.attackerLosses[k]>0){
                    s.ships[k]=Math.max(0,(s.ships[k]||0)-result.attackerLosses[k]);
                  }
                }
              }
              s.lastTick=Date.now();
        ...
        setCombatData(null);
        // Reload to refresh displayed state if losses were applied
        if(!combatData.testMode){setTimeout(function(){window.location.reload();},100);}
      }}/>}
      ...
      {viewer&&(()=>{
        ...
        return(<Viewer3D planet={viewer} allPlanets={planets} onClose={()=>setViewer(null)} isPlayer={isMyPlanet} owner={(lp.find(p=>p.idx===viewer.i)&&lp.find(p=>p.idx===viewer.i).name)} state={viewerState} onStateChange={viewerStateChange} onManage={null} userId={user&&user.id} natalState={natSt} onColonize={viewer.isWild?(()=>handleColonize(viewer.i)):null} alreadyColonized={isMyColony} onLaunchAttack={function(target,comp,testMode){var pq=(natSt&&natSt.buildings&&natSt.buildings.qg)||1;var enemy=generateBotMilitary(pq,target.i*7+1);setBattlePlan({targetPlanet:target,attackerComp:comp,defenderMilitary:enemy,testMode:testMode,playerBranch:(natSt&&natSt.missionFlags&&natSt.missionFlags.chosenBranch)||null});setViewer(null);}}/>);
      })()}
...
function AudioControls({onLogout}){
  ...
  // Cheat handler — extracted as named function to avoid JSX inline issues
  const doCheat=()=>{
    if(!window.confirm("MODE CHEAT ?\n\u2022 999K ressources\n\u2022 Tous batiments+vaisseaux+defenses au max"))return;
    try{
      const session=JSON.parse(localStorage.getItem("gx_session"));
      if(!session||!session.id){alert("Session introuvable.");return;}
      const key="gx_state_"+session.id;
      const s=JSON.parse(localStorage.getItem(key));
      if(!s){alert("Save introuvable.");return;}
      s.resources=s.resources||{};s.resources.metal=999000;s.resources.crystal=999000;s.resources.deuterium=999000;s.resources.energy=10000;
      s.buildings=s.buildings||{};
      s.buildings.qg=Math.max(s.buildings.qg||1,8);
      s.buildings.metal=Math.max(s.buildings.metal||0,12);
      s.buildings.crystal=Math.max(s.buildings.crystal||0,12);
      s.buildings.deut=Math.max(s.buildings.deut||0,10);
      s.buildings.solaire=Math.max(s.buildings.solaire||0,10);
      s.buildings.fusion=Math.max(s.buildings.fusion||0,8);
      s.buildings.depot=Math.max(s.buildings.depot||0,10);
      s.buildings.hangar=Math.max(s.buildings.hangar||0,8);
      s.buildings.radar=Math.max(s.buildings.radar||0,5);
      s.buildings.lab=Math.max(s.buildings.lab||0,5);
      s.orbitalBuildings=s.orbitalBuildings||{};
      s.orbitalBuildings.chantier_combat=Math.max(s.orbitalBuildings.chantier_combat||0,5);
      s.ships=s.ships||{};
      s.ships.chasseur=Math.max(s.ships.chasseur||0,80);
      s.ships.cargo=Math.max(s.ships.cargo||0,30);
      s.ships.croiseur=Math.max(s.ships.croiseur||0,20);
      s.ships.bombardier=Math.max(s.ships.bombardier||0,10);
      s.ships.cuirasse=Math.max(s.ships.cuirasse||0,5);
      s.ships.colonisateur=Math.max(s.ships.colonisateur||0,3);
      s.defenses=s.defenses||{};
      s.defenses.def_tourelle=Math.max(s.defenses.def_tourelle||0,20);
      s.defenses.def_bouclier=Math.max(s.defenses.def_bouclier||0,5);
      s.defenses.def_missiles=Math.max(s.defenses.def_missiles||0,10);
      s.defenses.def_forteresse=Math.max(s.defenses.def_forteresse||0,2);
      s.missionFlags=s.missionFlags||{};
      if(!s.missionFlags.chosenBranch)s.missionFlags.chosenBranch="mil";
      s.lastTick=Date.now();
      localStorage.setItem(key,JSON.stringify(s));
      alert("MODE CHEAT ACTIVE !");
      setTimeout(function(){window.location.reload();},500);
    }catch(e){alert("Erreur : "+e.message);}
  };
  
  return(<div style={_rs4}>
      <button onClick={doCheat} style={{padding:"7px 12px",borderRadius:6,border:"2px solid #ffd700",background:"#b8860b",color:"#fff",fontSize:11,fontFamily:"monospace",fontWeight:"bold",letterSpacing:1,cursor:"pointer",boxShadow:"0 0 12px rgba(255,215,0,0.5)"}}>{"\u{1F4B0} CHEAT"}</button>
      {showPanel&&(<div onClick={(e)=>{if(e.target===e.currentTarget)setShowPanel(false);}} style={{position:"fixed",top:0,left:0,right:0,bottom:0,zIndex:8999,background:"rgba(0,0,0,0.3)"}}/>)}
      {showPanel&&(
        <div style={{background:"rgba(4,10,24,.98)",border:"1px solid #1a3a60",borderRadius:14,padding:"14px 16px",width:220,boxShadow:"0 0 30px rgba(0,10,50,.9)",maxHeight:"80vh",overflow:"auto",WebkitOverflowScrolling:"touch"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
            <div style={{fontSize:9,color:"#3a6a9a",letterSpacing:2,fontFamily:"monospace"}}>{"AUDIO"}</div>
            <button onClick={()=>setShowPanel(false)} style={{background:"transparent",border:"1px solid #3a6a9a",borderRadius:5,color:"#5588aa",fontSize:11,padding:"3px 9px",cursor:"pointer",fontFamily:"monospace",lineHeight:1,fontWeight:"bold"}}>×</button>
          </div>
          ...
          <div style={{marginTop:14,paddingTop:12,borderTop:"1px dashed rgba(80,80,160,0.3)"}}>
            <button onClick={function(){
              if(!confirm("Reset complet de tes vaisseaux a 0 (cargo/chasseur/croiseur). Tes ressources et batiments sont preserves. Le hub devrait se debloquer. Continuer ?"))return;
              try{
                var keys=[];
                for(var ki=0;ki<localStorage.length;ki++){var kk=localStorage.key(ki);if(kk&&kk.indexOf("gx_state_")===0)keys.push(kk);}
                var fixedTotal=0;
                for(var ix=0;ix<keys.length;ix++){
                  ...
                  try{
                    ...
                    // Fix continents : limit cells to ~200 max (small continent baseline)
                    if(s.continents&&Array.isArray(s.continents)){
                      for(var ci=0;ci<s.continents.length;ci++){
                        var cont=s.continents[ci];
                        if(cont&&cont.cells&&Array.isArray(cont.cells)&&cont.cells.length>500){
                          // Sample down: keep cells uniformly distributed
                          var step=Math.ceil(cont.cells.length/200);
                          var sampled=[];
                          for(var ck=0;ck<cont.cells.length;ck+=step)sampled.push(cont.cells[ck]);
                          cont.cells=sampled;
                          // Remove _cluster property if present
                          for(var ck2=0;ck2<cont.cells.length;ck2++){
                            if(cont.cells[ck2]&&cont.cells[ck2]._cluster!==undefined){delete cont.cells[ck2]._cluster;}
                          }
                          fixedTotal++;
                        }
                      }
                    ...
                    if(s.researchQueue&&(typeof s.researchQueue!=="object"||typeof s.researchQueue.finishAt!=="number"))s.researchQueue=null;
                    // AGGRESSIVE: Reset all ships to 0 to debug the hub crash
                    s.ships={chasseur:0,cargo:0,croiseur:0};
                    // Also wipe the hangar queue if any
                    if(s.shipQueue)s.shipQueue=null;
                    if(s.shipsBuildQueue)s.shipsBuildQueue=null;
                    if(s.buildShipQueue)s.buildShipQueue=null;
                    // Delete any potential corrupted ship-related fields
                    delete s.shipBuilding;
                    delete s.buildingShip;
                    if(s.fleets&&Array.isArray(s.fleets)){
                      s.fleets=s.fleets.filter(function(f){return f&&typeof f==="object"&&f.id&&typeof f.arriveAt==="number"&&typeof f.departAt==="number"&&!isNaN(f.arriveAt);});
                    } else { s.fleets=[]; }
                    if(s.anomalies&&Array.isArray(s.anomalies)){
                      s.anomalies=s.anomalies.filter(function(a){return a&&a.id&&a.pos;});
                    } else { s.anomalies=[]; }
                    s.anomalyNotifs=[];
                    s.activeBoosts=[];
                    s.lastTick=Date.now();
                ...
                alert("Reparation appliquee ! "+fixedTotal+" continent(s) reduit(s). L\u2019app va recharger.");
                setTimeout(function(){window.location.reload();},500);
              }catch(e){alert("Erreur reparation : "+e.message);}
            }} style={{width:"100%",padding:"8px 0",borderRadius:6,border:"1px solid rgba(120,80,180,0.5)",background:"rgba(40,25,80,0.5)",color:"#aa88dd",fontSize:10,fontFamily:"monospace",letterSpacing:1.5,fontWeight:"bold",cursor:"pointer"}}>
              {"\u{1F527} RESET VAISSEAUX (debug hub)"}
            </button>
          </div>
          {/* ── DEBUG TOGGLE: collapsed by default to keep panel compact ── */}
          <button onClick={()=>setShowDebug(!showDebug)} style={{width:"100%",padding:"6px 0",borderRadius:6,border:"1px solid rgba(200,160,60,0.4)",background:"rgba(60,40,10,0.3)",color:"#ddbb55",fontSize:10,fontFamily:"monospace",letterSpacing:1.5,fontWeight:"bold",cursor:"pointer",marginTop:10,marginBottom:showDebug?6:0}}>{showDebug?"\u25BC OUTILS DEBUG":"\u25B6 OUTILS DEBUG"}</button>
          {showDebug&&(<div style={{padding:8,background:"rgba(60,40,10,0.3)",border:"1px solid rgba(200,160,60,0.4)",borderRadius:6}}>
            <div style={{fontSize:9,color:"#ddbb55",fontFamily:"monospace",letterSpacing:1.5,marginBottom:6,fontWeight:"bold",textAlign:"center"}}>{"\u{1F527} OUTILS DEBUG"}</div>
            <button onClick={function(){
              if(!window.confirm("Activer le MODE CHEAT ? Tu vas recevoir : 999K de chaque ressource, +50 vaisseaux de chaque type, tous les batiments au niv 5+, toutes les defenses au max."))return;
              try{
                var session=JSON.parse(localStorage.getItem("gx_session"));
                if(!session||!session.id){alert("Session introuvable.");return;}
                var key="gx_state_"+session.id;
                var s=JSON.parse(localStorage.getItem(key));
                if(!s){alert("Save introuvable.");return;}
                // RESOURCES — fill them up
                s.resources=s.resources||{};
                s.resources.metal=999000;
                s.resources.crystal=999000;
                s.resources.deuterium=999000;
                s.resources.energy=10000;
                // BUILDINGS — set to high levels
                s.buildings=s.buildings||{};
                s.buildings.qg=Math.max(s.buildings.qg||1,8);
                s.buildings.metal=Math.max(s.buildings.metal||0,12);
                s.buildings.crystal=Math.max(s.buildings.crystal||0,12);
                ...
                s.defenses.def_missiles=Math.max(s.defenses.def_missiles||0,10);
                s.defenses.def_forteresse=Math.max(s.defenses.def_forteresse||0,2);
                // MISSIONS — unlock branch if not chosen yet (give Mil for combat tests)
                s.missionFlags=s.missionFlags||{};
                if(!s.missionFlags.chosenBranch)s.missionFlags.chosenBranch="mil";
                s.lastTick=Date.now();
                localStorage.setItem(key,JSON.stringify(s));
                alert("\u{1F389} MODE CHEAT ACTIF !\nL\u2019app va recharger.");
                setTimeout(function(){window.location.reload();},500);
              }catch(e){alert("Erreur cheat : "+e.message);}
            }} style={{width:"100%",padding:"10px 0",borderRadius:6,border:"1px solid rgba(220,180,80,0.6)",background:"linear-gradient(180deg,rgba(180,140,40,0.5),rgba(120,90,20,0.5))",color:"#ffe88a",fontSize:11,fontFamily:"monospace",letterSpacing:1.5,fontWeight:"bold",cursor:"pointer",marginBottom:6}}>
              {"\u{1F4B0} MODE CHEAT (full starter)"}
            </button>
            <button onClick={function(){
              if(!window.confirm("Te donner +500K metal, cristal, deut ?"))return;
              try{
                var session=JSON.parse(localStorage.getItem("gx_session"));
                if(!session||!session.id){alert("Session introuvable.");return;}
                var key="gx_state_"+session.id;
                ...
                if(!s){alert("Save introuvable.");return;}
                s.resources=s.resources||{};
                s.resources.metal=(s.resources.metal||0)+500000;
                s.resources.crystal=(s.resources.crystal||0)+500000;
                s.resources.deuterium=(s.resources.deuterium||0)+500000;
                s.lastTick=Date.now();
                localStorage.setItem(key,JSON.stringify(s));
                alert("+500K resources !");
                setTimeout(function(){window.location.reload();},500);
              }catch(e){alert("Erreur : "+e.message);}
            }} style={{width:"100%",padding:"7px 0",borderRadius:6,border:"1px solid rgba(180,160,60,0.5)",background:"rgba(80,60,20,0.4)",color:"#ddcc88",fontSize:10,fontFamily:"monospace",letterSpacing:1.2,fontWeight:"bold",cursor:"pointer",marginBottom:6}}>
              {"\u{1F4B5} +500K RESSOURCES"}
            </button>
            <button onClick={function(){
              if(!window.confirm("Te donner +50 vaisseaux militaires (chasseurs/croiseurs/bombardiers) ?"))return;
              try{
                var session=JSON.parse(localStorage.getItem("gx_session"));
                if(!session||!session.id){alert("Session introuvable.");return;}
                var key="gx_state_"+session.id;
                ...
                if(!s){alert("Save introuvable.");return;}
                s.ships=s.ships||{};
                s.ships.chasseur=(s.ships.chasseur||0)+50;
                s.ships.croiseur=(s.ships.croiseur||0)+15;
                s.ships.bombardier=(s.ships.bombardier||0)+8;
                s.ships.cuirasse=(s.ships.cuirasse||0)+3;
                s.lastTick=Date.now();
                localStorage.setItem(key,JSON.stringify(s));
                alert("Vaisseaux ajoutes !");
                setTimeout(function(){window.location.reload();},500);
              }catch(e){alert("Erreur : "+e.message);}
            }} style={{width:"100%",padding:"7px 0",borderRadius:6,border:"1px solid rgba(80,160,200,0.5)",background:"rgba(20,40,80,0.4)",color:"#88ccdd",fontSize:10,fontFamily:"monospace",letterSpacing:1.2,fontWeight:"bold",cursor:"pointer"}}>
              {"\u{1F6F8} +50 VAISSEAUX MILITAIRES"}
            </button>
          </div>)}
...
// Patch THREE.js to tolerate NaN values in BufferGeometry (prevents white screen of death)
if(typeof window!=="undefined"&&!window._gxThreePatched){
  var checkAndPatchThree=function(){
    if(window.THREE&&window.THREE.BufferAttribute&&!window._gxThreePatched){
      window._gxThreePatched=true;
      var origSetAttr=window.THREE.BufferGeometry.prototype.setAttribute;
      window.THREE.BufferGeometry.prototype.setAttribute=function(name,attr){
        if(attr&&attr.array){
          var arr=attr.array;
          var hasNaN=false;
          ...
          if(hasNaN){
            console.warn("[GALAXIA] BufferGeometry NaN detected and cleaned in attribute:",name);
          }
        }
        return origSetAttr.call(this,name,attr);
      };
      // Also patch computeBoundingSphere to be defensive
      var origCBS=window.THREE.BufferGeometry.prototype.computeBoundingSphere;
      window.THREE.BufferGeometry.prototype.computeBoundingSphere=function(){
        try{
          var pos=this.attributes.position;
          if(pos&&pos.array){
            var arr=pos.array;
            for(var i=0;i<arr.length;i++){
              if(isNaN(arr[i])||!isFinite(arr[i]))arr[i]=0;
            }
          }
          return origCBS.call(this);
        }catch(e){
          // If still fails, set a default bounding sphere
          if(!this.boundingSphere)this.boundingSphere=new window.THREE.Sphere(new window.THREE.Vector3(0,0,0),1);
          return this.boundingSphere;
        }
      };
      console.log("[GALAXIA] THREE.js patched to tolerate NaN values");
    }
  };
  // Try patching now and also on load
  checkAndPatchThree();
  ...
  var patchInterval=setInterval(function(){
    checkAndPatchThree();
    patchTries++;
    if(window._gxThreePatched||patchTries>50)clearInterval(patchInterval);
  },100);
}

// Global error display — shows ANY JS error on screen as a big red banner
// Helps debug the white screen of death
if(typeof window!=="undefined"&&!window._gxErrorInstalled){
  window._gxErrorInstalled=true;
  var _gxErrors=[];
  var showErrorBanner=function(msg){
    var banner=document.getElementById("gxErrorBanner");
    if(!banner){
      banner=document.createElement("div");
      banner.id="gxErrorBanner";
      banner.style.cssText="position:fixed;top:0;left:0;right:0;background:#cc1133;color:#fff;padding:14px;font-family:monospace;font-size:11px;z-index:99999;max-height:50vh;overflow:auto;line-height:1.4;border-bottom:3px solid #ff5577;cursor:pointer;";
      banner.onclick=function(){
        var txt="GALAXIA ERREURS:\n\n"+_gxErrors.join("\n\n=====\n\n");
        try{
          ...
          alert("Erreurs copiees dans le presse-papier !");
        }catch(e){alert("Echec copie");}
      };
      document.body.appendChild(banner);
    }
    banner.innerHTML="<b>\u26A0 ERREUR DETECTEE (tap pour copier)</b><br><br>"+_gxErrors.map(function(e){return e.replace(/</g,"&lt;");}).join("<br><br>=====<br><br>");
  };
  // Helper: ignore errors that don't originate from our app (Claude environment, ads, etc.)
  var isOwnError=function(stack,filename){
    var s=(stack||"")+(filename||"");
    if(s.indexOf("claudeusercontent.com")>-1)return false;
    if(s.indexOf("claude.ai")>-1)return false;
    if(s.indexOf("anthropic")>-1)return false;
    if(s.indexOf("vercel-insights")>-1)return false;
    return true;
  };
  window.addEventListener("error",function(e){
    if(!isOwnError(e.error&&e.error.stack,e.filename))return;
    var msg="["+new Date().toLocaleTimeString()+"] "+(e.message||"?")+"\n  at "+(e.filename||"?")+":"+(e.lineno||"?")+":"+(e.colno||"?");
    if(e.error&&e.error.stack)msg+="\n  STACK: "+String(e.error.stack).substring(0,400);
    _gxErrors.push(msg);
    showErrorBanner(msg);
  });
  window.addEventListener("unhandledrejection",function(e){
    if(!isOwnError(e.reason&&e.reason.stack,""))return;
    var msg="["+new Date().toLocaleTimeString()+"] PROMISE: "+String(e.reason);
    if(e.reason&&e.reason.stack)msg+="\n  STACK: "+String(e.reason.stack).substring(0,400);
    _gxErrors.push(msg);
    showErrorBanner(msg);
  });
  // Also patch console.error to capture React errors — but ignore Three.js warnings
  var origConsoleError=console.error;
  console.error=function(){
    var args=Array.prototype.slice.call(arguments);
    var firstArg=args[0]?String(args[0]):"";
    // Ignore Three.js cosmetic warnings (NaN bounding sphere is now patched)
    if(firstArg.indexOf("THREE.")===0||firstArg.indexOf("computeBoundingSphere")>-1){
      return origConsoleError.apply(console,args);
    }
    // Ignore some standard React warnings that aren't crashes
    if(firstArg.indexOf("Warning: ")===0&&firstArg.indexOf("Error")===-1&&firstArg.indexOf("Cannot")===-1){
      return origConsoleError.apply(console,args);
    }
    var msg="["+new Date().toLocaleTimeString()+"] CONSOLE: "+args.map(function(a){
      if(a&&a.message)return a.message+(a.stack?"\n  STACK: "+String(a.stack).substring(0,400):"");
      try{return typeof a==="string"?a:JSON.stringify(a).substring(0,200);}catch(e){return String(a);}
    }).join(" ");
    _gxErrors.push(msg);
    showErrorBanner(msg);
    return origConsoleError.apply(console,args);
