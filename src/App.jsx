...
function CombatScene({targetPlanet,attackerComp,defenderMilitary,onClose,onResolve}){
  ...
  useEffect(function(){
    ...
    var cam=new T.PerspectiveCamera(60,W/H,0.1,3000);
    // 45-degree top-down view, looking from above and slightly behind
    // Battlefield extends from z=-150 (enemy/planet) to z=+80 (player ships)
    ...
    var planetGeo=new T.SphereGeometry(60,32,24);
    ...
    planetMesh.position.set(0,-30,-180);
    scene.add(planetMesh);
    
    // Atmosphere glow around planet
    var atmGeo=new T.SphereGeometry(63,32,24);
    ...
    function makeShip(type,color){
      ...
      var emissive=color;
      if(type==="chasseur"){
        // Small triangular fighter
        var bodyGeo=new T.ConeGeometry(1.2,3.5,4);
        var body=new T.Mesh(bodyGeo,new T.MeshPhongMaterial({color:color,emissive:emissive,emissiveIntensity:0.4,shininess:30}));
        body.rotation.x=Math.PI/2;
        g.add(body);
        // Glowing engine
        var eng=new T.Mesh(new T.SphereGeometry(0.5,8,6),new T.MeshBasicMaterial({color:0x88ccff,transparent:true,opacity:0.9}));
        eng.position.set(0,0,1.8);
        g.add(eng);
      } else if(type==="cargo"){
        // Boxy transport
        var c=new T.Mesh(new T.BoxGeometry(2,1.6,3.5),new T.MeshPhongMaterial({color:color,emissive:emissive,emissiveIntensity:0.2,shininess:10}));
        g.add(c);
      } else if(type==="croiseur"){
        // Medium cruiser: cylinder + small wings
        var cBody=new T.Mesh(new T.CylinderGeometry(1.3,1.3,5,8),new T.MeshPhongMaterial({color:color,emissive:emissive,emissiveIntensity:0.3,shininess:20}));
        cBody.rotation.x=Math.PI/2;
        g.add(cBody);
        var wing1=new T.Mesh(new T.BoxGeometry(4,0.3,1.2),new T.MeshPhongMaterial({color:color,emissive:emissive,emissiveIntensity:0.25}));
        wing1.position.set(0,0,0);
        g.add(wing1);
        // Engines
        var ce1=new T.Mesh(new T.SphereGeometry(0.6,8,6),new T.MeshBasicMaterial({color:0x88ccff,transparent:true,opacity:0.9}));
        ce1.position.set(0,0,2.6);
        g.add(ce1);
      } else if(type==="bombardier"){
        // Disc-shaped bomber
        var bodyB=new T.Mesh(new T.CylinderGeometry(2.2,1.6,1.5,8),new T.MeshPhongMaterial({color:color,emissive:emissive,emissiveIntensity:0.35,shininess:20}));
        g.add(bodyB);
        // Heavy weapon underneath
        var wpn=new T.Mesh(new T.BoxGeometry(0.8,0.4,2),new T.MeshPhongMaterial({color:0x223344,emissive:0x111111}));
        wpn.position.set(0,-0.8,0);
        g.add(wpn);
      } else if(type==="cuirasse"){
        // Massive battleship
        var hull=new T.Mesh(new T.BoxGeometry(4,2.2,8),new T.MeshPhongMaterial({color:color,emissive:emissive,emissiveIntensity:0.3,shininess:25}));
        g.add(hull);
        // Tower
        var tower=new T.Mesh(new T.BoxGeometry(2.5,1.5,3),new T.MeshPhongMaterial({color:color,emissive:emissive,emissiveIntensity:0.2}));
        tower.position.set(0,1.4,-1);
        g.add(tower);
        // Engines (3 thrusters)
        for(var ei=-1;ei<=1;ei++){
          var en=new T.Mesh(new T.SphereGeometry(0.8,8,6),new T.MeshBasicMaterial({color:0xffaa44,transparent:true,opacity:0.9}));
          en.position.set(ei*1.4,0,4.2);
          g.add(en);
        }
      } else {
        // Fallback
        var fb=new T.Mesh(new T.BoxGeometry(1,1,2),new T.MeshPhongMaterial({color:color,emissive:emissive,emissiveIntensity:0.3}));
        g.add(fb);
      }
      return g;
    }
    ...
    function makeDefense(type){
      ...
      if(type==="def_tourelle"){
        // Laser turret: short cylinder base + barrel
        var base=new T.Mesh(new T.CylinderGeometry(1.2,1.5,0.8,8),new T.MeshPhongMaterial({color:0x884444,emissive:0x441111,emissiveIntensity:0.4}));
        g.add(base);
        var barrel=new T.Mesh(new T.CylinderGeometry(0.3,0.3,2,6),new T.MeshPhongMaterial({color:0x664444,emissive:0x331111,emissiveIntensity:0.3}));
        barrel.rotation.x=Math.PI/4;
        barrel.position.set(0,0.7,0.6);
        g.add(barrel);
      } else if(type==="def_bouclier"){
        // Translucent dome
        var dome=new T.Mesh(new T.SphereGeometry(3,16,8,0,Math.PI*2,0,Math.PI/2),new T.MeshBasicMaterial({color:0x4488ff,transparent:true,opacity:0.25,side:T.DoubleSide}));
        g.add(dome);
      } else if(type==="def_missiles"){
        // Pointy missile launcher
        var rocket=new T.Mesh(new T.ConeGeometry(0.5,3,4),new T.MeshPhongMaterial({color:0xcc6644,emissive:0x441100,emissiveIntensity:0.4}));
        rocket.rotation.x=Math.PI/2.5;
        g.add(rocket);
      } else if(type==="def_forteresse"){
        // Massive fortress
        var f=new T.Mesh(new T.CylinderGeometry(2.5,3,3,6),new T.MeshPhongMaterial({color:0x553333,emissive:0x221100,emissiveIntensity:0.4,shininess:5}));
        g.add(f);
        var top=new T.Mesh(new T.BoxGeometry(2.5,1.2,2.5),new T.MeshPhongMaterial({color:0x664444}));
    ...
    var playerZ=80; // closer to camera
    var rowZ=playerZ;
    var idx=0;
    ...
    function animate(){
      ...
      cam.lookAt(0,0,0);
      // Subtle ship bobbing (anti-gravity feel)
      for(var si=0;si<attackerShips.length;si++){
        var s=attackerShips[si].mesh;
        s.position.y=Math.sin(t*1.2+si*0.7)*1.2;
      }
      for(var di=0;di<defenderShips.length;di++){
        var d=defenderShips[di].mesh;
        d.position.y=Math.sin(t*1.0+di*0.5+1.5)*1.2;
      }
      renderer.render(scene,cam);
    }
    animate();
  ...
  return (
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"#000",zIndex:300,overflow:"hidden"}}>
      ...
      <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"12px 14px",paddingBottom:"calc(12px + env(safe-area-inset-bottom,0px))",background:"linear-gradient(0deg,rgba(8,4,16,0.85),transparent)"}}>
        <div style={{display:"flex",justifyContent:"space-around",gap:8}}>
          <div style={{flex:1,padding:"8px 10px",background:"rgba(20,40,80,0.7)",border:"1px solid #4488cc",borderRadius:6}}>
            <div style={{fontSize:8,color:"#88ccff",fontFamily:"monospace",letterSpacing:1.5,marginBottom:3}}>{"\u{1F535} VOTRE FLOTTE"}</div>
            <div style={{fontSize:14,color:"#fff",fontFamily:"monospace",fontWeight:"bold"}}>{totalAtt} vaisseaux</div>
          </div>
          <div style={{flex:1,padding:"8px 10px",background:"rgba(80,20,30,0.7)",border:"1px solid #cc4466",borderRadius:6}}>
            <div style={{fontSize:8,color:"#ff88aa",fontFamily:"monospace",letterSpacing:1.5,marginBottom:3}}>{"\u{1F534} ENNEMI"}</div>
            <div style={{fontSize:14,color:"#fff",fontFamily:"monospace",fontWeight:"bold"}}>{totalDef} vaisseaux · {totalDefStruct} defenses</div>
          </div>
        </div>
        <div style={{marginTop:10,padding:8,background:"rgba(20,15,30,0.8)",border:"1px solid rgba(180,150,80,0.4)",borderRadius:5,textAlign:"center",fontSize:10,color:"#ddcc88",fontFamily:"monospace",letterSpacing:1.5}}>{"\u23F3 Phase 3.2 : Mise en place du theatre. Vaisseaux a venir en 3.3."}</div>
      </div>
    </div>
  );
}
