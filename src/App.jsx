...
// ──────────────────────────────────────────────────────
var ANOMALY_TYPES={
  asteroids:{id:"asteroids",icon:"\u{1FAA8}",name:"Champ d asteroides",color:[200,180,160],glowColor:[255,230,180],rarityWeight:50,lifetimeMin:2,lifetimeMax:4,description:"Un dense champ de roches metalliques."},
  gascloud:{id:"gascloud",icon:"\u{1F32B}\uFE0F",name:"Nuage de gaz",color:[100,255,180],glowColor:[150,255,200],rarityWeight:25,lifetimeMin:2,lifetimeMax:4,description:"Un nuage de gaz volatils."},
  wreck:{id:"wreck",icon:"\u{1F6F0}\uFE0F",name:"Epave ancienne",color:[140,140,180],glowColor:[200,180,255],rarityWeight:15,lifetimeMin:3,lifetimeMax:4,description:"Vestige d un vaisseau ancien."},
  storm:{id:"storm",icon:"\u26A1",name:"Tempete energetique",color:[120,180,255],glowColor:[180,220,255],rarityWeight:10,lifetimeMin:2,lifetimeMax:3,description:"Tempete de plasma."}
};
...
// ─────────────────────────────────────────────────────────────────
function ExplorationMode({planets,playerIdx,gSeed,anomalies,onClose}){
  const mountRef=useRef();
  ...
  const [hud,setHud]=useState({speed:0,nearest:"",dist:0,anomNear:null,anomDist:0});
  ...
  useEffect(()=>{
    ...
    const animate=()=>{
      ...
      if(hudT>0.25){
        ...
        // Find nearest anomaly within detection radius
        ...
        setHud({speed:Math.round(c.speed*10)/10,nearest:bestP?bestP.name:"",dist:Math.round(bestDist),anomNear:nearAnom,anomDist:Math.round(nearAnomDist)});
      }
...
function Dash({user,onOut}){
  ...
  return(<div style={{width:"100vw",height:"100svh",minHeight:"100dvh",maxHeight:"100dvh",background:"#000005",display:"flex",flexDirection:"column",overflow:"hidden",paddingBottom:"env(safe-area-inset-bottom,0px)"}}>
      ...
      {explorationMode&&threeOk&&<ExplorationMode planets={planets} playerIdx={idx} gSeed={gSeed} anomalies={myState&&myState.anomalies?myState.anomalies:[]} onClose={()=>setExplorationMode(false)}/>}
