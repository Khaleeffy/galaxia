...
// ─────────────────────────────────────────────────────────────────
// ── BattlePlanScreen — 2D tactical map for pre-combat ship placement ──
// Cosmic style: dark gradient background, glowing enemy zones, planet at top, large deployment area for player
function BattlePlanScreen({plan,onCancel,onEngage}){
  // Generate approximate enemy zones based on defender military
  // Position: planet at top, enemy zones just under it, front line at ~48%, deployment zone takes the lower half
  var enemyMilitary=plan.defenderMilitary;
  var hasIntel=plan.playerBranch==="exp"; // Exp branch sees precise data

  // Compute total enemy ships for zone count
  ...
  // Player's available ships
  var avail=plan.attackerComp;
  var totalPlayerShips=(avail.chasseur||0)+(avail.cargo||0)+(avail.croiseur||0)+(avail.bombardier||0)+(avail.cuirasse||0);
  ...
  return (
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"radial-gradient(ellipse at top,rgba(20,15,40,0.98) 0%,rgba(5,3,15,0.98) 80%)",zIndex:300,display:"flex",flexDirection:"column",overflow:"hidden",paddingTop:"env(safe-area-inset-top,0px)",paddingBottom:"env(safe-area-inset-bottom,0px)"}}>
      ...
      <style>{"@keyframes battlePulse { 0%,100% { opacity: 0.55; } 50% { opacity: 0.95; } } @keyframes dropPulse { 0%,100% { opacity: 0.5; } 50% { opacity: 0.85; } }"}</style>

      {/* Top HUD: title + cancel button (compact) */}
      <div style={{padding:"8px 12px",display:"flex",alignItems:"center",justifyContent:"space-between",background:"linear-gradient(180deg,rgba(15,10,30,0.85),transparent)",flexShrink:0}}>
        <div>
          ...
          <div style={{fontSize:9,color:"#7a99bb",fontFamily:"monospace",marginTop:2,letterSpacing:1}}>Cible : {planetName} · {totalPlayerShips} vaisseaux</div>
        </div>
      ...
      {/* Main map area — flex 1, takes most of the screen */}
      <div style={{flex:1,position:"relative",margin:"0 10px",background:"radial-gradient(ellipse at 50% 30%,rgba(30,40,80,0.4) 0%,rgba(8,5,20,0.6) 70%)",border:"1px solid rgba(80,120,180,0.3)",borderRadius:10,overflow:"hidden"}}>
        {/* Subtle grid overlay */}
        ...
        {/* Planet name ABOVE the planet (no more overlap with enemy zones) */}
        ...
        {/* Planet — top center, slightly smaller */}
        ...
        {/* Enemy zones — positioned in 22-43% band (above front line) */}
        ...
        {/* LIGNE DE FRONT — visual separator at 48% */}
        ...
        {/* DEPLOYMENT ZONE — 52% to 97%, large drop area for next step's drag&drop */}
        <div style={{position:"absolute",left:14,right:14,top:"52%",bottom:"3%",border:"2px dashed rgba(120,200,255,0.45)",borderRadius:10,background:"linear-gradient(180deg,rgba(80,160,220,0.06) 0%,rgba(80,160,220,0.14) 100%)",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",pointerEvents:"none",animation:"dropPulse 3s ease-in-out infinite"}}>
          <div style={{fontSize:11,color:"#88ccee",fontFamily:"monospace",letterSpacing:3,fontWeight:"bold"}}>{"\u2295 ZONE DE D\u00C9PLOIEMENT"}</div>
          <div style={{fontSize:9,color:"#5588aa",fontFamily:"monospace",letterSpacing:1,fontStyle:"italic",marginTop:6,opacity:0.85}}>{"Glissez vos vaisseaux ici (\u00E9tape suivante)"}</div>
        </div>

        {/* Recon hint — collapsed to icon top-right */}
      ...
      {/* Bottom: ship reserve panel — horizontal compact cards [icon name count] */}
      <div style={{padding:"6px 10px 4px",flexShrink:0}}>
        ...
        <div style={{display:"flex",gap:5,overflowX:"auto",WebkitOverflowScrolling:"touch"}}>
          {Object.keys(SHIP_INFO).map(function(shipId){
            ...
            return (
              <div key={shipId} style={{flex:1,minWidth:88,padding:"6px 8px",background:"rgba(20,30,50,0.7)",border:"1px solid "+info.color+"55",borderRadius:6,display:"flex",alignItems:"center",gap:6}}>
                ...
                <div style={{fontSize:14,color:"#fff",fontFamily:"monospace",fontWeight:"bold",flexShrink:0}}>{qty}</div>
              </div>
            );
          })}
        </div>
      ...
      {/* Engage button — compact */}
      <div style={{padding:"4px 10px 10px",flexShrink:0}}>
        <button onClick={function(){onEngage(null);}} style={{width:"100%",padding:"11px 0",borderRadius:7,border:"1px solid rgba(204,68,102,0.6)",cursor:"pointer",background:"linear-gradient(180deg,rgba(200,40,80,0.9),rgba(140,20,50,0.9))",color:"#fff",fontFamily:"monospace",fontSize:12,letterSpacing:2.5,fontWeight:"bold",boxShadow:"0 0 16px rgba(220,80,100,0.4)"}}>
          {"\u2694\uFE0F ENGAGER LE COMBAT"}
        </button>
      </div>
    </div>
