...
function CombatScene({targetPlanet,attackerComp,defenderMilitary,testMode,placements,onClose,onResolve}){
  ...
  useEffect(function(){
    ...
    var camTarget={x:0,y:0,z:0}; // What the camera looks at (battlefield center)
    ...
    var camRadius=130; // Distance from target
    var camRadiusMin=50,camRadiusMax=300; // Zoom limits
    ...
    camControlsRef.current.recenter=function(){
      camTheta=0;camPhi=Math.PI*0.32;camRadius=130;
      camTarget.x=0;camTarget.y=0;camTarget.z=0;
      updateCamera();
    };
    camControlsRef.current.zoomIn=function(){
      camRadius=Math.max(camRadiusMin,camRadius-25);
      updateCamera();
    };
    camControlsRef.current.zoomOut=function(){
      camRadius=Math.min(camRadiusMax,camRadius+25);
      updateCamera();
    };
    ...
    if(hasPlacements){
      // Map 2D map % (from BattlePlanScreen) to 3D battlefield coords
      function pl2dTo3dX(px){return Math.max(-180,Math.min(180,(px-50)*2.5));}
      function pl2dTo3dZ(py){return Math.max(8,Math.min(115,((py-52)/45)*90+15));}
    ...
    for(var dfk=0;dfk<dnPresent;dfk++){
      ...
      // Evenly spread types across X = -100 to +100 at Z = -35
      var dCenterX=dnPresent===1?0:(-100+(dfk/(dnPresent-1))*200);
      var dCenterZ=-35;
    ...
    for(var dt=0;dt<dnDefPresent;dt++){
      ...
      var dsCenterX=dnDefPresent===1?0:(-90+(dt/(dnDefPresent-1))*180);
      var dsCenterZ=-90;
        ...
        if(selectedSet.size>0){
          ...
          if(Math.abs(rayDir.y)>0.001){
            ...
            if(tParam>0){
              ...
              targetZ=Math.max(-150,Math.min(120,targetZ));
