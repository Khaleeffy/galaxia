...
function CombatScene({targetPlanet,attackerComp,defenderMilitary,testMode,placements,onClose,onResolve}){
  ...
  useEffect(function(){
    ...
    // Color per ship type for the count badge border
    var SHIP_BADGE_COLORS={chasseur:"#88ccff",cargo:"#cccc88",croiseur:"#ff8855",bombardier:"#ff5588",cuirasse:"#aa66ff"};
    ...
    // ── Spawn defender ships (enemy bot) — far side of battlefield ──
    var defenderColor=0xdd4466; // red
    var defenderShips=[];
    var defShipsData=(defenderMilitary&&defenderMilitary.ships)||{};
    var enemyZ=-30;
    idx=0;
    for(var dfk=0;dfk<attackerTypes.length;dfk++){
      var dft=attackerTypes[dfk];
      var dqty=defShipsData[dft]||0;
      for(var dqi=0;dqi<dqty;dqi++){
        var dship=makeShip(dft,defenderColor);
        dship.scale.set(3.5,3.5,3.5);
        var drow=Math.floor(idx/8);
        var dcol=idx%8;
        dship.position.set((dcol-3.5)*12,0,enemyZ-drow*12);
        // Already facing forward (toward player) by default
        scene.add(dship);
        var dstats=getShipCombatStats(dft);
        defenderShips.push({mesh:dship,type:dft,hp:dstats.hp,maxHp:dstats.hp,dps:dstats.dps,range:dstats.range,fireCooldown:0,target:null,alive:true,side:"defender"});
        idx++;
      }
    }
    
    // ── Spawn defenses on planet surface (in front of planet) ──
    var defenses=[];
    var defData=(defenderMilitary&&defenderMilitary.defenses)||{};
    var defTypes=["def_tourelle","def_bouclier","def_missiles","def_forteresse"];
    var defIdx=0;
    var defZ=-90;
    for(var dt=0;dt<defTypes.length;dt++){
      var defType=defTypes[dt];
      var dQty=defData[defType]||0;
      // Cap visual count to avoid overload (visual cap, gameplay uses real count)
      var visualQty=Math.min(dQty,defType==="def_tourelle"?12:(defType==="def_missiles"?8:dQty));
      for(var dvq=0;dvq<visualQty;dvq++){
        var defMesh=makeDefense(defType);
        defMesh.scale.set(3.5,3.5,3.5);
        var dRow=Math.floor(defIdx/10);
        var dCol=defIdx%10;
        defMesh.position.set((dCol-4.5)*14,-2,defZ-dRow*12);
        scene.add(defMesh);
        var dfs=getDefenseCombatStats(defType);
        defenses.push({mesh:defMesh,type:defType,hp:dfs.hp,maxHp:dfs.hp,dps:dfs.dps,range:dfs.range,fireCooldown:0,target:null,alive:true,side:"defender"});
        defIdx++;
      }
    }
    ...
    function animate(){
      ...
      // ── Update squad count badges (position follows live mesh average; text updates only when count changes) ──
      for(var sq=0;sq<attackerSquads.length;sq++){
        var squad=attackerSquads[sq];
        var living=0;
        var meanX=0,meanZ=0,livingMeshCount=0;
        for(var mii=0;mii<squad.meshIndices.length;mii++){
          var sm=attackerShips[squad.meshIndices[mii]];
          if(sm.alive){
        ...
        if(livingMeshCount>0){
          ...
          squad.badgeSprite.position.set(meanX,14,meanZ);
        }
        ...
        if(attackersAlive===0||defendersAlive===0){
          ...
          setTimeout(function(){
            ...
            for(var ki2=0;ki2<defenderShips.length;ki2++){
              ...
              if(dsh.alive)defSurv[dsh.type]=(defSurv[dsh.type]||0)+1;
              else defLost[dsh.type]=(defLost[dsh.type]||0)+1;
            }
            ...
            for(var ki3=0;ki3<defenses.length;ki3++){
              ...
              if(dst.alive)defStrSurv[dst.type]=(defStrSurv[dst.type]||0)+1;
              else defStrLost[dst.type]=(defStrLost[dst.type]||0)+1;
            }
            setCombatResult({
    ...
    return function(){
      ...
      // Dispose squad count badge sprites and textures
