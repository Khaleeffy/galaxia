...
// Cosmic style: dark gradient background, glowing enemy zones, planet at center, deployment area for player
function BattlePlanScreen({plan,onCancel,onEngage}){
  // Generate approximate enemy zones based on defender military
  // Position: planet at center-top, player deploys around bottom half (encerclement possible)
  ...
  // Pseudo-random distribution: split enemies into 2-3 zones for readability
  function makeEnemyZones(){
    if(totalEnemyShips===0&&totalEnemyDef===0)return [];
    var zones=[];
    // Defenses: always around the planet (center, close)
    if(totalEnemyDef>0){
      zones.push({x:50,y:30,size:Math.min(60,20+totalEnemyDef*4),strength:totalEnemyDef,kind:"defense",label:"Défenses"});
    }
    // Ships: 1-3 fleet groups depending on size
    if(totalEnemyShips>0){
      var groups=totalEnemyShips<=10?1:(totalEnemyShips<=30?2:3);
      var perGroup=Math.ceil(totalEnemyShips/groups);
      // Spread groups around the planet area
      var positions=[
        [{x:50,y:42,size:35,strength:perGroup}],
        [{x:32,y:38,size:30,strength:perGroup},{x:68,y:38,size:30,strength:perGroup}],
        [{x:28,y:35,size:28,strength:perGroup},{x:50,y:46,size:32,strength:perGroup},{x:72,y:35,size:28,strength:perGroup}],
      ];
  ...
  return (
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"radial-gradient(ellipse at top,rgba(20,15,40,0.98) 0%,rgba(5,3,15,0.98) 80%)",zIndex:300,display:"flex",flexDirection:"column",overflow:"hidden",paddingTop:"env(safe-area-inset-top,0px)",paddingBottom:"env(safe-area-inset-bottom,0px)"}}>
      ...
      {/* Top HUD: title + cancel button */}
      <div style={{padding:"10px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",background:"linear-gradient(180deg,rgba(15,10,30,0.85),transparent)"}}>
        <div>
          <div style={{fontSize:12,color:"#aaccee",fontFamily:"monospace",fontWeight:"bold",letterSpacing:2.5}}>{"\u{1F5FA}\uFE0F PLAN D\u2019ATTAQUE"}</div>
          <div style={{fontSize:9,color:"#7a99bb",fontFamily:"monospace",marginTop:2,letterSpacing:1}}>Cible : {plan.targetPlanet&&plan.targetPlanet.name} · {totalPlayerShips} vaisseaux disponibles</div>
        </div>
        <button onClick={onCancel} style={{background:"rgba(40,15,25,0.85)",border:"1px solid #aa4466",borderRadius:5,color:"#ffaabb",fontSize:11,padding:"5px 12px",cursor:"pointer",fontFamily:"monospace",fontWeight:"bold",letterSpacing:1.5}}>{"× ANNULER"}</button>
      </div>
      
      {/* Main map area — flex 1 */}
      <div style={{flex:1,position:"relative",margin:"0 12px",background:"radial-gradient(ellipse at 50% 30%,rgba(30,40,80,0.4) 0%,rgba(8,5,20,0.6) 70%)",border:"1px solid rgba(80,120,180,0.3)",borderRadius:10,overflow:"hidden"}}>
        {/* Subtle grid overlay */}
        ...
        {/* Planet at top center */}
        <div style={{position:"absolute",top:"15%",left:"50%",transform:"translate(-50%,-50%)",width:80,height:80,borderRadius:"50%",background:"radial-gradient(circle at 35% 30%,#aaccdd 0%,#557799 60%,#223344 100%)",boxShadow:"0 0 40px rgba(120,170,220,0.4),inset -10px -10px 20px rgba(0,0,0,0.5)"}}/>
        <div style={{position:"absolute",top:"calc(15% + 50px)",left:"50%",transform:"translateX(-50%)",fontSize:9,color:"#aaccdd",fontFamily:"monospace",letterSpacing:1.5,whiteSpace:"nowrap"}}>{plan.targetPlanet&&plan.targetPlanet.name}</div>
        ...
        {zones.map(function(z,i){
          ...
          return (
            <div key={i} style={{position:"absolute",top:z.y+"%",left:z.x+"%",transform:"translate(-50%,-50%)",width:z.size+"px",height:z.size+"px",borderRadius:"50%",background:"radial-gradient(circle,"+color+" 0%,transparent 70%)",border:"2px dashed "+borderColor,boxShadow:"0 0 20px "+color,display:"flex",alignItems:"center",justifyContent:"center",pointerEvents:"none"}}>
              <div style={{fontSize:10,color:"#ffccdd",fontFamily:"monospace",fontWeight:"bold",textShadow:"0 0 6px "+borderColor,letterSpacing:1,textAlign:"center",lineHeight:1.3}}>
                {hasIntel?(z.kind==="defense"?"\u{1F6E1}\uFE0F "+z.strength:"\u{1F6F8} ~"+z.strength):"?"}
                {!hasIntel&&<div style={{fontSize:7,marginTop:2,opacity:0.7}}>{z.kind==="defense"?"déf.":"flotte"}</div>}
              </div>
            </div>
          );
        })}
        
        {/* Deployment zone hint — bottom half */}
        <div style={{position:"absolute",bottom:0,left:0,right:0,height:"55%",background:"linear-gradient(0deg,rgba(80,140,200,0.08) 0%,transparent 100%)",borderTop:"1px dashed rgba(120,180,220,0.3)",pointerEvents:"none",display:"flex",alignItems:"flex-start",justifyContent:"center",paddingTop:8}}>
          <div style={{fontSize:9,color:"#7a99bb",fontFamily:"monospace",letterSpacing:1.5,opacity:0.8}}>{"\u{1F535} ZONE DE DÉPLOIEMENT"}</div>
        </div>
        
        {/* Hint text in middle of deployment zone */}
        <div style={{position:"absolute",bottom:"30%",left:0,right:0,textAlign:"center",fontSize:10,color:"#88aacc",fontFamily:"monospace",letterSpacing:1.2,fontStyle:"italic",opacity:0.6,pointerEvents:"none"}}>{"Le drag & drop arrive à l\u2019étape suivante..."}</div>
        
        {/* Intel hint — top right */}
        {!hasIntel&&<div style={{position:"absolute",top:8,right:8,padding:"5px 8px",background:"rgba(40,30,60,0.7)",border:"1px solid rgba(120,100,180,0.4)",borderRadius:5,fontSize:8,color:"#ccaadd",fontFamily:"monospace",letterSpacing:1,backdropFilter:"blur(4px)",maxWidth:140}}>{"\u{1F50D} Recon limitée. Branche Exploration = détails précis."}</div>}
      </div>
      
      {/* Bottom: ship reserve panel */}
      <div style={{padding:"10px 14px",background:"linear-gradient(0deg,rgba(15,10,30,0.85),transparent)"}}>
        <div style={{fontSize:8,color:"#88aacc",fontFamily:"monospace",letterSpacing:1.8,marginBottom:6,fontWeight:"bold"}}>{"\u{1F4E6} VOTRE FLOTTE EN RÉSERVE"}</div>
        <div style={{display:"flex",gap:6,marginBottom:10,overflowX:"auto",WebkitOverflowScrolling:"touch"}}>
          {Object.keys(SHIP_INFO).map(function(shipId){
            ...
            return (
              <div key={shipId} style={{flex:"0 0 auto",padding:"6px 10px",background:"rgba(20,30,50,0.7)",border:"1px solid "+info.color+"55",borderRadius:6,minWidth:80,textAlign:"center"}}>
                <div style={{fontSize:18,filter:"drop-shadow(0 0 4px "+info.color+"66)"}}>{info.icon}</div>
                <div style={{fontSize:9,color:info.color,fontFamily:"monospace",letterSpacing:0.5,marginTop:2}}>{info.name}</div>
                <div style={{fontSize:11,color:"#fff",fontFamily:"monospace",fontWeight:"bold",marginTop:2}}>{qty}</div>
              </div>
            );
          })}
        </div>
        ...
        {/* Engage button */}
        <button onClick={function(){onEngage(null);}} style={{width:"100%",padding:"14px 0",borderRadius:6,border:"1px solid rgba(204,68,102,0.6)",cursor:"pointer",background:"linear-gradient(180deg,rgba(200,40,80,0.85),rgba(140,20,50,0.85))",color:"#fff",fontFamily:"monospace",fontSize:12,letterSpacing:2.5,fontWeight:"bold",boxShadow:"0 0 16px rgba(220,80,100,0.4)"}}>
          {"\u2694\uFE0F ENGAGER LE COMBAT"}
        </button>
      </div>
    </div>
