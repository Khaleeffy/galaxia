...
const colorVar=r();
return{seed:s,i,name:mkName(s),type,dist:d,temp:Math.round(400-d*80+(r()-0.5)*60),light:Math.max(5,Math.round(100/(d*d)*(0.7+r()*0.6))),size:3000+Math.floor(r()*12000),water:w,atmos:ATMOS[Math.floor(r()*ATMOS.length)],moons:Math.floor(r()*r()*5),tilt:r()*40,clouds:type===“Gazeuse”?0:type===“Glaciale”?0.55:type===“Désertique”?0.08:0.3+r()*0.4,hasRings,ringTilt:r()*0.6-0.3,colorVar,orbitAngle:(i/50)*Math.PI*2+r()*0.3,orbitRadius:80+(i/50)*340,orbitRadius3d:4+r()*18,orbitInc:r()*Math.PI*0.6-Math.PI*0.3,orbitAsc:r()*Math.PI*2,orbitAngle3d:r()*Math.PI*2,orbitSpeed:0.08+r()*0.18};
}
...
const habOf=p=>{let s=0;if(p.temp>-20&&p.temp<60)s+=3;else if(p.temp>-50&&p.temp<100)s+=1;if(p.water>20&&p.water<80)s+=2;if(p.atmos===“Respirable”)s+=3;else if([“Dense”,“Légère”].includes(p.atmos))s+=1;if(p.light>20&&p.light<80)s+=2;return{l:s>=8?“Habitable”:s>=5?“Marginale”:s>=2?“Hostile”:“Inhospitalier”,c:s>=8?”#40c870”:s>=5?”#c0b030”:s>=2?”#c06030”:”#c03020”};};
const TERRAIN_TYPES={
...
tundra: {name:“Toundra”, icon:“🧊”,bonuses:{deut:0.4},           buildable:true},
swamp:  {name:“Marais”,  icon:“🌿”,bonuses:{crystal:0.3,deut:0.2},buildable:true},
};
...
return{gw:GW,gh:GH,seaLevel,components,labels,isLand};
}
...
regions=comps.map(c=>({…c,gw:GW,gh:GH}));
}
...
let shape;
if(region.cells&&region.cells.length>0&&!region.*fallback){
...
shape=Array.from({length:nPts},(*,p)=>{
...
const zones=Array.from({length:nZones},(_,zi)=>{
  ...
  const dp=(r()-0.5)*0.28,dt=r()*Math.PI*2;
  const lx2=cx+Math.cos(dt)*dp,ly2=cy+(r()-0.5)*0.28,lz2=cz+Math.sin(dt)*dp;
...
const CONTINENT_VERSION = 4; // v4: parentId for partition-sibling edge detection // bump to regenerate continents for all saves
...
// Transfer building placements: prefer matching terrain type, then any free buildable zone
existingBuildings.forEach(({ type, building }) => {
let placed = false;
for (const cont of continents) {
const match = cont.zones.find(z => z.type === type && !z.building);
if (match) { match.building = building; placed = true; break; }
}
if (!placed) {
for (const cont of continents) {
const anyZone = cont.zones.find(z => !z.building && z.terrain && z.terrain.buildable);
if (anyZone) { anyZone.building = building; break; }
}
}
});
...
function ContinentModal({continent,state,onBuildInZone,onClose,readOnly}){
const canvasRef=useRef();
const planetType=state&&state.planetType||“telluric”;
const[selZone,setSelZone]=useState(null);
const[imgTick,setImgTick]=useState(0);
useEffect(()=>{
// Preload zone images
Object.keys(ZONE_IMAGES).forEach(type=>{
const img=new Image();
img.onload=()=>setImgTick(t=>t+1);
img.src=ZONE_IMAGES[type];
});
// Preload planet map background
if(PLANET_MAP_BACKGROUND[planetType]&&!_mapBgCache[planetType]){
const bgImg=new Image();
bgImg.onload=()=>{_mapBgCache[planetType]=bgImg;setImgTick(t=>t+1);};
bgImg.src=PLANET_MAP_BACKGROUND[planetType];
}
},[planetType]);
...
affectedRef.current=[];
cvs.forEach(c=>{if(c!==canvasRef.current){affectedRef.current.push({el:c,prev:c.style.pointerEvents});c.style.pointerEvents=“none”;}});
},0);
return()=>{
...
const getZonePos=(z)=>{
// Layout relative to the image with objectFit:fill (stretched to container)
const layoutMap={
telluric:{yMin:0.18,yMax:0.52,xMin:0.14,xMax:0.86}
};
const L=layoutMap[planetType];
if(L){
const containerW=W,containerH=size.h-48;
const nx=L.xMin+z.mapX*(L.xMax-L.xMin);
const ny=L.yMin+z.mapY*(L.yMax-L.yMin);
return{px:nx*containerW,py:48+ny*containerH};
}
return{px:W*0.16+z.mapX*(W*0.68),py:100+z.mapY*(size.h-48-180)};
};
useEffect(()=>{
const cv=canvasRef.current;if(!cv)return;
const HD=H-48;cv.width=W;cv.height=HD;
const ctx=cv.getContext(“2d”);
// Planet-specific map background: if image exists, canvas is transparent (image shown via HTML img below)
// Otherwise, draw procedural parchment fallback
const hasBgImage=!!PLANET_MAP_BACKGROUND[planetType];
if(hasBgImage){
// Clear canvas to transparent - HTML img element handles the background
ctx.clearRect(0,0,W,HD);
}else{
// Planet-colored background based on planet type
const typeColors={
“Tellurique”:[”#1a3a20”,”#0a1f10”,”#041008”],
“Gazeuse”:[”#4a3020”,”#2a1810”,”#140a06”],
“Glaciale”:[”#2a4a6a”,”#152538”,”#070f18”],
“Désertique”:[”#6a4a1a”,”#3a260e”,”#1f1408”],
“Océanique”:[”#1a3a5a”,”#0d1f30”,”#050f18”],
“Volcanique”:[”#4a1a0a”,”#2a0d05”,”#140602”],
“Forestière”:[”#1a4a1a”,”#0d2510”,”#050f05”]
};
const cols=typeColors[planetType]||typeColors[“Tellurique”];
const parch=ctx.createRadialGradient(W/2,HD/2,0,W/2,HD/2,Math.max(W,HD)*0.7);
parch.addColorStop(0,cols[0]);
parch.addColorStop(0.6,cols[1]);
parch.addColorStop(1,cols[2]);
ctx.fillStyle=parch;ctx.fillRect(0,0,W,HD);
// Subtle starfield for cosmic feel
for(let i=0;i<40;i++){
ctx.fillStyle=“rgba(255,255,255,”+(0.15+Math.random()*0.35)+”)”;
ctx.beginPath();
ctx.arc(Math.random()*W,Math.random()*HD,0.5+Math.random()*1,0,Math.PI*2);
ctx.fill();
}
}
// Title cartouche: only draw if no custom image (image may have its own)
if(!hasBgImage){
const titleW=Math.min(260,W*0.5),titleH=44;
const tx=W/2-titleW/2,ty=18;
const tp=new Path2D();
tp.moveTo(tx+20,ty);tp.lineTo(tx+titleW-20,ty);
tp.bezierCurveTo(tx+titleW,ty,tx+titleW,ty+titleH,tx+titleW-20,ty+titleH);
tp.lineTo(tx+20,ty+titleH);
tp.bezierCurveTo(tx,ty+titleH,tx,ty,tx+20,ty);
ctx.fillStyle=“rgba(240,220,180,0.95)”;ctx.fill(tp);
ctx.strokeStyle=“rgba(80,55,30,0.7)”;ctx.lineWidth=1.5;ctx.stroke(tp);
ctx.fillStyle=”#3a2a14”;ctx.font=“italic bold 18px serif”;
ctx.textAlign=“center”;ctx.textBaseline=“middle”;
ctx.fillText(continent.name,W/2,ty+titleH/2);
}
const cshape=continent.shape||[];
const topSafe=80;
const bottomSafe=70;
const availH=HD-topSafe-bottomSafe;
const maxR=1.5;
const vertSquish=0.85;
const cscl=Math.min(availH/(2*maxR*vertSquish+0.2),(W-80)/(2*maxR+0.2));
const cxC=W/2,cyC=topSafe+availH/2;
const contPath=new Path2D();
const pts=[];
cshape.forEach((p,i)=>{
const x=cxC+Math.cos(p.a)*p.r*cscl;
const y=cyC+Math.sin(p.a)*p.r*cscl*0.85;
pts.push({x,y});
i===0?contPath.moveTo(x,y):contPath.lineTo(x,y);
});
contPath.closePath();
// Continent shape removed - only zones are shown
// Image-based terrain rendering using pre-generated illustrations
const _zoneImgCache={};
const getZoneImg=(type)=>{
if(_zoneImgCache[type])return _zoneImgCache[type];
const src=ZONE_IMAGES[type];
if(!src)return null;
const img=new Image();
img.src=src;
_zoneImgCache[type]=img;
return img;
};
const drawZoneImage=(type,x,y,s)=>{
const img=getZoneImg(type);
if(!img||!img.complete||img.naturalWidth===0){
// Not loaded yet - draw placeholder
ctx.strokeStyle=”#6a4018”;ctx.lineWidth=1;
...
// Ensure images trigger re-render when loaded
Object.keys(ZONE_IMAGES).forEach(type=>{
const img=getZoneImg(type);
if(img&&!img.complete){
img.onload=()=>{if(canvasRef.current){const ev=new Event(“zoneimgload”);canvasRef.current.dispatchEvent(ev);}};
}
});
const drawMountain=(x,y,s)=>drawZoneImage(“mountain”,x,y,s);
const drawTree=(x,y,s)=>drawZoneImage(“forest”,x,y,s);
const drawWave=(x,y,s)=>drawZoneImage(“ocean”,x,y,s);
const drawCoast=(x,y,s)=>drawZoneImage(“coast”,x,y,s);
const drawVolcano=(x,y,s)=>drawZoneImage(“volcano”,x,y,s);
const drawIce=(x,y,s)=>drawZoneImage(“ice”,x,y,s);
const drawTundra=(x,y,s)=>drawZoneImage(“tundra”,x,y,s);
const drawDune=(x,y,s)=>drawZoneImage(“desert”,x,y,s);
const drawPlain=(x,y,s)=>drawZoneImage(“plain”,x,y,s);
const drawSwamp=(x,y,s)=>drawZoneImage(“swamp”,x,y,s);
const ZONE_DECO={mountain:drawMountain,forest:drawTree,ocean:drawWave,coast:drawCoast,volcano:drawVolcano,ice:drawIce,tundra:drawTundra,desert:drawDune,plain:drawPlain,swamp:drawSwamp};
// Draw each zone as a free-standing hand-drawn illustration — no circles
(continent.zones||[]).forEach(z=>{
const {px,py}=getZonePos(z);
const drawFn=ZONE_DECO[z.type]||drawPlain;
const isSel=selZone&&selZone.id===z.id;
const hasBld=!!z.building;
const drawSize=isSel?100:85;
// Subtle glow for selected / built zones
ctx.save();
if(isSel){ctx.shadowColor=“rgba(255,130,30,0.7)”;ctx.shadowBlur=18;}
else if(hasBld){ctx.shadowColor=“rgba(200,140,50,0.5)”;ctx.shadowBlur=10;}
drawFn(px,py,drawSize);
ctx.restore();
// Building icon above the drawing if built
if(hasBld){
const b=BUILDINGS.find(b=>b.id===z.building);
if(b){
// Small golden badge
ctx.save();
ctx.fillStyle=”#f0d890”;ctx.strokeStyle=”#5a3a10”;ctx.lineWidth=1.5;
ctx.beginPath();ctx.arc(px+drawSize*0.8,py-drawSize*0.7,16,0,Math.PI*2);ctx.fill();ctx.stroke();
ctx.font=“14px serif”;ctx.textAlign=“center”;ctx.textBaseline=“middle”;
ctx.fillText(b.icon,px+drawSize*0.8,py-drawSize*0.7);
ctx.restore();
}
}
// Zone name below
ctx.font=“italic bold “+(isSel?14:11)+“px serif”;
ctx.textAlign=“center”;ctx.textBaseline=“middle”;
ctx.strokeStyle=“rgba(240,220,180,0.9)”;ctx.lineWidth=3;
ctx.strokeText(z.name,px,py+drawSize*0.75+18);
ctx.fillStyle=”#3a2410”;
ctx.fillText(z.name,px,py+drawSize*0.75+18);
if(hasBld){
const b=BUILDINGS.find(b=>b.id===z.building);
if(b){
ctx.font=“italic 9px serif”;
ctx.strokeStyle=“rgba(240,220,180,0.9)”;ctx.lineWidth=2.5;
ctx.strokeText(b.name,px,py+drawSize*0.75+32);
ctx.fillStyle=”#6a3a10”;
ctx.fillText(b.name,px,py+drawSize*0.75+32);
}
}
});
const ccX=W-56,ccY=HD-56;
ctx.save();ctx.translate(ccX,ccY);
ctx.strokeStyle=“rgba(80,55,30,0.7)”;ctx.lineWidth=1.2;
ctx.beginPath();ctx.arc(0,0,28,0,Math.PI*2);ctx.stroke();
ctx.beginPath();ctx.arc(0,0,22,0,Math.PI*2);ctx.stroke();
ctx.fillStyle=”#3a2a14”;ctx.font=“italic bold 11px serif”;ctx.textAlign=“center”;ctx.textBaseline=“middle”;
ctx.fillText(“N”,0,-18);ctx.fillText(“S”,0,18);ctx.fillText(“E”,18,0);ctx.fillText(“O”,-18,0);
ctx.fillStyle=”#6a3a10”;ctx.beginPath();ctx.moveTo(0,-14);ctx.lineTo(-3,0);ctx.lineTo(0,-10);ctx.lineTo(3,0);ctx.closePath();ctx.fill();
ctx.fillStyle=”#3a2a14”;ctx.beginPath();ctx.moveTo(0,14);ctx.lineTo(-3,0);ctx.lineTo(0,10);ctx.lineTo(3,0);ctx.closePath();ctx.fill();
ctx.restore();
},[continent,selZone,W,H,imgTick,planetType]);
const handleClick=e=>{
const cv=canvasRef.current;if(!cv)return;
const rect=cv.getBoundingClientRect();
const HD=H-48;
const mx=(e.clientX-rect.left)*(W/rect.width),my=(e.clientY-rect.top)*(HD/rect.height);
let hit=null;
(continent.zones||[]).forEach(z=>{
const {px,py}=getZonePos(z);
if(Math.sqrt((mx-px)*(mx-px)+(my-py)*(my-py))<90)hit=z;
});
setSelZone(hit&&selZone&&hit.id===selZone.id?null:hit||null);
};
const rl=(state.buildings||{}).usine_robots||0;
const wrapSt={position:“fixed”,top:0,left:0,right:0,bottom:0,zIndex:500,pointerEvents:“auto”,background:”#040812”,height:“100dvh”,maxHeight:“100dvh”};
const hdrSt={position:“absolute”,top:0,left:0,right:0,height:48,display:“flex”,alignItems:“center”,justifyContent:“space-between”,padding:“0 14px”,background:“rgba(8,14,24,0.9)”,backdropFilter:“blur(6px)”,borderBottom:“1px solid #1a3050”,zIndex:10};
const panelSt={position:“absolute”,left:“50%”,bottom:18,transform:“translateX(-50%)”,width:“min(460px,94vw)”,maxHeight:“62vh”,overflowY:“auto”,WebkitOverflowScrolling:“touch”,background:“rgba(10,20,40,0.92)”,border:“1px solid #1a3050”,borderRadius:10,padding:“14px 16px 20px”,boxShadow:“0 0 40px rgba(0,0,0,0.7)”,color:”#9abcd8”,fontFamily:“serif”};
const hintSt={position:“absolute”,bottom:22,left:“50%”,transform:“translateX(-50%)”,padding:“7px 18px”,background:“rgba(10,20,40,0.85)”,border:“1px solid #1a3050”,borderRadius:16,color:”#9abcd8”,fontSize:11,fontFamily:“serif”,fontStyle:“italic”,letterSpacing:1};
return(<div style={wrapSt}>
{PLANET_MAP_BACKGROUND[planetType]&&(
<img src={PLANET_MAP_BACKGROUND[planetType]} alt=”” style={{position:“absolute”,top:48,left:0,right:0,width:“100%”,height:“calc(100% - 48px)”,objectFit:“fill”,pointerEvents:“none”,zIndex:0}}/>
)}
<canvas ref={canvasRef} onClick={handleClick} style={{position:“absolute”,top:48,left:0,right:0,width:“100%”,height:“calc(100% - 48px)”,cursor:“crosshair”,display:“block”,zIndex:1}}/>
<div style={hdrSt}>
<div style={{display:“flex”,alignItems:“center”,gap:10,flex:1,minWidth:0}}>
<span style={{fontSize:18,color:”#c8e0ff”,fontFamily:“serif”,fontStyle:“italic”,fontWeight:“bold”,flexShrink:0}}>{continent.name}</span>
<span style={{fontSize:10,color:”#5a7aa0”,fontFamily:“serif”,flexShrink:0}}>{”— “}{(continent.zones||[]).length}{” contrées”}</span>
{state&&state.planetType&&PLANET_PASSIVE_BONUS[state.planetType]&&(
<span style={{fontSize:10,color:”#b8924a”,fontFamily:“serif”,fontStyle:“italic”,marginLeft:“auto”,padding:“3px 10px”,borderRadius:6,background:“rgba(240,220,180,0.12)”,border:“1px solid rgba(168,138,90,0.4)”,whiteSpace:“nowrap”,overflow:“hidden”,textOverflow:“ellipsis”}}>{“✦ “}{state.planetType}{” — “}{PLANET_PASSIVE_BONUS[state.planetType].label}</span>
)}
</div>
<button onClick={onClose} style={{background:“rgba(240,220,180,0.15)”,border:“1px solid #a88a5a”,color:”#c8e0ff”,cursor:“pointer”,fontSize:12,padding:“5px 14px”,borderRadius:6,fontFamily:“serif”,fontStyle:“italic”,flexShrink:0,marginLeft:8}}>{“fermer”}</button>
</div>
{selZone&&!readOnly&&(
<div style={panelSt}>
<ZoneBuildPanel zone={selZone} state={state} rl={rl} allContinents={state.continents||[]} onBuild={(z,b,lv)=>{onBuildInZone(z,b,lv);setSelZone(null);}} onClose={()=>setSelZone(null)}/>
</div>
)}
{!selZone&&!isLandscape&&(<div style={hintSt}>{“Cliquer sur une contrée”}</div>)}

  </div>);
}

function ZoneBuildPanel({zone,state,rl,allContinents,onBuild,onClose}){
...
const cost=placedBld?bCost(placedBld,lv):null;
const afford=cost?canAfford(state.resources,cost):false;
const busy=!!state.queue;
const secs=placedBld?bTime(placedBld,lv,rl):0;
const icons={ocean:“🌊”,coast:“🏖”,plain:“🌾”,forest:“🌲”,mountain:“⛰”,volcano:“🌋”,ice:“🧊”,desert:“🏜”};
return(<div>
<div style={{display:“flex”,justifyContent:“space-between”,alignItems:“center”,marginBottom:10,paddingBottom:8,borderBottom:“1px solid rgba(106,74,32,0.4)”}}>
<span style={{fontSize:15,color:”#9abcd8”,fontFamily:“serif”,fontWeight:“bold”,fontStyle:“italic”}}>{icons[zone.type]||“🌾”}{” “}{zone.name}</span>
<button onClick={onClose} style={{background:“transparent”,border:“1px solid #8a6a3a”,color:”#6a4a20”,cursor:“pointer”,fontSize:11,padding:“3px 10px”,borderRadius:4,fontFamily:“serif”,fontStyle:“italic”}}>{“×”}</button>
</div>
{placedBld?(<div>
<div style={{fontSize:13,color:”#9abcd8”,marginBottom:6,fontFamily:“serif”}}>{placedBld.icon}{” “}<b>{placedBld.name}</b>{” — Niv. “}{lv}</div>
{placedBld.terrainBonus&&placedBld.terrainBonus.includes(zone.type)&&<div style={{fontSize:10,color:”#2a5a10”,marginBottom:6,fontFamily:“serif”,fontStyle:“italic”}}>{“✓ Bonus de terrain actif”}</div>}
<div style={{fontSize:10,marginBottom:8,display:“flex”,gap:8,flexWrap:“wrap”,fontFamily:“serif”}}>
{cost.metal>0&&<span style={{color:state.resources.metal>=cost.metal?”#3a2410”:”#8a2010”}}>{“⛏ “}{fmt(cost.metal)}</span>}
{cost.crystal>0&&<span style={{color:state.resources.crystal>=cost.crystal?”#3a2410”:”#8a2010”}}>{“💎 “}{fmt(cost.crystal)}</span>}
{cost.deut>0&&<span style={{color:state.resources.deut>=cost.deut?”#3a2410”:”#8a2010”}}>{“⚗ “}{fmt(cost.deut)}</span>}
<span style={{color:”#6a4a20”,marginLeft:“auto”}}>{“⏱ “}{fmtT(secs)}</span>
</div>
<button onClick={()=>{if(afford&&!busy)onBuild(zone,placedBld,lv);}} disabled={!afford||busy} style={{width:“100%”,padding:“9px 0”,borderRadius:6,border:“1px solid “+(afford&&!busy?”#3a6a20”:”#8a6a3a”),cursor:afford&&!busy?“pointer”:“not-allowed”,background:afford&&!busy?“linear-gradient(180deg,#8aa858,#5a7a30)”:“rgba(180,160,120,0.5)”,color:afford&&!busy?”#f8eacc”:”#8a6a3a”,fontSize:12,fontFamily:“serif”,fontStyle:“italic”,fontWeight:“bold”}}>
{busy?“File occupée”:!afford?“Ressources insuffisantes”:“Améliorer au niveau “+(lv+1)}
</button>
</div>):(<div>
<div style={{fontSize:11,color:”#6a4a20”,marginBottom:10,fontFamily:“serif”,fontStyle:“italic”}}>{“Choisir un édifice à bâtir :”}</div>
<div style={{display:“grid”,gridTemplateColumns:“1fr 1fr 1fr”,gap:6}}>
{BUILDINGS.filter(b=>b.id!==“qg”).map(b=>{
const blv=state.buildings[b.id]||0;
const bcost=bCost(b,blv);
const baff=canAfford(state.resources,bcost);
const alreadyPlaced=allContinents.some(c=>c.zones.some(z=>z.building===b.id));
const isBonus=b.terrainBonus&&b.terrainBonus.includes(zone.type);
const disabled=!baff||busy||alreadyPlaced;
return(<button key={b.id} onClick={()=>{if(!disabled)onBuild(zone,b,blv);}} disabled={disabled} style={{padding:“7px 5px”,borderRadius:6,border:“1px solid “+(isBonus?”#3a6a20”:”#8a6a3a”),background:isBonus?“rgba(180,200,120,0.4)”:“rgba(240,220,180,0.5)”,color:!disabled?”#3a2410”:”#8a6a3a”,cursor:disabled?“not-allowed”:“pointer”,fontFamily:“serif”,textAlign:“center”}}>
<div style={{fontSize:18}}>{b.icon}</div>
<div style={{fontSize:9,marginTop:3,fontStyle:“italic”}}>{b.name.split(” “)[0]}</div>
{isBonus&&<div style={{fontSize:8,color:”#3a6a20”,fontStyle:“italic”}}>{“bonus”}</div>}
{alreadyPlaced&&<div style={{fontSize:8,color:”#8a2010”,fontStyle:“italic”}}>{“placé”}</div>}
</button>);
})}
</div>
</div>)}
...
if(pb>0){const ic=mix([r,g,b],p.s,Math.min(pb*0.8,0.85));r=ic[0];g=ic[1];b=ic[2];}
}
const idx=(py*sz+px)*4;d[idx]=clamp(r);d[idx+1]=clamp(g);d[idx+2]=clamp(b);d[idx+3]=255;
}}ctx.putImageData(img,0,0);return cv;
}
function genNebulaTexture(seed,sz=512){
const cv=document.createElement(“canvas”);cv.width=sz;cv.height=sz;
const ctx=cv.getContext(“2d”),img=ctx.createImageData(sz,sz),d=img.data;
const n1=noise2d(seed),n2=noise2d(seed+7777),n3=noise2d(seed+12345);
const themes=[{r:[60,20,100],g:[20,10,80],b:[80,40,140]},{r:[20,60,100],g:[10,30,80],b:[40,80,140]},{r:[80,20,20],g:[20,10,10],b:[40,20,60]},{r:[20,80,60],g:[10,40,30],b:[30,60,80]}];
const th=themes[seed%themes.length];
for(let py=0;py<sz;py++){for(let px=0;px<sz;px++){
const u=px/sz*2,v=py/sz*2,n=fbm(n1,u,v,6,2.0,0.55),n_=fbm(n2,u+n*.8,v+n*.8,5,2.1,0.5);
const density=Math.max(0,n_*.5+.3),filament=fbm(n3,u*1.5+2,v*1.5+2,4)*.5+.5,alpha=Math.pow(density,2)*filament*0.35;
const r=Math.round(th.r[0]+density*(th.r[1]-th.r[0])+filament*(th.r[2]-th.r[1]));
const g=Math.round(th.g[0]+density*(th.g[1]-th.g[0])+filament*(th.g[2]-th.g[1]));
const b=Math.round(th.b[0]+density*(th.b[1]-th.b[0])+filament*(th.b[2]-th.b[1]));
const idx=(py*sz+px)*4;d[idx]=clamp(r,0,255);d[idx+1]=clamp(g,0,255);d[idx+2]=clamp(b,0,255);d[idx+3]=Math.round(alpha*255);
}}ctx.putImageData(img,0,0);return cv;
}
...
if(t===“Désertique”){const i=Math.floor(v*3)%3;const os=[[185,148,52],[160,80,40],[200,120,30]];const ls=[[208,165,70],[190,110,60],[225,150,50]];return{o:os[i],l:ls[i],l2:mix(ls[i],[248,225,155],.32),m:[158,118,38],s:[235,210,155]};}
...
// Volcanic terrain: lava channels, ash/sulfur plateaus, dark basalt
const vNoise=cylFbm(n,2.5,lat,5,2.0,0.5,theta);
const vNoise2=cylFbm(n2,1.8,lat,4,2.0,0.5,theta,13,13);
if(vNoise>.28){
// Lava channels - rare, very bright
const lm=Math.min(1,(vNoise-.28)/.08);
bc=mix(p.l,p.lava||p.l2,lm);
}else if(vNoise2>.08){
// Ash/sulfur plateaus - lighter color for contrast vs basalt
const am=Math.min(1,(vNoise2-.08)/.16);
bc=mix(p.l,p.l2,am);
}else{
// Dark basalt (base)
bc=p.l;
...
// Desert: smooth gradient from darker dunes (beach/l) to lighter dunes (l2),
// no jumps to bright peak color. Wide [0..0.7] range for most of surface.
if(h>0.75){
// Highest dunes: mostly l2 but never reaching pure s (too bright)
bc=mix(p.l2||p.l,p.l2||p.l,1);
}else if(h>0.45){
// Mid-upper: l → l2 gradient
bc=mix(p.l,p.l2||p.l,(h-0.45)/0.30);
}else if(h>0.15){
// Mid-lower: mostly l with hints of warmer tone
bc=mix(p.beach||p.l,p.l,0.4+h*0.8);
}else{
// Low areas: warm beach tones
bc=p.beach||p.l;
}
}else if(h>.82){
...
const BUILDINGS=[
{id:“qg”,name:“Quartier General”,icon:“🏛️”,base:{metal:800,crystal:400,deut:200},factor:3.0,produces:“qg”,terrainBonus:[“plain”,“coast”,“desert”,“mountain”,“forest”,“volcano”]},
{id:“mine_metal”,name:“Mine de Metal”,icon:“⛏️”,base:{metal:60,crystal:15,deut:0},factor:1.5,produces:“metal”,terrainBonus:[“mountain”,“volcano”,“desert”]},
...
{id:“synth_deut”,name:“Synthetiseur”,icon:“⚗️”,base:{metal:225,crystal:75,deut:0},factor:1.5,produces:“deut”,terrainBonus:[“coast”,“plain”]},
...
{id:“usine_robots”,name:“Usine Robots”,icon:“🤖”,base:{metal:400,crystal:120,deut:40},factor:2.0,produces:“speed”,terrainBonus:[“plain”,“mountain”]},
{id:“hangar”,name:“Hangar”,icon:“🚀”,base:{metal:400,crystal:200,deut:100},factor:2.0,produces:“ships”,terrainBonus:[“plain”,“coast”,“desert”]},
];
const SHIPS=[
...
const bTime=(b,lv,rl)=>{const c=bCost(b,lv);return Math.max(30+lv*20,Math.round((c.metal+c.crystal)/(200*(1+(rl||0)))));};
const prodH=(lv,base,bonus)=>Math.round(base*lv*Math.pow(1.1,lv)*(1+(bonus||0)));
...
const eProd=(lv,planetType)=>{
const base=Math.round(20*lv*Math.pow(1.1,lv));
const passive=planetType&&PLANET_PASSIVE_BONUS&&PLANET_PASSIVE_BONUS[planetType];
const eb=(passive&&passive.energy)||0;
return Math.round(base*(1+eb));
};
const eRatio=(bld,planetType)=>{const n=eNeed(bld),p=eProd(bld.centrale||0,planetType);return n===0?1:Math.min(1,p/n);};
...
function getTerrainBonuses(zones,planetType){
...
for(const cont of continents){
if(placed)break;
const z=cont.zones.find(z=>z.terrain&&z.terrain.buildable&&qgDef.terrainBonus.includes(z.type))
||cont.zones.find(z=>z.terrain&&z.terrain.buildable);
if(z){z.building=“qg”;placed=true;}
}
const zones=continents.flatMap(c=>c.zones);
return{resources:{metal:500,crystal:300,deut:100},
buildings:{qg:1,mine_metal:0,mine_crystal:0,synth_deut:0,centrale:0,usine_robots:0,hangar:0},
...
if(s.buildings&&s.buildings.qg===undefined) s={…s,buildings:{qg:1,…s.buildings}};
if(!s.planetType&&planet) s={…s,planetType:planet.type};

const pt=planet&&planet.type;const now=Date.now(),dt=(now-s.lastTick)/3600000,ratio=eRatio(s.buildings,pt),tb=getTerrainBonuses(s.zones,pt);
...
const camDrag=useRef({on:false,x:0,y:0,theta:0.4,phi:0.8,vTheta:0,vPhi:0,dist:28,moved:false});
useEffect(()=>{
...
const nebCv=genNebulaTexture(gSeed,512),nebTex=new T.CanvasTexture(nebCv);
const nebMesh=new T.Mesh(new T.SphereGeometry(220,32,32),new T.MeshBasicMaterial({map:nebTex,side:T.BackSide,transparent:true,opacity:.55,depthWrite:false}));
scene.add(nebMesh);
[[5000,100,180,.12,.95,0xffffff],[3000,80,130,.18,.6,0xddeeff],[1500,60,100,.28,.4,0xaaccff],[800,50,80,.38,.3,0xffffcc]].forEach(([cnt,mn,mx,sz,op,col])=>{
...
scene.add(new T.Points(sg,new T.PointsMaterial({color:col,size:sz,transparent:true,opacity:op})));
});
...
[[2.0,.22,0xffee88],[3.2,.12,0xffcc44],[5.5,.06,0xff8800],[9,.03,0xff6600]].forEach(([r,op,col])=>{
...
const flareCv=document.createElement(“canvas”);flareCv.width=flareCv.height=64;
const fCtx=flareCv.getContext(“2d”),fGrad=fCtx.createRadialGradient(32,32,0,32,32,32);
fGrad.addColorStop(0,“rgba(255,248,200,1)”);fGrad.addColorStop(.3,“rgba(255,220,80,.6)”);fGrad.addColorStop(1,“rgba(255,150,0,0)”);
fCtx.fillStyle=fGrad;fCtx.fillRect(0,0,64,64);
const flare=new T.Sprite(new T.SpriteMaterial({map:new T.CanvasTexture(flareCv),transparent:true,opacity:.6,blending:T.AdditiveBlending,depthTest:false}));
flare.scale.set(6,6,1);scene.add(flare);
scene.add(new T.PointLight(0xfff5e0,2.5,80));scene.add(new T.AmbientLight(0x030510,.8));
planets.forEach(p=>{
const pts=[];
...
const planetMeshes=[];
planets.forEach((p,i)=>{
const size=0.14+(p.size/15000)*0.24;
...
cd.on=true;cd.moved=false;cd.startedHere=true;
...
cd.dist=Math.max(6,Math.min(55,cd.dist+delta*.06));
}
cd.pinchDist=dist;
return;
}
...
const oU=e=>{
const cd=camDrag.current;
cd.pinchDist=null; // reset pinch on finger lift
cd.on=false;
if(!cd.moved&&cd.startedHere){
...
if(hits.length>0){const idx2=planetMeshes.findIndex(pm=>pm.mesh===hits[0].object);if(idx2>=0)onSelect(idx2);}
}
cd.startedHere=false;
};
const onWheel=e=>{const cd=camDrag.current;cd.dist=Math.max(6,Math.min(55,cd.dist+e.deltaY*.025));};
...
const cx2=cd.dist*Math.sin(cd.phi)*Math.cos(cd.theta),cy2=cd.dist*Math.cos(cd.phi),cz2=cd.dist*Math.sin(cd.phi)*Math.sin(cd.theta);
cam.position.set(cx2,cy2,cz2);cam.lookAt(0,0,0);
...
const pulse=.95+Math.sin(t*1.8)*.05;sunMesh.scale.setScalar(pulse);flare.material.opacity=.5+Math.sin(t*2.2)*.1;
...
// Label at centroid
ctx.save();
ctx.font=“bold “+(sz/36)+“px monospace”;
ctx.textAlign=“center”;ctx.textBaseline=“middle”;
var labelText=(cont.name||””).toUpperCase();
// Dark outer shadow for legibility
ctx.shadowColor=“rgba(0,0,0,0.95)”;ctx.shadowBlur=sz/45;
ctx.fillStyle=“rgba(0,0,0,0.9)”;
ctx.fillText(labelText,lx,ly);
// Bright core text with colored glow
ctx.shadowColor=“rgba(”+Math.min(255,cr+120)+”,”+Math.min(255,cg+120)+”,”+Math.min(255,cb+120)+”,0.85)”;
ctx.shadowBlur=sz/90;
...
function Planet3D({planet,zones,continents,onZoneClick,isMyPlanet,fs,onBuildingClick,buildings,queue,onFinalizeQueue,onContinentClick}){
const mountRef=useRef(),stateRef=useRef({}),dragRef=useRef({on:false,x:0,y:0,rx:0,ry:0,vx:0,vy:0,moved:false});
...
const cnt=layer===0?3000:1500,sp=new Float32Array(cnt*3),rad=layer===0?40:28;
for(let i=0;i<cnt*3;i+=3){const a=Math.random()*Math.PI*2,b=Math.acos(2*Math.random()-1),r=rad+Math.random()*15;sp[i]=r*Math.sin(b)*Math.cos(a);sp[i+1]=r*Math.sin(b)*Math.sin(a);sp[i+2]=r*Math.cos(b);}
const sg=new T.BufferGeometry();sg.setAttribute(“position”,new T.BufferAttribute(sp,3));
scene.add(new T.Points(sg,new T.PointsMaterial({color:layer===0?0xffffff:0xaaccff,size:layer===0?0.048:0.06,transparent:true,opacity:layer===0?0.8:0.5})));
}
if(!texRef.current)texRef.current=genSurfaceTexture(planet,1024);
...
spike.position.set(cfg.x,cfg.h/2+s*.15,cfg.z);
spike.rotation.z=cfg.rot;spike.rotation.x=cfg.rot*.5;
spike.userData={crystalI:ci,baseEmit:.3};g.add(spike);
// Inner light core
const core=basic3(new T.ConeGeometry(cfg.r*.3,cfg.h*.8,6),cfg.col,{tr:true,op:.4,add:true});
core.position.copy(spike.position);core.rotation.copy(spike.rotation);g.add(core);
});
...
ch.material.emissiveIntensity=.2+Math.sin(t*2+ch.userData.crystalI*1.1)*.2;
ch.material.opacity=.82+Math.sin(t*1.5+ch.userData.crystalI*.8)*.08;
}
});
};
...
// 3 rows of solar panels at angles
const panelConfigs=[
{x:-s*.7,y:s*.4,z:0,rx:-.3,ry:0},
{x:0,y:s*.55,z:0,rx:-.15,ry:0},
{x:s*.7,y:s*.4,z:0,rx:-.3,ry:0},
];
const panels=[];
...
frame.position.set(cfg.x,cfg.y,cfg.z);frame.rotation.set(cfg.rx,cfg.ry,0);g.add(frame);
// Panel surface (blue solar cells)
const panel=mesh3(new T.BoxGeometry(s*.5,s*.3,.006),0x1144aa,{emit:0x0033aa,emitI:.3,shine:100});
panel.position.set(cfg.x,cfg.y,.003);panel.rotation.set(cfg.rx,cfg.ry,0);g.add(panel);
// Cell grid lines
[-.16,0,.16].forEach(cx=>{
const line=basic3(new T.BoxGeometry(.002,s*.28,.007),0x3366cc,{tr:true,op:.5});
line.position.set(cfg.x+cx,cfg.y,.005);line.rotation.set(cfg.rx,cfg.ry,0);g.add(line);
});
// Support arm
...
g.userData._anim=(t)=>{
panels.forEach((p,pi)=>{p.material.emissiveIntensity=.25+Math.sin(t*1.5+pi*.8)*.15;});
...
add.call({add:(m)=>{ship.add(m);return m;}},mesh3(new T.ConeGeometry(s*.18,s*.5,5),0x6677aa,{emit:0x223355,emitI:.2}),[0,s*.25,0],[Math.PI/2,0,0]);
add.call({add:(m)=>{ship.add(m);return m;}},mesh3(new T.BoxGeometry(s*.5,s*.04,s*.18),0x556688),[0,s*.15,0]);
// Landing lights
const lights=[];
for(let l=0;l<4;l++){
const a=l*Math.PI/2;
...
g.userData._anim=(t)=>{
ship.position.y=s*.12+Math.sin(t*.8)*.015;
ship.rotation.y=t*.2;
lights.forEach(lt=>{lt.material.opacity=.4+Math.sin(t*3+lt.userData.ltPhase)*.5;});
};
}

// Apply level scale (not for QG which handles its own)
if(buildingId!==“qg”)g.scale.setScalar(1+lv*0.07);
return g;
}
...
if(isMyPlanet&&zones){
  ...
  zones.forEach((zone,zi)=>{
    ...
    if(zone.building){
      // ── OCCUPIED ZONE: building + halo underneath ──
      const bldGroup=makeBuildingGroup(T,zone.building,buildingsRef.current&&buildingsRef.current[zone.building]||0,false,1);
      bldGroup.position.copy(normal.clone().multiplyScalar(1.01));
      bldGroup.setRotationFromQuaternion(alignQ);
      bldGroup.userData={buildingId:zone.building,zoneId:zi};
      mesh.add(bldGroup);
      markers.push(bldGroup);
      // Glowing halo disc under building
      const haloGeo=new T.CircleGeometry(0.09,32);
      ...
      buildingAnims.push({update:(t)=>{
        ...
        halo.material.opacity=fade*(0.18+Math.sin(t*1.8+halo.userData.phase)*0.08);
      }});
    } else {
  ...
  buildingAnims.push({update:(t)=>{
    ...
    if(ready){
      drawTimer("Cliquer pour finaliser",true);
...
const sun=new T.DirectionalLight(0xfff5e0,2.0);sun.position.set(5,2,4);scene.add(sun);
scene.add(new T.DirectionalLight(0x0a1428,.25).position.set(-4,-2,-3));
scene.add(new T.AmbientLight(0x0d1520,.45));
dragRef.current.rx=stateRef.current.rx||0;dragRef.current.ry=stateRef.current.ry||0;
let t=stateRef.current.t||0,af;
const easeOut=x=>1-Math.pow(1-x,3);
const loop=()=>{
  af=requestAnimationFrame(loop);t+=.004;
  if(!introDone){introT+=.016/INTRO_DUR;if(introT>=1){introT=1;introDone=true;}cam.position.z=CAM_START+(CAM_END-CAM_START)*easeOut(introT);}
  const dr=dragRef.current;
  if(!dr.on){dr.vx*=.94;dr.vy*=.94;dr.ry+=dr.vx;dr.rx+=dr.vy;}
  dr.rx=Math.max(-1.4,Math.min(1.4,dr.rx));
  mesh.rotation.y=dr.ry+t*.08;mesh.rotation.x=dr.rx;
  if(cloudMesh){cloudMesh.rotation.y=dr.ry+t*.11;cloudMesh.rotation.x=dr.rx;}
  ...
  // Fade continent labels based on camera distance
  continentLabelSprites.forEach(s=>{
    const d=cam.position.distanceTo(s.position);
    if(s.userData&&s.userData.labelMat)s.userData.labelMat.opacity=d<3.5?Math.min(1,(3.5-d)/1):0;
  });
  // Also call _anim on building groups
  markers.forEach(m=>{if(m.userData&&m.userData._anim)m.userData._anim(t);});
...
const raycaster=new T.Raycaster();
// Store continent centroids for nearest-neighbor click selection on the sphere
// (Voronoi-style: each click goes to the continent with the closest centroid)
if(isMyPlanet&&continents&&continents.length){continents.forEach(cont=>{
  var hit={position:new T.Vector3(cont.cx,cont.cy,cont.cz),userData:{contId:cont.id}};
  contHits.push(hit);
});}
const oU=e=>{
  const dr=dragRef.current;
  ...
  if(wasOn&&!dr.moved&&isMyPlanet){
    ...
    const mx=((e.clientX-rect.left)/rect.width)*2-1,my=-((e.clientY-rect.top)/rect.height)*2+1;
    raycaster.setFromCamera({x:mx,y:my},cam);
    if(onContinentClick&&contHits.length>0){
      var planetHit=raycaster.intersectObject(mesh,false);
      if(planetHit.length>0){
        var localPt=mesh.worldToLocal(planetHit[0].point.clone()).normalize();
        // Pick the continent whose centroid has the highest dot-product = nearest on sphere
        var bestCont=null,bestDot=-2;
        contHits.forEach(function(h){
          var cp=h.position.clone().normalize();
          var d=localPt.dot(cp);
          if(d>bestDot){bestDot=d;bestCont=h;}
        });
        if(bestCont){onContinentClick(bestCont.userData.contId);return;}
      }
    }
    if(markers.length>0){
...
return()=>{cancelAnimationFrame(af);ro2.disconnect();renderer.domElement.removeEventListener("mousedown",oD);renderer.domElement.removeEventListener("touchstart",oD);renderer.domElement.removeEventListener("mousemove",oM);renderer.domElement.removeEventListener("touchmove",oM);renderer.domElement.removeEventListener("mouseup",oU);renderer.domElement.removeEventListener("touchend",oU);window.removeEventListener("mousemove",oM);window.removeEventListener("mouseup",oU);renderer.dispose();while(div.firstChild)div.removeChild(div.firstChild);};
```
...
const bestBlds=BUILDINGS.filter(b=>b.terrainBonus&&b.terrainBonus.includes(zone.type)&&b.id!==“qg”);
...
const _rs0={position:“fixed”,top:0,left:0,right:0,bottom:0,zIndex:500,display:“flex”,alignItems:“center”,justifyContent:“center”,padding:“12px”,background:“rgba(0,0,0,.55)”};
return(<div onClick={onClose} style={_rs0}>
<div onClick={e=>e.stopPropagation()} style={{width:“min(280px,92vw)”,maxHeight:“80vh”,overflowY:“auto”,WebkitOverflowScrolling:“touch”,background:“linear-gradient(160deg,#07101f,#0a1628)”,border:“1px solid #1a3050”,borderRadius:16,padding:18,fontFamily:“monospace”,color:”#9abcd8”,boxShadow:“0 0 40px rgba(0,20,80,.9)”}}>
...
const col=k===“metal”?”#ffaa44”:k===“crystal”?”#44ccff”:”#44ffaa”;
const ic=k===“metal”?“⛏️”:k===“crystal”?“💎”:“⚗️”;
...
<div>
<div style={{fontSize:11,color:col,fontFamily:“monospace”,fontWeight:“bold”}}>{”+”+pct+”%”}</div>
<div style={{fontSize:8,color:”#2a4a6a”}}>{k===“metal”?“production de métal”:k===“crystal”?“production de cristal”:“production de deutérium”}</div>
</div>
</div>
);
})
:<div style={{fontSize:9,color:”#2a4a6a”,padding:“5px 8px”}}>{“Aucun bonus de terrain”}</div>
}
</div>
{bestBlds.length>0&&(
<div>
...
{bv&&<div style={{fontSize:8,color:”#3a7a54”}}>{“✓ +”+Math.round(bv*100)+”% sur ce terrain”}</div>}
</div>
</div>
);
})}
...
function BuildingInfoModal({bld,zone,lv,state,onUpgrade,onClose}){
if(!bld||!zone)return null;
const cost=bCost(bld,lv);
const canUp=canAfford(state.resources,cost)&&!state.queue;
const rl=state.buildings.usine_robots||0;
const secs=bTime(bld,lv,rl);
const isBonus=(bld.terrainBonus&&bld.terrainBonus.includes(zone.type));
const tb=getTerrainBonuses(state.zones||[]);
const bon=tb[bld.produces]||0;
const prodInfo=bld.produces===“metal”?fmt(prodH(lv,30,bon))+”/h metal”:
bld.produces===“crystal”?fmt(prodH(lv,20,bon))+”/h cristal”:
bld.produces===“deut”?fmt(prodH(lv,10,bon))+”/h deuterium”:
bld.produces===“energy”?“E “+fmt(eProd(lv))+” energie”:
bld.produces===“speed”?“x”+(lv+1)+” vitesse”:“Niveau “+lv;
const _rs1={position:“fixed”,top:0,left:0,right:0,bottom:0,zIndex:500,display:“flex”,alignItems:“center”,justifyContent:“center”,padding:“12px”,background:“rgba(0,0,0,.6)”};
return(<div onClick={onClose} style={_rs1}>
<div onClick={e=>e.stopPropagation()} style={{width:“min(300px,92vw)”,maxHeight:“80vh”,overflowY:“auto”,WebkitOverflowScrolling:“touch”,background:“linear-gradient(160deg,#07101f,#0a1628)”,border:“1px solid #1a3050”,borderRadius:16,padding:20,fontFamily:“monospace”,color:”#9abcd8”,boxShadow:“0 0 40px rgba(0,20,80,.9)”}}>
...
bld.produces===“energy”?“E+”+fmt(eProd(lv+1)):
...
const _vs4={width:“100%”,padding:“8px 0”,borderRadius:8,border:“none”,cursor:“pointer”,background:“linear-gradient(135deg,#0d2860,#0a1840)”,color:”#6090c8”,fontFamily:“monospace”,fontSize:9};
const _vs5={width:“100%”,padding:“7px 0”,borderRadius:8,border:“1px solid #0a1828”,cursor:“pointer”,background:“transparent”,color:”#1e3050”,fontFamily:“monospace”,fontSize:9};
function Viewer3D({planet,onClose,isPlayer,owner,state,onStateChange,onManage}){
...
if(!onStateChange||!state)return;
const cost=bCost(b,lv2);
const secs=bTime(b,lv2,state.buildings.usine_robots||0);
...
const hdrSt={display:“flex”,alignItems:“center”,justifyContent:“space-between”,padding:“4px 10px”,background:“rgba(4,10,20,.97)”,flexShrink:0,borderBottom:“1px solid #0a1828”};
const btnSt={padding:“3px 8px”,background:“transparent”,border:“1px solid #0d2040”,color:”#4a6a8a”,borderRadius:6,cursor:“pointer”,fontSize:11};
const bodySt={flex:1,display:“flex”,overflow:“hidden”,minHeight:0};
const globeSt={flex:“0 0 58%”,position:“relative”,overflow:“hidden”};
const panelSt={flex:“0 0 42%”,borderLeft:“1px solid #0a1828”,overflow:“hidden”,display:“flex”,flexDirection:“column”};
const scrollSt={height:“100%”,overflowY:“scroll”,overflowX:“hidden”,display:“flex”,flexDirection:“column”,gap:0};
const tabRowSt={display:“flex”,gap:1,padding:“4px”,background:”#040c18”,flexShrink:0};
const noLoadSt={display:“flex”,alignItems:“center”,justifyContent:“center”,height:“100%”,color:”#1e3050”,fontFamily:“monospace”,fontSize:11};
const _fsClick=()=>{const el=document.documentElement;(el.requestFullscreen||el.webkitRequestFullscreen||el.mozRequestFullScreen||function(){}).call(el);};
return(<div style={outerSt}>
<div style={hdrSt}>
<div>
<span style={{fontFamily:“monospace”,fontSize:13,color:”#c8e0ff”,letterSpacing:1}}>{(state&&state.planetName)||planet.name}</span>
<span style={{fontSize:9,color:”#2a4a6a”,marginLeft:8}}>{planet.type}{PLANET_PASSIVE_BONUS[planet.type]?” · “+PLANET_PASSIVE_BONUS[planet.type].label:””}{isPlayer?” ⭐”:””}{owner?” · 👤”+owner:””}</span>
</div>
<div style={{display:“flex”,gap:5}}>
<button onClick={_fsClick} style={btnSt}>{“⛶”}</button>
...
<div style={scrollSt}>
<div style={tabRowSt}>
{[[“info”,“📊”],[“zones”,“🗺️”]].map((tb)=>(<button key={tb[0]}
onClick={()=>setViewTab(tb[0])}
style={{flex:1,padding:“5px 0”,fontSize:9,border:“none”,cursor:“pointer”,background:viewTab===tb[0]?”#0a1828”:“transparent”,color:viewTab===tb[0]?”#7090b8”:”#1e3050”,fontFamily:“monospace”}}>
{tb[1]}{” “}{tb[0]===“info”?“Infos”:“Zones”}
</button>))}
</div>
{qActive&&(<div style={{margin:“6px 8px”,padding:“8px 10px”,borderRadius:8,background:“rgba(8,18,40,.9)”,border:“1px solid #0d2040”}}>
{qReady?<button onClick={handleFinalize} style={_vs1}>
{“✓ TERMINER : “}{qBDef&&qBDef.name}
</button>
:<div style={{fontSize:9,color:”#88aadd”,textAlign:“center”}}>
{“🔧 “}{qBDef&&qBDef.name}{” — “}{Math.floor(qRemain/3600000)}{“h”}{Math.floor((qRemain%3600000)/60000)}{“m”}{Math.floor((qRemain%60000)/1000)}{“s”}
</div>}
</div>)}
{viewTab===“info”&&(<div style={{padding:“10px 12px”}}>
<div style={{display:“flex”,alignItems:“center”,justifyContent:“space-between”,padding:“8px 12px”,borderRadius:10,background:“linear-gradient(135deg,rgba(10,25,60,.7),rgba(6,16,40,.9))”,border:“1px solid “+hab.c+“44”,marginBottom:12,boxShadow:“0 0 12px “+hab.c+“22”}}>
<span style={{fontSize:9,color:”#6a8aaa”,fontFamily:“monospace”,letterSpacing:1.5}}>{“HABITABILITÉ”}</span>
<span style={{fontSize:11,color:hab.c,fontFamily:“monospace”,fontWeight:“bold”,letterSpacing:1}}>{hab.l.toUpperCase()}</span>
</div>

```
        {PLANET_PASSIVE_BONUS[planet.type]&&(
          <div style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px",borderRadius:10,background:"linear-gradient(135deg,rgba(60,40,10,.3),rgba(30,22,6,.5))",border:"1px solid rgba(184,138,90,0.35)",marginBottom:14}}>
            <span style={{fontSize:14,color:"#e8c878"}}>{"✦"}</span>
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:8,color:"#8a6a3a",fontFamily:"monospace",letterSpacing:1.5,marginBottom:2}}>{"BONUS PLANÉTAIRE"}</div>
              <div style={{fontSize:11,color:"#e8c878",fontFamily:"serif",fontStyle:"italic"}}>{PLANET_PASSIVE_BONUS[planet.type].label}</div>
            </div>
          </div>
        )}

        <div style={{marginBottom:12}}>
          <div style={{fontSize:8,color:"#2a5080",letterSpacing:2,marginBottom:6,fontFamily:"monospace"}}>{"— IDENTITÉ —"}</div>
          {[
            ["Type",planet.type],
            ["Distance","UA "+planet.dist],
            ["Superficie",planet.size.toLocaleString()+" km²"],
            ["Lunes",planet.moons||0],
          ].map((row)=>(
            <div key={row[0]} style={{display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:10,padding:"5px 0",borderBottom:"1px solid rgba(10,20,40,.6)"}}>
              <span style={{color:"#5a7a9a",fontFamily:"serif"}}>{row[0]}</span>
              <span style={{color:"#a8c8e8",fontFamily:"monospace",fontSize:10}}>{row[1]}</span>
            </div>
          ))}
        </div>

        <div style={{marginBottom:12}}>
          <div style={{fontSize:8,color:"#2a5080",letterSpacing:2,marginBottom:6,fontFamily:"monospace"}}>{"— CLIMAT —"}</div>
          {[
            ["Température",(planet.temp>0?"+":"")+planet.temp+"°C",planet.temp>60||planet.temp<-50?"#c06030":planet.temp>-20&&planet.temp<60?"#40c870":"#c0b030"],
            ["Eau",planet.water+"%",planet.water>20&&planet.water<80?"#40a8c8":"#8ab0cc"],
            ["Atmosphère",planet.atmos||"Aucune",planet.atmos==="Respirable"?"#40c870":"#8ab0cc"],
            ["Luminosité",planet.light+"%",planet.light>20&&planet.light<80?"#e8c878":"#8ab0cc"],
          ].map((row)=>(
            <div key={row[0]} style={{display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:10,padding:"5px 0",borderBottom:"1px solid rgba(10,20,40,.6)"}}>
              <span style={{color:"#5a7a9a",fontFamily:"serif"}}>{row[0]}</span>
              <span style={{color:row[2]||"#a8c8e8",fontFamily:"monospace",fontSize:10}}>{row[1]}</span>
            </div>
          ))}
        </div>
        ...
        {isPlayer&&state&&(
          <div style={{marginBottom:8}}>
            <div style={{fontSize:8,color:"#2a5080",letterSpacing:2,marginBottom:6,fontFamily:"monospace"}}>{"— RESSOURCES —"}</div>
            {[
              ["⛏️","Métal",state.resources.metal,"#7090b8"],
              ["💎","Cristal",state.resources.crystal,"#40a8c8"],
              ["⚗️","Deutérium",state.resources.deut,"#40c890"]
            ].map((r)=>(
              <div key={r[1]} style={{display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:10,padding:"5px 0",borderBottom:"1px solid rgba(10,20,40,.6)"}}>
                <span style={{color:"#5a7a9a",fontFamily:"serif"}}><span style={{marginRight:6}}>{r[0]}</span>{r[1]}</span>
                <span style={{color:r[3],fontFamily:"monospace",fontSize:11,fontWeight:"bold"}}>{fmt(r[2])}</span>
              </div>
            ))}
          </div>
        )}
      </div>)}
      {viewTab==="zones"&&(<div style={{padding:"6px 8px"}}>
          {zones.map(z=>{
            ...
            return(<div key={z.id}
                ...
                style={_vs3}>
                <span style={{fontSize:11}}>{z.terrain&&z.terrain.emoji||"🌍"}</span>
                <div style={{flex:1}}>
                  <div style={{fontSize:9,color:"#6a8aaa"}}>{z.name}</div>
                  {bDef&&<div style={{fontSize:8,color:"#ffd700"}}>{bDef.icon}{" "}{bDef.name}</div>}
                </div>
              </div>);
          })}
        </div>)}
      {isPlayer&&(<div style={{padding:"6px 8px",marginTop:"auto",flexShrink:0,display:"flex",flexDirection:"column",gap:4}}>
          {isPlayer&&onManage&&<button onClick={()=>{AudioEngine.safeClick();onManage();}} style={_vs4}>{"🏗 GÉRER LA PLANÈTE"}</button>}
...
{selContinent&&<ContinentModal continent={selContinent} state={state||{planetType:planet&&planet.type,buildings:{},resources:{metal:0,crystal:0,deut:0},continents:conts}} onBuildInZone={isPlayer?handleBuildInZone:()=>{}} onClose={()=>setSelContinent(null)} readOnly={!isPlayer}/>}
```
...
const ratio=eRatio(bld),ep=eProd(bld.centrale||0),en=eNeed(bld);
const ec=ratio<0.5?”#e05030”:ratio<1?”#e0c030”:”#40c870”;
const tb=getTerrainBonuses(zones||[]);
return(<div style={{display:“flex”,gap:8,flexWrap:“wrap”,padding:“5px 10px”,background:“rgba(6,15,28,.95)”,borderBottom:“1px solid #0a1828”,fontFamily:“monospace”,fontSize:10,alignItems:“center”,flexShrink:0}}>
{[[“⛏️”,res.metal,”#7090b8”,tb.metal],[“💎”,res.crystal,”#40a8c8”,tb.crystal],[“⚗️”,res.deut,”#40c890”,tb.deut]].map(([ic,v,c,bon])=>(
<div key={ic} style={{display:“flex”,alignItems:“center”,gap:2}}>
<span>{ic}</span>
<span style={{color:c,fontWeight:“bold”}}>{fmt(v)}</span>
{bon>0&&<span style={{fontSize:7,color:”#40c870”}}>{”+”}{ Math.round(bon*100)}{”%”}</span>}
</div>
))}
<div style={{display:“flex”,alignItems:“center”,gap:3,marginLeft:“auto”}}>
<span>{“⚡”}</span>
<span style={{color:ec}}>{ep}{”/”}{en}</span>
{ratio<1&&<span style={{color:”#e08030”,fontSize:8}}>{”(”}{Math.round(ratio*100)}{”%)”}</span>}
</div>
</div>
);
}
...
<span style={{color:”#2a4060”,marginLeft:“auto”}}>{“⏱ “}{fmtT(bTime(qgDef,qgLv,rl))}</span>
</div>
...
const info=b.produces===“metal”?”+”+fmt(prodH(lv+1,30,bon))+”/h”:b.produces===“crystal”?”+”+fmt(prodH(lv+1,20,bon))+”/h”:b.produces===“deut”?”+”+fmt(prodH(lv+1,10,bon))+”/h”:b.produces===“energy”?“E+”+fmt(eProd(lv+1)):b.produces===“speed”?“x”+(lv+2)+” vit”:“Niv.”+(lv+1);
...
const secs=bTime(b,lv,prev.buildings.usine_robots||0);
...
const secs=Math.max(60,Math.round((tc.metal+tc.crystal)/(200*(1+(prev.buildings.hangar||0)))));
...
const ns={…prev,ships:ns2,fleets:[…(prev.fleets||[]),f]};
saveGS(userId,ns);onStateChange&&onStateChange(ns);
return ns;
});
};
...
const TABS=[[“buildings”,“🏗️”,“Bâtiments”],[“ships”,“🛸”,“Vaisseaux”],[“fleet”,“🚀”,“Flottes”]];
...
```
        ...
        <div style={{display:"flex",alignItems:"center",gap:5}}>
          {renaming
            ?<input autoFocus value={nameInput} onChange={e=>setNameInput(e.target.value)}
                ...
                style={{flex:1,background:"#0a1828",border:"1px solid #2255aa",borderRadius:5,color:"#c8e0ff",fontSize:11,padding:"2px 6px",fontFamily:"monospace",outline:"none"}}/>
  ...
  <ResBar res={state.resources} bld={state.buildings} zones={state.zones} planetType={planet&&planet.type}/>
  {state.queue&&(()=>{
    const q=state.queue;
    const ready=q.ready;
...
const INP={width:“100%”,padding:“9px 12px”,background:”#060f1c”,border:“1px solid #0d2035”,borderRadius:8,color:”#9abcd8”,fontSize:13,fontFamily:“monospace”,outline:“none”};
...
function ResourceInfoModal({type,state,onClose}){
if(!type||!state)return null;
const tb=getTerrainBonuses(state.zones||[]);
const bld=state.buildings||{};
const ratio=eRatio(bld);
...
const prodPerH=prodH(lv,info.prodBase,bon)*ratio;
...
<div style={{fontSize:9,color:”#2a4a6a”}}>{“Stock : “}{fmt(state.resources[type]||0)}</div>
</div>
</div>
...
<div style={{marginBottom:10}}>
<div style={{fontSize:8,color:”#1e3050”,letterSpacing:2,marginBottom:6}}>{“PRODUCTION”}</div>
...
<div>
<div style={{fontSize:11,color:info.color,fontFamily:“monospace”,fontWeight:“bold”}}>{fmt(prodPerH)}{”/h”}</div>
<div style={{fontSize:8,color:”#2a4060”}}>{“niveau “}{lv}</div>
</div>
...
{ratio<1&&<div style={{fontSize:9,color:”#e08030”}}>{“⚡ “}{Math.round(ratio*100)}{”% efficacité”}</div>}
</div>
)}
</div>
</div>
...
const pick=i=>{AudioEngine.safeClick();setViewer(planets[i]);};
...
return(<div style={{width:“100vw”,height:“100dvh”,background:”#000005”,display:“flex”,flexDirection:“column”,overflow:“hidden”,paddingLeft:safeL,paddingRight:safeR}}>
{selResource&&myState&&<ResourceInfoModal type={selResource} state={{…myState,planetType:mine.type}} onClose={()=>setSelResource(null)}/>}
{viewer&&<Viewer3D planet={viewer} onClose={()=>setViewer(null)} isPlayer={viewer.i===idx} owner={(lp.find(p=>p.idx===viewer.i)&&lp.find(p=>p.idx===viewer.i).name)} state={viewer.i===idx?myState:null} onStateChange={viewer.i===idx?handleStateChange:null} onManage={viewer.i===idx?()=>{setViewer(null);setPanel(true);}:null}/>}
...
<div style={{display:“flex”,alignItems:“center”,justifyContent:“space-between”,padding:“4px 10px”,background:“rgba(4,12,24,.95)”,borderBottom:“1px solid #0a1828”,flexShrink:0,height:38}}>
...
<div style={{position:“absolute”,bottom:6,left:0,right:0,textAlign:“center”,fontSize:8,color:”#1a2a3a”,pointerEvents:“none”}}>GLISSER · SCROLL · CLIQUER</div>
</div>
)}
{tab===“players”&&(
...
<div style={{display:“flex”,borderTop:“1px solid #0a1828”,background:“rgba(4,12,24,.97)”,flexShrink:0,paddingBottom:“env(safe-area-inset-bottom,0px)”}}>
...
s.textContent=”@media(orientation:portrait){#portrait-lock{display:flex!important;}}@media(orientation:landscape){#portrait-lock{display:none!important;}}@keyframes fadeIn{from{opacity:0}to{opacity:1}}@keyframes twinkle{0%,100%{opacity:.2}50%{opacity:.8}}@keyframes pulse{0%,100%{box-shadow:0 0 12px rgba(68,255,170,.4)}50%{box-shadow:0 0 24px rgba(68,255,170,.9)}}*{box-sizing:border-box;margin:0;padding:0}html,body{background:#000005;height:100%;overflow:hidden}::-webkit-scrollbar{width:3px;height:3px}::-webkit-scrollbar-thumb{background:#0d2035;border-radius:2px}input,button{-webkit-tap-highlight-color:transparent;}@import url(https://fonts.googleapis.com/css2?family=Share+Tech+Mono&display=swap);”;
document.head.appendChild(s);
