...
function mkP(gS,i){
  ...
  const colorVar=r();
  return{seed:s,i,name:mkName(s),type,dist:d,temp:Math.round(400-d*80+(r()-0.5)*60),light:Math.max(5,Math.round(100/(d*d)*(0.7+r()*0.6))),size:3000+Math.floor(r()*12000),water:w,atmos:ATMOS[Math.floor(r()*ATMOS.length)],moons:Math.floor(r()*r()*5),ringsUnlocked:false,moonsUnlocked:false,tilt:r()*40,clouds:type==="Gazeuse"?0:type==="Glaciale"?0.55:type==="Désertique"?0.08:0.3+r()*0.4,hasRings,ringTilt:r()*0.6-0.3,colorVar,orbitAngle:(i/50)*Math.PI*2+r()*0.3,orbitRadius:80+(i/50)*340,orbitRadius3d:4+r()*18,orbitInc:r()*Math.PI*0.6-Math.PI*0.3,orbitAsc:r()*Math.PI*2,orbitAngle3d:r()*Math.PI*2,orbitSpeed:0.08+r()*0.18};
}
...
function genNebulaTexture(seed,sz=1024){
  ...
  // Richer, more vibrant nebula themes inspired by Hubble/James Webb imagery
  const themes=[
  ...
  for(let py=0;py<sz;py++){for(let px=0;px<sz;px++){
    const u=px/sz*2,v=py/sz*2;
    const n=fbm(n1,u,v,6,2.0,0.55);
    const n_=fbm(n2,u+n*.8,v+n*.8,5,2.1,0.5);
    const density=Math.max(0,n_*.6+.35);
    const filament=fbm(n3,u*1.5+2,v*1.5+2,4)*.5+.5;
    // Hot spots — bright areas where stars are being born
    const hotspot=Math.pow(Math.max(0,fbm(n4,u*0.8+5,v*0.8+5,3)),3)*1.5;
    const alpha=Math.pow(density,1.8)*filament*0.55+hotspot*0.4;
    ...
    // Accent color mixed in hot zones
    r+=hotspot*(th.accent[0]-r)*0.6;
    g+=hotspot*(th.accent[1]-g)*0.6;
    b+=hotspot*(th.accent[2]-b)*0.6;
    const idx=(py*sz+px)*4;d[idx]=clamp(r,0,255);d[idx+1]=clamp(g,0,255);d[idx+2]=clamp(b,0,255);d[idx+3]=Math.round(Math.min(1,alpha)*255);
  }}ctx.putImageData(img,0,0);return cv;
}
...
function GalaxyMap3D({planets,playerIdx,onSelect,gSeed}){
  ...
  return <div style={{width:"100%",height:"100%",position:"relative"}}>
    ...
    <button onClick={recenter} style={{position:"absolute",bottom:12,right:12,padding:"5px 10px",background:"rgba(8,14,24,0.85)",border:"1px solid #1a3050",borderRadius:6,color:"#9abcd8",fontSize:10,cursor:"pointer",zIndex:5,backdropFilter:"blur(4px)",fontFamily:"monospace"}}>☀ RECENTRER</button>
  </div>;
}
...
function Viewer3D({planet,onClose,isPlayer,owner,state,onStateChange,onManage}){
  ...
  const panelSt={flex:"0 0 42%",borderLeft:"1px solid #0a1828",overflow:"hidden",display:"flex",flexDirection:"column",display:focusMode?"none":"flex"};
...
function Dash({user,onOut}){
  ...
  const pick=i=>{AudioEngine.safeClick();setGalaxyFS(false);setViewer(planets[i]);};
