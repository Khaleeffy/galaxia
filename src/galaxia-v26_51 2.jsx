...
function BattlePlanScreen({plan,onCancel,onEngage}){
  ...
  return (
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"radial-gradient(ellipse at top,rgba(20,15,40,0.98) 0%,rgba(5,3,15,0.98) 80%)",zIndex:300,display:"flex",flexDirection:"column",overflow:"hidden",paddingTop:"env(safe-area-inset-top,0px)",paddingBottom:"env(safe-area-inset-bottom,0px)"}}>
...
function CombatScene({targetPlanet,attackerComp,defenderMilitary,testMode,onClose,onResolve}){
  var T=window.THREE;
  ...
  useEffect(function(){
    ...
    // ── Spawn attacker ships (player) — bottom-left of battlefield ──
    ...
    var spawnPos=[];
    // We'll spawn in rows on the player side
    var playerZ=40; // closer to center
    var rowZ=playerZ;
    var idx=0;
    for(var atk=0;atk<attackerTypes.length;atk++){
      ...
      for(var qi=0;qi<qty;qi++){
        ...
        // Distribute in a grid: 8 per row, spaced 12 units (room for bigger ships)
        var row=Math.floor(idx/8);
        var col=idx%8;
        ship.position.set((col-3.5)*12,0,rowZ+row*12);
        ship.rotation.y=Math.PI; // facing the enemy (toward planet)
        scene.add(ship);
...
function Dash({user,onOut}){
  ...
  return(<div style={{width:"100vw",height:"100svh",minHeight:"100dvh",maxHeight:"100dvh",background:"#000005",display:"flex",flexDirection:"column",overflow:"hidden",paddingBottom:"env(safe-area-inset-bottom,0px)"}}>
      ...
      {combatData&&threeOk&&<CombatScene targetPlanet={combatData.targetPlanet} attackerComp={combatData.attackerComp} defenderMilitary={combatData.defenderMilitary} testMode={combatData.testMode} onClose={()=>setCombatData(null)} onResolve={(result)=>{
