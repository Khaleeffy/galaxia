...
function CombatScene({targetPlanet,attackerComp,defenderMilitary,onClose,onResolve}){
  ...
  var [phase,setPhase]=useState("staging"); // staging | combat | result
  ...
  useEffect(function(){
    ...
    // ── Nebula glow: a few large soft sprites ──
    var nebulaColors=[0x4422aa,0x882244,0x224488];
    for(var n=0;n<3;n++){
      var ngeo=new T.SphereGeometry(80+Math.random()*40,8,8);
      var nmat=new T.MeshBasicMaterial({color:nebulaColors[n],transparent:true,opacity:0.06,blending:T.AdditiveBlending,side:T.DoubleSide});
      var nmesh=new T.Mesh(ngeo,nmat);
      nmesh.position.set((Math.random()-0.5)*400,(Math.random()-0.5)*100-50,(Math.random()-0.5)*400-200);
      scene.add(nmesh);
    }
