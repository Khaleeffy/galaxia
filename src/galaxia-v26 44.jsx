...
function CombatScene({targetPlanet,attackerComp,defenderMilitary,onClose,onResolve}){
  var T=window.THREE;
  ...
  return (
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"#000",zIndex:300,overflow:"hidden"}}>
      ...
      {combatResult&&<CombatResultModal result={combatResult} targetPlanet={targetPlanet} onClose={onClose}/>}
    </div>
  );
}

// CombatResultModal — VICTOIRE / DÉFAITE screen with losses summary
function CombatResultModal({result,targetPlanet,onClose}){
  ...
  return (
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.85)",zIndex:400,display:"flex",alignItems:"center",justifyContent:"center",padding:14,backdropFilter:"blur(8px)"}}>
      <div style={{maxWidth:480,width:"100%",maxHeight:"92vh",overflow:"auto",WebkitOverflowScrolling:"touch",background:isWin?"linear-gradient(135deg,rgba(20,60,40,0.97),rgba(10,40,25,0.97))":"linear-gradient(135deg,rgba(60,15,25,0.97),rgba(40,8,15,0.97))",border:"2px solid "+(isWin?"rgba(100,220,140,0.6)":"rgba(220,80,100,0.6)"),borderRadius:10,padding:20,boxShadow:"0 0 40px "+(isWin?"rgba(100,220,140,0.4)":"rgba(220,80,100,0.4)")}}>
        ...
        {/* Note about losses being purely visual for now */}
        <div style={{padding:8,background:"rgba(180,150,80,0.15)",border:"1px solid rgba(180,150,80,0.4)",borderRadius:5,marginBottom:14,fontSize:9,color:"#ddcc88",fontFamily:"monospace",lineHeight:1.5,textAlign:"center"}}>{"\u{2139}\uFE0F Pertes affichées uniquement (les pertes réelles seront appliquées en 3.6b)"}</div>
...
function AttackPrepModal({planet,natalState,attackComp,setAttackComp,onClose,onLaunch}){
...
function Viewer3D({planet,allPlanets,onClose,isPlayer,owner,state,onStateChange,onManage,userId,natalState,onColonize,alreadyColonized,onLaunchAttack}){
  ...
  return(<div style={outerSt}>
    ...
    {showAttackModal&&natalState&&<AttackPrepModal planet={planet} natalState={natalState} attackComp={attackComp} setAttackComp={setAttackComp} onClose={function(){setShowAttackModal(false);}} onLaunch={function(){setShowAttackModal(false);if(onLaunchAttack){onLaunchAttack(planet,attackComp);}}}/>}
...
function Dash({user,onOut}){
  ...
  return(<div style={{width:"100vw",height:"100svh",minHeight:"100dvh",maxHeight:"100dvh",background:"#000005",display:"flex",flexDirection:"column",overflow:"hidden",paddingBottom:"env(safe-area-inset-bottom,0px)"}}>
      ...
      {combatData&&threeOk&&<CombatScene targetPlanet={combatData.targetPlanet} attackerComp={combatData.attackerComp} defenderMilitary={combatData.defenderMilitary} onClose={()=>setCombatData(null)} onResolve={(result)=>{setCombatData(null);}}/>}
...
function AudioControls({onLogout}){
  ...
  const doCheat=()=>{
    alert("DEBUG: CHEAT clicked");
    if(!window.confirm("MODE CHEAT ?"))return;
    try{
