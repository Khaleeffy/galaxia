...
// ──────────────────────────────────────────────────────
// HubDebugWrapper — function component (no class) qui catch les errors via window.onerror
function HubDebugWrapper(props){
  var React=window.React||window.r;
  var useState=React?React.useState:null;
  var useEffect=React?React.useEffect:null;
  var caught=useState?useState(null):[null,function(){}];
  var hubError=caught[0];
  var setHubError=caught[1];
  // Install a global error handler when this wrapper mounts; remove on unmount
  if(useEffect){
    useEffect(function(){
      var prevHandler=window.onerror;
      var prevReject=window.onunhandledrejection;
      window.onerror=function(msg,url,line,col,err){
        try{setHubError({msg:String(msg),line:line,col:col,stack:err&&err.stack?String(err.stack).slice(0,500):"(no stack)"});}catch(e){}
        if(prevHandler)return prevHandler.apply(this,arguments);
        return false;
      };
      window.onunhandledrejection=function(e){
        try{setHubError({msg:"Promise rejected: "+String(e.reason),stack:e.reason&&e.reason.stack?String(e.reason.stack).slice(0,500):"(no stack)"});}catch(err){}
        if(prevReject)return prevReject.apply(this,arguments);
      };
      return function(){
        window.onerror=prevHandler;
        window.onunhandledrejection=prevReject;
      };
    },[]);
  }
  // If we caught an error, show debug screen
  if(hubError){
    var saveStr="";
    try{saveStr=localStorage.getItem("gx_state_"+props.userId)||"(introuvable)";}catch(e){saveStr="(erreur lecture)";}
    var copyAll=function(){
      var txt="ERREUR: "+hubError.msg+"\nLINE: "+hubError.line+"\nSTACK: "+hubError.stack+"\n\nSAVE:\n"+saveStr;
      try{
    ...
    return React.createElement("div",{style:{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,8,0.97)",zIndex:9999,padding:20,overflowY:"auto",fontFamily:"monospace",color:"#d8b8ff"}},
      React.createElement("div",{style:{maxWidth:600,margin:"0 auto"}},
        React.createElement("div",{style:{fontSize:18,fontWeight:"bold",color:"#ff6688",marginBottom:14}},"\u26A0 BUG DETECTE"),
        React.createElement("div",{style:{background:"rgba(80,30,30,0.5)",border:"1px solid #aa4466",borderRadius:8,padding:12,marginBottom:10,fontSize:11,wordBreak:"break-word"}},
          React.createElement("div",{style:{fontWeight:"bold",color:"#ffaabb"}},"Erreur :"),
          React.createElement("div",{style:{color:"#fff",marginTop:4}},hubError.msg),
          hubError.line?React.createElement("div",{style:{color:"#cc8899",marginTop:4,fontSize:9}},"Ligne "+hubError.line+":"+hubError.col):null
        ),
        hubError.stack?React.createElement("details",{style:{marginBottom:10,fontSize:10,color:"#9a8ac0"}},
          React.createElement("summary",{style:{cursor:"pointer",padding:6}},"Voir la stack"),
          React.createElement("pre",{style:{background:"#0a0612",padding:8,borderRadius:4,overflow:"auto",fontSize:9}},hubError.stack)
        ):null,
        React.createElement("button",{onClick:copyAll,style:{width:"100%",padding:"12px 0",background:"linear-gradient(135deg,rgba(40,80,140,0.95),rgba(20,50,90,0.95))",border:"1px solid #66aacc",borderRadius:8,color:"#fff",fontSize:12,fontWeight:"bold",cursor:"pointer",fontFamily:"monospace",letterSpacing:1,marginBottom:10}},
          "\u{1F4CB} COPIER ERREUR + SAVE"),
        React.createElement("button",{onClick:function(){setHubError(null);if(props.onClose)props.onClose();},style:{width:"100%",padding:"10px 0",background:"rgba(20,15,30,0.8)",border:"1px solid #4a3070",borderRadius:6,color:"#9a8ac0",fontSize:10,cursor:"pointer",fontFamily:"monospace",letterSpacing:1}},
          "FERMER")
      )
    );
  }
  // Otherwise render the actual HubSection
  try{
    return React.createElement(HubSection,props);
  }catch(e){
    setHubError&&setHubError({msg:String(e&&e.message||e),stack:e&&e.stack?String(e.stack).slice(0,500):"(no stack)"});
    return null;
  }
}
...
function Viewer3D({planet,allPlanets,onClose,isPlayer,owner,state,onStateChange,onManage,userId,natalState,onColonize,alreadyColonized}){
  ...
  return(<div style={outerSt}>
    ...
    {hubSection&&isPlayer&&state&&userId&&<HubDebugWrapper section={hubSection} planet={planet} userId={userId} planets={allPlanets} myIdx={planet&&planet.i} initialState={state} onStateChange={onStateChange} onClose={()=>setHubSection(null)} onOpenZoneInfo={z=>setSelHubZoneInfo(z)}/>}
