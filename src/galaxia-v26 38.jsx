...
function CombatScene({targetPlanet,attackerComp,defenderMilitary,onClose,onResolve}){
  ...
  useEffect(function(){
    ...
    function getShipCombatStats(type){
      if(type==="chasseur")return {hp:80,dps:35,range:55};
      if(type==="cargo")return {hp:120,dps:5,range:30};
      if(type==="croiseur")return {hp:400,dps:80,range:75};
      if(type==="bombardier")return {hp:300,dps:120,range:85};
      if(type==="cuirasse")return {hp:1500,dps:200,range:100};
      return {hp:80,dps:20,range:50};
    }
    function getDefenseCombatStats(type){
      if(type==="def_tourelle")return {hp:150,dps:30,range:65};
      if(type==="def_bouclier")return {hp:1000,dps:0,range:0};
      if(type==="def_missiles")return {hp:50,dps:200,range:120};
      if(type==="def_forteresse")return {hp:3000,dps:150,range:90};
