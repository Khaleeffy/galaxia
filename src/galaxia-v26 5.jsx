...
// ──────────────────────────────────────────────────────
function EmpireModal({userId,planets,natalIdx,activePlanetIdx,onSwitch,onClose}){
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"4px 10px",paddingLeft:"calc(10px + "+safeL+")",paddingRight:"calc(10px + "+safeR+")",background:"rgba(4,12,24,.95)",borderBottom:"1px solid #0a1828",flexShrink:0,height:38}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          {myState&&(()=>{
            ...
            return(<PlayerAvatar name={user.name} state={cosmeticState} size={28} onClick={()=>{AudioEngine.safeClick&&AudioEngine.safeClick();setShowProfile(true);}}/>);
          })()}
        </div>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          ...
          {myState&&(()=>{
            const natStForMissions=activePlanetIdx===null?myState:loadGS(user.id);
            if(!natStForMissions)return null;
            const activeMissions=getActiveMissions(natStForMissions);
            const readyCount=activeMissions.filter(m=>m.check(natStForMissions)).length;
            const branch=natStForMissions.missionFlags&&natStForMissions.missionFlags.chosenBranch;
          ...
          {(()=>{
            ...
            const showBtn=empCount>0;
            if(!showBtn)return null;
            return(
              <button onClick={()=>{AudioEngine.safeClick();setShowEmpire(true);}} style={{position:"relative",padding:"4px 8px",background:"rgba(100,160,240,0.15)",border:"1px solid #3060a0",borderRadius:6,color:"#aaccee",fontSize:9,cursor:"pointer",fontFamily:"monospace",letterSpacing:1,fontWeight:"bold"}}>
                {"🏛️ EMPIRE"}
                <span style={{marginLeft:4,padding:"1px 5px",background:"rgba(100,180,240,0.3)",borderRadius:8,fontSize:8}}>{empCount+1}</span>
              </button>
            );
          })()}
          <button onClick={()=>{AudioEngine.safeClick();setPanel(true);}} style={{padding:"4px 8px",background:activePlanetIdx!==null?"rgba(100,180,240,.1)":"rgba(255,215,0,.1)",border:"1px solid "+(activePlanetIdx!==null?"#1e4a80":"#3a3000"),borderRadius:6,color:activePlanetIdx!==null?"#aaccee":"#ffd700",fontSize:9,cursor:"pointer",fontFamily:"monospace"}}>{(activePlanetIdx!==null?"🌱 ":"⭐ ")+((myState&&myState.planetName)||mine.name)}</button>
        ...
        {tab==="players"&&(
          <div style={{flex:1,overflow:"auto",padding:"8px 10px",paddingLeft:"calc(10px + "+safeL+")",paddingRight:"calc(10px + "+safeR+")",WebkitOverflowScrolling:"touch"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:5}}>
              {planets.filter(p=>!p.isWild).map((p,i)=>{
            ...
            {/* Wild planets section */}
            <div style={{fontSize:8,color:"#3a5070",letterSpacing:2,margin:"16px 0 8px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              <span>{"🌱 FRONTIÈRES SAUVAGES "+planets.filter(p=>p.isWild).length}</span>
              <span style={{fontSize:7,color:"#2a3c5a",fontStyle:"italic"}}>{"(colonisables)"}</span>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:5}}>
              {planets.filter(p=>p.isWild).map((p)=>{
                const globalIdx=planets.indexOf(p);
                const[c]=gc(p.type);
                const myEmpire=(loadGS(user.id)||{}).empire||[];
                const isMyColony=myEmpire.includes(globalIdx);
                return(<div key={globalIdx} onClick={()=>pick(globalIdx)} style={{display:"flex",alignItems:"center",gap:6,padding:"6px 8px",borderRadius:7,cursor:"pointer",background:isMyColony?"rgba(100,180,240,0.08)":"rgba(6,15,28,.5)",border:(isMyColony?"1px solid #3060a0":"1px dashed #1a2840"),opacity:isMyColony?1:0.8}}>
                    <div style={{width:7,height:7,borderRadius:"50%",background:c,flexShrink:0,opacity:isMyColony?1:0.5,boxShadow:isMyColony?"0 0 4px "+c:"none"}}/>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:9,color:isMyColony?"#aaccee":"#4a6a88",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{isMyColony?"🌱 ":""}{p.name}</div>
                      <div style={{fontSize:8,color:isMyColony?"#6090c0":"#2a4060",fontStyle:"italic"}}>{isMyColony?"Ma colonie":"Inoccupée"}</div>
                    </div>
                  </div>
                );
              })}
