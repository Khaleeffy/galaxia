...
function CombatScene({targetPlanet,attackerComp,defenderMilitary,onClose,onResolve}){
  ...
  useEffect(function(){
    ...
    function getShipCombatStats(type){
      if(type==="chasseur")return {hp:80,dps:35,range:18};
      if(type==="cargo")return {hp:120,dps:5,range:12};
      if(type==="croiseur")return {hp:400,dps:80,range:25};
      if(type==="bombardier")return {hp:300,dps:120,range:30};
      if(type==="cuirasse")return {hp:1500,dps:200,range:35};
      return {hp:80,dps:20,range:15};
    }
    function getDefenseCombatStats(type){
      if(type==="def_tourelle")return {hp:150,dps:30,range:22};
      if(type==="def_bouclier")return {hp:1000,dps:0,range:0};
      if(type==="def_missiles")return {hp:50,dps:200,range:45};
      if(type==="def_forteresse")return {hp:3000,dps:150,range:30};
    ...
    function spawnLaser(fromPos,toPos,color){
      ...
      if(len<0.1)return;
      var laserGeo=new T.CylinderGeometry(0.15,0.15,len,6);
      var laserMat=new T.MeshBasicMaterial({color:color,transparent:true,opacity:0.9,blending:T.AdditiveBlending});
      var laser=new T.Mesh(laserGeo,laserMat);
      laser.position.set((fromPos.x+toPos.x)/2,(fromPos.y+toPos.y)/2,(fromPos.z+toPos.z)/2);
      laser.lookAt(toPos);
      laser.rotateX(Math.PI/2);
      laser.userData.bornAt=clock.getElapsedTime();
      scene.add(laser);
      lasers.push({mesh:laser,bornAt:clock.getElapsedTime()});
    }
    ...
    function animate(){
      ...
      // Animate active lasers (fade quickly)
      for(var li=lasers.length-1;li>=0;li--){
        var lz=lasers[li];
        var lAge=t-lz.bornAt;
        if(lAge>0.25){
          scene.remove(lz.mesh);
          lz.mesh.geometry.dispose();
          lz.mesh.material.dispose();
          lasers.splice(li,1);
        } else {
          lz.mesh.material.opacity=0.9*(1-lAge/0.25);
        }
      }
