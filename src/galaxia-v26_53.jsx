...
function CombatScene({targetPlanet,attackerComp,defenderMilitary,testMode,placements,onClose,onResolve}){
  ...
  useEffect(function(){
    ...
    // ── Spawn attacker ships (player) — use BattlePlan placements if provided, else default grid ──
    var attackerColor=0x4488dd; // blue
    var attackerShips=[]; // array of {mesh, type}
    ...
    if(hasPlacements){
      ...
      // 2D x ∈ [0,100]% → 3D X ∈ [-125, +125] (lateral spread)
      // 2D y ∈ [52,97]% → 3D Z ∈ [+15, +105] (depth, positive = player side)
      ...
      for(var pli=0;pli<placements.length;pli++){
        ...
        // Square-ish formation: shipsPerRow = ceil(sqrt(count)), spacing 7 units, centered on (centerX, centerZ)
        var shipsPerRow=Math.max(1,Math.ceil(Math.sqrt(pCount)));
        var totalRows=Math.ceil(pCount/shipsPerRow);
        var spacing=7;
        for(var pqi=0;pqi<pCount;pqi++){
          var pship=makeShip(pType,attackerColor);
          pship.scale.set(3.5,3.5,3.5);
          var pcol=pqi%shipsPerRow;
          var prow=Math.floor(pqi/shipsPerRow);
          var pOffsetX=(pcol-(shipsPerRow-1)/2)*spacing;
          var pOffsetZ=(prow-(totalRows-1)/2)*spacing;
          pship.position.set(centerX+pOffsetX,0,centerZ+pOffsetZ);
          pship.rotation.y=Math.PI; // facing the enemy/planet
          scene.add(pship);
          var pStats=getShipCombatStats(pType);
          attackerShips.push({mesh:pship,type:pType,moving:false,destX:0,destZ:0,hp:pStats.hp,maxHp:pStats.hp,dps:pStats.dps,range:pStats.range,fireCooldown:0,target:null,alive:true,side:"attacker"});
        }
      }
    }else{
      // Fallback: default linear grid layout (8 per row at Z=40)
      var spawnPos=[];
      var playerZ=40;
      var rowZ=playerZ;
      var idx=0;
        ...
        if(attackersAlive===0||defendersAlive===0){
          ...
          setTimeout(function(){
            ...
            for(var ki=0;ki<attackerShips.length;ki++){
              ...
              if(ksh.alive)atkSurv[ksh.type]=(atkSurv[ksh.type]||0)+1;
              else atkLost[ksh.type]=(atkLost[ksh.type]||0)+1;
            }
