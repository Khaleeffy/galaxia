...
// ──────────────────────────────────────────────────────
// ── HubErrorBoundary — catches crashes in HubSection and shows recovery UI ──
// If your hub crashes (e.g. corrupt save), you can copy your save data and apply auto-fixes
class HubErrorBoundary extends React.Component {
  ...
  componentDidCatch(error,errorInfo){
    ...
    console.error("Hub crashed:",error,errorInfo);
  }
  copyToClipboard(text){
    ...
    }catch(e){alert("Echec copie. Selectionne le texte manuellement.");}
  }
  applyAutoFix(){
    try{
      var userId=this.props.userId;
      var raw=localStorage.getItem("gx_state_"+userId);
      if(!raw){alert("Save introuvable");return;}
      var s=JSON.parse(raw);
      // Common fixes:
      // 1. Clear stuck queue (build/research that may be corrupted)
      if(s.queue&&typeof s.queue!=="object")s.queue=null;
      if(s.queue&&!s.queue.finishAt)s.queue=null;
      // 2. Clear corrupted fleets (missing required fields)
      ...
      // 3. Ensure ships object is well-formed (cargo bug)
      if(!s.ships||typeof s.ships!=="object")s.ships={};
      var shipKeys=Object.keys(s.ships);
      for(var i=0;i<shipKeys.length;i++){
        var v=s.ships[shipKeys[i]];
        if(typeof v!=="number"||isNaN(v)||v<0)s.ships[shipKeys[i]]=0;
      }
      // 4. Ensure resources are valid numbers
      if(!s.resources)s.resources={metal:0,crystal:0,deut:0};
      var resKeys=["metal","crystal","deut"];
      for(var j=0;j<resKeys.length;j++){
        if(typeof s.resources[resKeys[j]]!=="number"||isNaN(s.resources[resKeys[j]]))s.resources[resKeys[j]]=0;
      }
      // 5. Ensure anomalies array is well-formed
      ...
      // 6. Clear pending notifs
      s.anomalyNotifs=[];
      // 7. Reset lastTick to now to avoid huge tick gap
      s.lastTick=Date.now();
      // Save the cleaned state
      localStorage.setItem("gx_state_"+userId,JSON.stringify(s));
      alert("Reparation appliquee ! Rechargement...");
      setTimeout(function(){window.location.reload();},500);
    }catch(e){
      alert("Echec reparation: "+e.message);
    }
  }
  render(){
    if(this.state.hasError){
      var userId=this.props.userId;
      var rawSave="";
      try{rawSave=localStorage.getItem("gx_state_"+userId)||"(vide)";}catch(e){rawSave="(erreur acces)";}
      var errMsg=this.state.error?(this.state.error.message||String(this.state.error)):"Erreur inconnue";
      var self=this;
      return (
        <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,8,0.97)",zIndex:9999,padding:20,overflowY:"auto",fontFamily:"monospace",color:"#d8b8ff"}}>
          <div style={{maxWidth:600,margin:"0 auto"}}>
            <div style={{fontSize:18,fontWeight:"bold",color:"#ff6688",marginBottom:14}}>{"\u26A0 ERREUR DU HUB"}</div>
            <div style={{fontSize:11,color:"#cc8899",marginBottom:14,lineHeight:1.5}}>
              {"Le hub planete a plante au rendu. Voici quelques options :"}
            </div>
            <div style={{background:"rgba(80,30,30,0.5)",border:"1px solid #aa4466",borderRadius:8,padding:12,marginBottom:12,fontSize:10,color:"#ffaabb",wordBreak:"break-word"}}>
              <div style={{fontWeight:"bold",marginBottom:4}}>{"Erreur :"}</div>
              <div>{errMsg}</div>
            </div>
            <button onClick={function(){self.applyAutoFix();}} style={{width:"100%",padding:"12px 0",background:"linear-gradient(135deg,rgba(40,80,50,0.95),rgba(20,50,30,0.95))",border:"1px solid #66cc88",borderRadius:8,color:"#fff",fontSize:12,fontWeight:"bold",cursor:"pointer",fontFamily:"monospace",letterSpacing:1,marginBottom:10}}>
              {"\u{1F527} REPARATION AUTO"}
            </button>
            <div style={{fontSize:9,color:"#9a8ac0",marginBottom:14,lineHeight:1.4,textAlign:"center"}}>
              {"(Nettoie les flottes/anomalies/queue corrompues, garde tes batiments et ressources)"}
            </div>
            <button onClick={function(){self.copyToClipboard(rawSave);}} style={{width:"100%",padding:"10px 0",background:"linear-gradient(135deg,rgba(40,50,80,0.95),rgba(20,30,50,0.95))",border:"1px solid #6688cc",borderRadius:8,color:"#fff",fontSize:11,fontWeight:"bold",cursor:"pointer",fontFamily:"monospace",letterSpacing:1,marginBottom:10}}>
              {"\u{1F4CB} COPIER LA SAVE (pour Claude)"}
            </button>
            <button onClick={function(){self.copyToClipboard(errMsg);}} style={{width:"100%",padding:"10px 0",background:"rgba(60,40,80,0.85)",border:"1px solid #8866cc",borderRadius:8,color:"#fff",fontSize:11,fontWeight:"bold",cursor:"pointer",fontFamily:"monospace",letterSpacing:1,marginBottom:14}}>
              {"\u{1F4CB} COPIER L\u2019ERREUR"}
            </button>
            <button onClick={function(){self.props.onClose&&self.props.onClose();self.setState({hasError:false,error:null});}} style={{width:"100%",padding:"8px 0",background:"rgba(20,15,30,0.8)",border:"1px solid #4a3070",borderRadius:6,color:"#9a8ac0",fontSize:10,cursor:"pointer",fontFamily:"monospace",letterSpacing:1}}>
              {"FERMER LE HUB"}
            </button>
            <details style={{marginTop:18,fontSize:9,color:"#7a6aa0"}}>
              <summary style={{cursor:"pointer",padding:6}}>{"Voir la save (avance)"}</summary>
              <textarea readOnly value={rawSave} style={{width:"100%",height:200,background:"#0a0612",color:"#9a8ac0",border:"1px solid #4a3070",borderRadius:4,padding:8,fontSize:9,fontFamily:"monospace",marginTop:6}}/>
            </details>
          </div>
        </div>
      );
...
function Viewer3D({planet,allPlanets,onClose,isPlayer,owner,state,onStateChange,onManage,userId,natalState,onColonize,alreadyColonized}){
  ...
  return(<div style={outerSt}>
    ...
    {hubSection&&isPlayer&&state&&userId&&<HubErrorBoundary userId={userId} onClose={()=>setHubSection(null)}><HubSection section={hubSection} planet={planet} userId={userId} planets={allPlanets} myIdx={planet&&planet.i} initialState={state} onStateChange={onStateChange} onClose={()=>setHubSection(null)} onOpenZoneInfo={z=>setSelHubZoneInfo(z)}/></HubErrorBoundary>}
