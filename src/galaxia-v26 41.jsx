  ...
  const recoveryData=(()=>{
    ...
    try{
      ...
      Object.values(allUsers).forEach(u=>{
        if(u.isBot)return; // skip bots
        const saveKey="gx_state_"+u.id;
        const saveStr=localStorage.getItem(saveKey)||"";
        const sizeKB=(saveStr.length/1024).toFixed(1);
        out.push({email:u.email,name:u.name||"(sans nom)",id:u.id,sizeKB});
      });
      // Sort by save size descending — biggest first (most likely your real account)
      out.sort((a,b)=>parseFloat(b.sizeKB)-parseFloat(a.sizeKB));
      return out;
    }catch(e){return [{email:"erreur: "+e.message,name:"",id:"",sizeKB:"0"}];}
  })();
  const go=async()=>{
  ...
  return(<div style={{width:"100vw",height:"100svh",minHeight:"100dvh",background:"#000005",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",position:"relative",paddingBottom:"env(safe-area-inset-bottom,0px)",paddingTop:"env(safe-area-inset-top,0px)"}}>
      ...
      <div style={{display:"flex",width:"100%",maxWidth:700,maxHeight:"90dvh",margin:"0 16px",background:"linear-gradient(160deg,#060f1c,#08141f)",border:"1px solid #0d2035",borderRadius:16,overflow:"hidden",boxShadow:"0 0 60px rgba(0,20,60,.8)",position:"relative",zIndex:1}}>
        ...
        <div style={{flex:1,padding:"16px 18px",overflow:"auto",WebkitOverflowScrolling:"touch"}}>
          ...
          {showRecovery&&recoveryData&&<div style={{marginBottom:10,padding:10,background:"rgba(20,40,30,0.6)",border:"1px solid #2a4a2a",borderRadius:7,maxHeight:200,overflow:"auto",WebkitOverflowScrolling:"touch"}}>
            <div style={{fontSize:9,color:"#88dd88",fontFamily:"monospace",marginBottom:6,letterSpacing:1.2}}>{"Comptes trouvés ("+recoveryData.length+") — la grosse save = ton compte"}</div>
            {recoveryData.length===0?<div style={{fontSize:10,color:"#aaccaa",fontFamily:"monospace"}}>Aucun compte trouvé.</div>:recoveryData.map((u,i)=>(
              <div key={i} style={{padding:"8px 10px",marginBottom:6,background:"rgba(10,20,15,0.7)",border:"1px solid #1a2a1a",borderRadius:5,fontFamily:"monospace",cursor:"pointer"}} onClick={()=>{setEmail(u.email);setShowRecovery(false);}}>
