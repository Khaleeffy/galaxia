...
function CombatScene({targetPlanet,attackerComp,defenderMilitary,onClose,onResolve}){
  ...
  useEffect(function(){
    ...
    var cam=new T.PerspectiveCamera(50,W/H,0.1,3000);
    // 45-degree top-down view, looking from above and slightly behind
    cam.position.set(0,180,160);
    cam.lookAt(0,0,0);
    ...
    planetMesh.position.set(0,-90,-30);
    scene.add(planetMesh);
    
    // Atmosphere glow around planet
    var atmGeo=new T.SphereGeometry(85,32,24);
    ...
    for(var atk=0;atk<attackerTypes.length;atk++){
      ...
      for(var qi=0;qi<qty;qi++){
        ...
        // Distribute in a grid: 8 per row, spaced 6 units
        var row=Math.floor(idx/8);
        var col=idx%8;
        ship.position.set((col-3.5)*6,0,rowZ+row*6);
    ...
    for(var dfk=0;dfk<attackerTypes.length;dfk++){
      ...
      for(var dqi=0;dqi<dqty;dqi++){
        ...
        var dcol=idx%8;
        dship.position.set((dcol-3.5)*6,0,enemyZ-drow*6);
    ...
    for(var dt=0;dt<defTypes.length;dt++){
      ...
      for(var dvq=0;dvq<visualQty;dvq++){
        ...
        var dCol=defIdx%10;
        defMesh.position.set((dCol-4.5)*8,-2,defZ-dRow*7);
        scene.add(defMesh);
    ...
    function animate(){
      ...
      for(var si=0;si<attackerShips.length;si++){
        ...
        s.position.y=Math.sin(t*1.2+si*0.7)*0.4;
      }
      for(var di=0;di<defenderShips.length;di++){
        var d=defenderShips[di].mesh;
        d.position.y=Math.sin(t*1.0+di*0.5+1.5)*0.4;
      }
      renderer.render(scene,cam);
    }
    animate();
