...
function BattlePlanScreen({plan,onCancel,onEngage}){
  ...
  return (
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"radial-gradient(ellipse at top,rgba(20,15,40,0.98) 0%,rgba(5,3,15,0.98) 80%)",zIndex:300,display:"flex",flexDirection:"column",overflow:"hidden",paddingTop:"env(safe-area-inset-top,0px)",paddingBottom:"env(safe-area-inset-bottom,0px)"}}>
      ...
      {/* Bottom: ship reserve panel — compact cards */}
      <div style={{padding:"6px 10px 4px",flexShrink:0}}>
        ...
        <div style={{display:"flex",gap:5}}>
          {Object.keys(SHIP_INFO).map(function(shipId){
            ...
            return (
              <div key={shipId} style={{flex:1,minWidth:0,padding:"5px 2px",background:"rgba(20,30,50,0.7)",border:"1px solid "+info.color+"55",borderRadius:6,textAlign:"center"}}>
                <div style={{fontSize:16,filter:"drop-shadow(0 0 3px "+info.color+"66)",lineHeight:1}}>{info.icon}</div>
                <div style={{fontSize:8,color:info.color,fontFamily:"monospace",letterSpacing:0.3,marginTop:3}}>{info.name}</div>
                <div style={{fontSize:13,color:"#fff",fontFamily:"monospace",fontWeight:"bold",marginTop:1}}>{qty}</div>
              </div>
            );
          })}
        </div>
