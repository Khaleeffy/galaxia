...
function MissionsModal({state,onClaim,onClose}){
  ...
  return(<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,zIndex:600,background:"rgba(0,5,15,0.92)",backdropFilter:"blur(10px)",display:"flex",flexDirection:"column",fontFamily:"monospace",overflow:"hidden"}}>
    ...
    {lockedInfo&&(()=>{
      ...
      return(
        <div onClick={()=>setLockedInfo(null)} style={{position:"fixed",inset:0,background:"rgba(0,5,15,0.9)",zIndex:700,display:"flex",alignItems:"center",justifyContent:"center",padding:16,backdropFilter:"blur(8px)"}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"4px 10px",paddingLeft:"calc(10px + "+safeL+")",paddingRight:"calc(10px + "+safeR+")",background:"rgba(4,12,24,.95)",borderBottom:"1px solid #0a1828",flexShrink:0,height:38}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          {myState&&(()=>{
            ...
            return(<div style={{display:"flex",gap:6,fontSize:9,fontFamily:"monospace"}}>
              <span onClick={()=>{AudioEngine.safeSoftClick&&AudioEngine.safeSoftClick();setSelResource("metal");}} style={{color:wM||"#7090b8",cursor:"pointer"}}>{"⛏️"}{fmt(mM)}<span style={{opacity:0.5,fontSize:8}}>{"/"+fmt(maxM)}</span></span>
              <span onClick={()=>{AudioEngine.safeSoftClick&&AudioEngine.safeSoftClick();setSelResource("crystal");}} style={{color:wC||"#40a8c8",cursor:"pointer"}}>{"💎"}{fmt(mC)}<span style={{opacity:0.5,fontSize:8}}>{"/"+fmt(maxC)}</span></span>
              <span onClick={()=>{AudioEngine.safeSoftClick&&AudioEngine.safeSoftClick();setSelResource("deut");}} style={{color:wD||"#40c890",cursor:"pointer"}}>{"⚗️"}{fmt(mD)}<span style={{opacity:0.5,fontSize:8}}>{"/"+fmt(maxD)}</span></span>
              {(()=>{
          ...
          {myState&&(()=>{
            ...
            const readyCount=activeMissions.filter(m=>!m._locked&&m.check(natStForMissions)).length;
          ...
          <button onClick={()=>{AudioEngine.safeClick();setViewer(mine);}} style={{padding:"4px 8px",background:activePlanetIdx!==null?"rgba(100,180,240,.1)":"rgba(255,215,0,.1)",border:"1px solid "+(activePlanetIdx!==null?"#1e4a80":"#3a3000"),borderRadius:6,color:activePlanetIdx!==null?"#aaccee":"#ffd700",fontSize:9,cursor:"pointer",fontFamily:"monospace"}}>{(activePlanetIdx!==null?"🌱 ":"⭐ ")+((myState&&myState.planetName)||mine.name)}</button>
